...
def build_existing_suite_registry() -> tuple[ExistingSuite, ...]:
    registry = (
        ...
        ExistingSuite(
            id="ui-recommendation-profiles",
            title="推薦分析 Profile / Regime lifecycle UI 測試",
            modes=(HealthcheckMode.FULL,),
            command=(PYTHON, "-m", "pytest", "tests/test_ui_qt_recommendation_profiles.py", "-q", "-o", "addopts="),
            non_destructive=True,
            path="tests/test_ui_qt_recommendation_profiles.py",
            tabs=("recommendation",),
            covered_healthcheck_ids=("R-002", "R-003", "R-004", "RECOMMEND-ISSUE-001"),
            covered_flow_ids=("research-validation-loop",),
        ),
        ExistingSuite(
            id="ui-recommendation-next-steps",
            title="推薦分析下一步文案 / 跨頁導引測試",
            modes=(HealthcheckMode.FULL,),
            command=(PYTHON, "-m", "pytest", "tests/test_ui_qt_recommendation_next_steps_copy_text.py", "-q", "-o", "addopts="),
            non_destructive=True,
            path="tests/test_ui_qt_recommendation_next_steps_copy_text.py",
            tabs=("recommendation", "watchlist", "portfolio", "cross-flow"),
            covered_healthcheck_ids=("R-016", "R-017", "R-018", "R-020", "X-004", "X-005"),
            covered_flow_ids=("research-validation-loop", "portfolio-check-loop"),
        ),
        ExistingSuite(
            id="ui-watchlist-candidate-pool",
            title="觀察清單候選池 / Research Lab handoff 文案測試",
            modes=(HealthcheckMode.FULL,),
            command=(PYTHON, "-m", "pytest", "tests/test_ui_qt_watchlist_candidate_pool_copy_text.py", "-q", "-o", "addopts="),
            non_destructive=True,
            path="tests/test_ui_qt_watchlist_candidate_pool_copy_text.py",
            tabs=("watchlist", "research", "cross-flow"),
            covered_healthcheck_ids=("W-001", "W-005", "W-013", "X-004"),
            covered_flow_ids=("research-validation-loop",),
        ),
        ExistingSuite(
            id="ui-portfolio-view",
            title="持倉管理 fake-service UI 測試",
            modes=(HealthcheckMode.FULL,),
            command=(PYTHON, "-m", "pytest", "tests/test_ui_qt_portfolio_view.py", "-q", "-o", "addopts="),
            non_destructive=True,
            path="tests/test_ui_qt_portfolio_view.py",
            tabs=("portfolio", "cross-flow"),
            covered_healthcheck_ids=("P-001", "P-005", "P-008", "P-013", "P-014", "P-015", "P-016"),
            covered_flow_ids=("portfolio-check-loop",),
        ),
        ExistingSuite(
            id="ui-runtime-view",
            title="Runtime Observatory 中文化 / 判讀 UI 測試",
            modes=(HealthcheckMode.FULL,),
            command=(PYTHON, "-m", "pytest", "tests/test_ui_qt_runtime_view.py", "-q", "-o", "addopts="),
            non_destructive=True,
            path="tests/test_ui_qt_runtime_view.py",
            tabs=("runtime",),
            covered_healthcheck_ids=("RT-001", "RT-002", "RT-003", "RT-004", "RT-005", "RT-006"),
            covered_flow_ids=("governance-observability-loop",),
        ),
        ExistingSuite(
            id="qa-update-tab",
