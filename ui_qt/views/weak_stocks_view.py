...
class WeakStocksView(QWidget):
    ...
    def _refresh_stocks(self, use_cache: bool = True):
        ...
        try:
            ...
            # 調用服務（重新計算）
            dto_method = getattr(self.screening_service, "get_weak_stocks_dto", None)
            result = (
                dto_method(period=period, top_n=50).to_legacy()
                if callable(dto_method)
                else self.screening_service.get_weak_stocks(period=period, top_n=50)
            )
            
            # 處理新的返回格式（元組：DataFrame, universe_count）
            if isinstance(result, tuple):
                if len(result) != 2:
                    raise TypeError("弱勢個股服務回傳格式不是 (DataFrame, universe_count)")
            ...
            if not isinstance(df, pd.DataFrame):
                raise TypeError("弱勢個股服務未回傳 DataFrame")
            ...
            self.stocks_model = PandasTableModel(self._empty_frame(), red_positive_columns={'跌幅%'})
            self.stocks_table.setModel(self.stocks_model)
            self.refresh_btn.setVisible(False)
            self.load_btn.setVisible(True)
            if self.watchlist_service:
