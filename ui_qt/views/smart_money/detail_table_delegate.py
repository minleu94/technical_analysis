...
class DetailTableDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option, index):
        ...
        if col_name == '淨買賣超' and hasattr(model, '_dataframe'):
            ...
            if 0 <= row_idx < len(df):
                ...
                # 計算該列最大絕對值以進行比例分配
                max_abs_val = df[col_name].abs().max()
                if pd.isna(max_abs_val) or max_abs_val == 0:
                    max_abs_val = 1.0
                    
                # 繪製雙向水平長條圖
                self._draw_bidirectional_bar(painter, rect, val, max_abs_val)
        else:
            # 預設文字繪製
    ...
    def _draw_bidirectional_bar(self, painter: QPainter, rect: QRect, val: float, max_val: float):
