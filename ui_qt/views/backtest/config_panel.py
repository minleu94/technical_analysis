...
class BacktestConfigPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        # 取消執行按鈕
        self.cancel_btn = QPushButton("取消執行")
        ...
        self.cancel_btn.clicked.connect(lambda: self.parent_view._on_cancel_clicked())
        progress_layout.addWidget(self.cancel_btn)

        # 並行開關 (防呆：僅在批次選股模式顯示)
        self.parallel_checkbox = QCheckBox("啟用多核心並行 (僅適用大批次回測)")
        self.parallel_checkbox.setChecked(False)
        self.parallel_checkbox.setVisible(False)
    ...
    def _update_ui_state_by_mode(self, mode_id: str):
        ...
        # 7. 並行開關 (防呆：僅在批次選股模式下顯示)
        if hasattr(self, 'parallel_checkbox') and self.parallel_checkbox:
            self.parallel_checkbox.setVisible(is_batch)
