...
from ui_qt.views.backtest import research_run_metadata
...
class BacktestView(QWidget):
    ...
    def _save_single_backtest_to_research_registry(self, run_name: str, notes: str) -> str:
        ...
        metadata = research_run_metadata.build_single_backtest_metadata(
    ...
    def _save_recommendation_portfolio_to_research_registry(self, run_name: str, notes: str) -> str:
        ...
        metadata = research_run_metadata.build_recommendation_portfolio_metadata(
    ...
    def _factor_save_kwargs(self, details: dict[str, Any]) -> dict[str, Any]:
        return research_run_metadata.factor_save_kwargs(details)

    def _single_backtest_metrics(self, report: BacktestReportDTO) -> dict[str, Any]:
        return research_run_metadata.single_backtest_metrics(report)

    def _next_research_run_id(self, prefix: str) -> str:
        return research_run_metadata.next_research_run_id(prefix)

    def _research_payload_hash(self, payload: dict[str, Any]) -> str:
        return research_run_metadata.research_payload_hash(payload)

    def _json_safe(self, value: Any) -> Any:
        return research_run_metadata.json_safe(value)

    def _money_to_cents(self, value: Any) -> int:
        return research_run_metadata.money_to_cents(value)

    def _bps_to_bp_x100(self, value: Any) -> int:
        return research_run_metadata.bps_to_bp_x100(value)

    def _pct_to_bp(self, value: Any) -> int | None:
        return research_run_metadata.pct_to_bp(value)
        ...
        if action == action_add_portfolio:
            ...
            if hasattr(main_window, 'portfolio_service'):
                ...
                if dialog.exec() == QDialog.DialogCode.Accepted:
                    ...
                    if run_id and run_id != "unsaved_backtest_run" and self.run_repository:
                        try:
                            run_obj = self.run_repository.get_run(run_id)
                            if run_obj:
                                promoted_version_id = run_obj.promoted_version_id
                        except Exception as e:
                            logger.warning("查詢回測已升級版本 ID 失敗: %s", e)
