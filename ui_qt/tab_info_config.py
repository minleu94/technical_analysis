...
TAB_INFO = {
    ...
    "smart_money_flow": {
        ...
        "what": "這是主力資金追蹤系統，將零散的分點進出資料轉化為可操作的交易信號，具備雙視角分析能力。",
        "how_to_use": [
            "使用「股票視角」找出多個主力分點正在共同佈局（連續吸籌、高集中度）的標的",
            "使用「分點視角」追蹤特定大戶（如凱基台北）最近正在買賣哪些股票",
            "查看「主力分數」與「信號標籤」快速篩選出高勝率的候選標的",
            "將感興趣的股票「加入觀察清單」以便後續進行技術分析與策略回測"
        ],
        "misconceptions": [
            "不是絕對的買賣點：主力買進不代表立刻會漲，可能只是在建倉",
            "分數高不保證獲利：高分數代表籌碼集中且方向一致，仍需配合價格行為（如突破）",
            "分點會換手或對敲：需防範主力左手換右手的假象，建議結合多個分點一致性來判斷"
        ]
    },
    "portfolio": {
