...
class WatchlistAnalysisService:
    ...
    def fetch(self, stock_code: str, source_id: str, as_of_date: date) -> WatchlistAnalysisDTO:
        ...
        with closing(sqlite3.connect(f"{db_path.resolve().as_uri()}?mode=ro", uri=True)) as conn:
            ...
            if source_id:
                row = conn.execute("SELECT data_path FROM runs WHERE result_id = ?", (source_id,)).fetchone()
            else:
                row = conn.execute("SELECT data_path FROM runs ORDER BY created_at DESC, result_id LIMIT 1").fetchone()
        if row is None:
            return WatchlistAnalysisDTO(stock_code, "missing", message="找不到對應的已保存推薦來源。")
        path = Path(row[0]).resolve()
        if not path.is_relative_to(self.runs_dir.resolve()):
            raise ValueError("推薦來源路徑超出保存目錄")
        result = RecommendationResultDTO.from_dict(json.loads(path.read_text(encoding="utf-8")))
