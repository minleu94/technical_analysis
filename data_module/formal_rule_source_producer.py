...
它只在 PIT cutoff 前建立當日 clock、universe identity 與 machine
revalidation receipt；09:00--13:30 的正式 Rule capture 仍由既有
``prospective_rule_only_decision`` producer 執行。任何輸出都限制在 repo
publication root，D 槽 market database 只以 SQLite read-only 讀取。
...
MACHINE_RULE_SOURCE_PRODUCER_VERSION = "machine-revalidated-rule-source.v2"
    ...
    result.update(
        {
            ...
            "machine_review_version": MACHINE_RULE_SOURCE_PRODUCER_VERSION,
...
def _verify_existing_bundle(
    ...
    *,
    ...
    required = [paths[name] for name in ("clock", "owner", "symbols", "identity", "receipt")]
...
def validate_machine_revalidation_bundle(
    ...
    *,
    ...
    if receipt.get("producer_version") != MACHINE_RULE_SOURCE_PRODUCER_VERSION:
        raise FormalRuleSourceProducerError(
    ...
    if receipt.get("market_db_file_hash") != market_hash or identity.get(
        "source_db_file_hash"
    ) != market_hash or owner.get("revalidated_market_db_file_hash") != market_hash:
        raise FormalRuleSourceProducerError(
            "machine_revalidation_market_db_file_hash_mismatch"
        )
    ...
    # receipt 只是一個可攜的索引；正式 consumer 還要用同一份唯讀 DB
    # 重新讀 T-1 window、重算 source hash 與 frozen universe。這樣 caller
    # 不能靠重簽 receipt/identity 的自我宣告把改過的 rows 帶入決議。
    try:
    ...
    if (
        current_max_available > receipt_observed
        or current_max_available > observed_utc
    ):
        raise FormalRuleSourceProducerError(
            "machine_revalidation_current_source_available_after_observation"
        )
    if str(current_window.max_available_timestamp) != max_available_raw:
        raise FormalRuleSourceProducerError(
            "machine_revalidation_current_source_max_available_mismatch"
        )
    try:
    ...
    if file_sha256(market_path) != market_hash:
        raise FormalRuleSourceProducerError(
            "machine_revalidation_market_db_changed_during_read"
        )
    files = receipt.get("files")
    if not isinstance(files, Mapping):
        raise FormalRuleSourceProducerError(
...
def produce_current_rule_source_bundle(
    *,
    ...
    ``now`` 僅供隔離測試注入；公開 CLI 不提供時間覆寫。若現在已越過
    PIT 08:30 且當日 bundle 尚未存在，入口保持 blocked，避免用事後時間
    冒充 pre-open clock。
    """

    observed_utc, observed_taipei = _normalise_observed(now)
    target_day = observed_taipei.date()
    if observed_taipei.timetz().replace(tzinfo=None) >= TAIWAN_PIT_CUTOFF:
        # 仍會在下方讀一次 current source 以找 deterministic bundle；若找
        # 不到既有 bundle，絕不在 cutoff 後新建同日 clock。
        past_cutoff = True
    else:
        past_cutoff = False
    ...
    if past_cutoff:
        raise FormalRuleSourceWaiting(
            "same_day_rule_source_preopen_window_missed"
        )
