...
> **最後整理**：2026-06-24
> **目前主線**：Month 6.1 Strategy Lifecycle 人工審核深化、全 UI 健檢收尾與資料更新可靠性治理
> **主要入口**：`ui_qt/main.py`

baldr 是一套台股研究與投資決策工作台，目標是把資料更新、市場觀察、推薦分析、策略回測、籌碼監控、基本面因子、每日決策、策略生命週期與持倉管理串成可驗證、可回溯、可演化的研究與決策流程。

本檔只保留專案最短入口。狀態判讀請以 [docs/00_core/PROJECT_SNAPSHOT.md](docs/00_core/PROJECT_SNAPSHOT.md) 為準；未來 6 個月工程路線請以 [docs/00_core/ROADMAP_6M_ENGINEERING.md](docs/00_core/ROADMAP_6M_ENGINEERING.md) 為準；目前架構與模組邊界請以 [docs/01_architecture/system_architecture.md](docs/01_architecture/system_architecture.md) 為準。
...
- UI 健檢與呈現層：已導入 Midnight Analyst theme tokens、全域 QSS 與設計系統文件；Healthcheck batches 已推進 Daily Decision Desk dashboard、Smart Money 語意診斷、推薦 Profile lifecycle、Research Lab 結果呈現與最佳化工作排程邊界。
- 資料更新可靠性：MoneyDJ 券商分點更新已加入 HTTP fast path 與交易日預檢，正常頁面不再先啟動 Selenium，並保留 Selenium fallback。

## 接下來先做什麼

- 繼續 Month 6.1，聚焦 lifecycle QA、manual approval workflow、Review Dashboard、Evidence Explainability 與完整驗收清單。
- 收斂全 UI 健檢後續批次，確保 8 個頂層工作區的版面、文字密度、控制項一致性與狀態提示不偏離 service / DTO 邊界。
- 維持資料更新管線的非破壞式驗證，尤其是 MoneyDJ fast path、TPEX daily price 補齊、SQLite sync 與 broker flow 品質狀態。
...
- [docs/01_architecture/system_architecture.md](docs/01_architecture/system_architecture.md)：目前系統架構、模組邊界、資料流與高風險技術契約。
...
| `decision_module/` | Decision Domain，推薦、打分、篩選、Regime、因子、權重契約與籌碼信號。 |
...
| `data_module/` | 資料設定、載入、處理、SQLite / CSV 邊界、基本面資料層與受控 backfill workflow。 |
...
- 2026-06-24：依目前 Snapshot 與 system architecture 更新根目錄 README，補上 Month 6.1、Healthcheck batches、MoneyDJ HTTP fast path、架構權威入口與驗證邊界。
