...
- V2.0 Phase 1 read-only Workbench prototype slice、Phase 1.5 / Phase 2 前置 formal read-only source adapter 與 Phase 2 Workbench MVP shell 已完成第一版：新增 `WorkbenchDashboardDTO`、read-only composer、Historical Replay JSON summary adapter、`WorkbenchSourceService`、`scripts/inspect_v2_workbench_prototype.py` prototype CLI，以及 Qt `決策工作台` read-only view / table models / main-tab integration。CLI 與 UI 都只讀 existing sources / DTO，不寫 evidence、不建立 scheduler、不下單、不套用 lifecycle action；完整 operating loop 與舊 Tab drill-down 仍待後續 gate。
