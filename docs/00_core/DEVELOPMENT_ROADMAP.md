...
### 本週 Done（券商分點擴充、長碼解密與總公司判定 + 數據更新流程快速/安全分流 + Phase 4.2 持倉層籌碼面風險提示與下鑽整合 + Phase 4.1 持倉管理深化 BUG 修正與 Gap 補全 + Research Lab 工作流重整與說明文檔同步）
...
2. ✅ **回測時間軸契約治理**（2026-06-10）
   * 已修正 `BrokerSimulator` 的 `next_open` 帳務錯位，T 日訊號不再提前反映 T+1 成交
   * `execution_price="close"` 與推薦組合回測同日收盤成交假設已加入警告與 metadata 揭露
   * 已新增 no-look-ahead / benchmark / timeline contract 測試

3. ✅ **金融核心數值治理（核心金額邊界階段性完成）**
   * ✅ Phase 1 已啟動：新增 `financial_module/units.py`，以 `Decimal` / 整數股數 / 基點建立金額與整股邊界；`app_module/financial_units.py` 僅保留相容 re-export
   * ✅ `BrokerSimulator` 買賣手續費、滑價、證交稅與整股股數計算已改由金融單位 helper 處理，`Trade` DTO 邊界維持 float 以保持 UI / repository 相容
   * ✅ `portfolio_module/core.py` 的平均成本、已實現損益與投入金額已改用 Decimal 邊界計算，並拒絕非整數股數
   * ✅ `backtest_module/performance_metrics.py` 的交易損益統計、`app_module/recommendation_portfolio_backtest_service.py` / DTO 的推薦組合 PnL 與 mark-to-market PnL、`app_module/portfolio_service.py` 的 Portfolio summary 加總已改用 Decimal 金額邊界
   * ✅ 金融 float 邊界與防回歸掃描治理已完成：
     * 建立固定金融核心白名單（6 個核心模組檔案）。
     * 利用 AST 靜態解析偵測 `float()`、`.astype(float)`、`dtype=float`。
     * 實施 `dto`、`analytics`、`visualization` 註解分類標記管制（`# numeric-boundary: <category>`）。
     * 建立 pytest 整合的 repository gate，確保白名單內無任何未標記的浮點數邊界。

4. ✅ **Phase 4.1 Portfolio 深化**（2026-06-11）
   * [x] 策略版本追蹤視圖
   * [x] Price 對照（進場價 vs 目前價 vs 停損停利區間）
   * [x] 持倉層風險提示
5. ✅ **Phase 4.2 Portfolio 籌碼監控與下鑽**（2026-06-11）
   * [x] 券商買賣超累計與連續天數計算（PortfolioChipService）
   * [x] 籌碼監控 UI Tab 與追蹤分點買賣明細表格
   * [x] 持倉到主力流向的單向/雙向下鑽連動
