...
3. 🚧 **金融核心數值治理**
   * ✅ Phase 1 已啟動：新增 `app_module/financial_units.py`，以 `Decimal` / 整數股數 / 基點建立金額與整股邊界
   * ✅ `BrokerSimulator` 買賣手續費、滑價、證交稅與整股股數計算已改由金融單位 helper 處理，`Trade` DTO 邊界維持 float 以保持 UI / repository 相容
   * ⏳ 下一步：將 `performance_metrics.py`、`recommendation_portfolio_backtest_service.py`、`portfolio_module/core.py`、`portfolio_service.py` 逐步遷移到相同邊界
...
* ⚠️ **金融核心裸 float**：已建立 `financial_units.py` 與 `BrokerSimulator` 成本計算邊界，但 `performance_metrics.py`、`recommendation_portfolio_backtest_service.py`、`portfolio_module/core.py` 等仍待遷移。
