...
### 下一步 Next（Phase 5 效能與研究輸出 → 參數打分治理）

1. 🎯 **P0：指標參數改進與評分分位數化設計（Phase 2.5 優先級 3）**
   - 將評分引擎的 `buy_score` / `sell_score` 由原始絕對數值改為動態分位數（Quantile-based）打分，提升跨市場與 Regime 的穩定性。
   - 標準化與改進核心指標參數（RSI/MACD/KD/ADX/MA/ATR/BBANDS），避免單一股票過擬合。
