...
3. 🚧 **金融核心數值治理**
   ...
   * ⏳ 下一步：將 `performance_metrics.py`、`recommendation_portfolio_backtest_service.py`、`portfolio_service.py` 逐步遷移到相同邊界
...
* ⚠️ **金融核心裸 float**：已建立 `financial_module/units.py` 與 `BrokerSimulator` / Portfolio domain 成本計算邊界，但 `performance_metrics.py`、`recommendation_portfolio_backtest_service.py` 等仍待遷移。
