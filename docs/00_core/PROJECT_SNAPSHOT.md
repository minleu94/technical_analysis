...
2. **下一階段主線：Evidence-Driven baldr + V1.1 / V1.2 / V1.3 / V1.4 / V1.5 / V1.6 v1 已完成，V1.7-V1.9 保持版本化準備**：Evidence Event Store v1 / Forward Outcome Calculator v1 / Evidence Importers v1 / E2E smoke / Forward Performance Read Model v1 / Daily Decision Desk durable snapshot source / source coverage inspection v1 / Forward Performance Dashboard read-only UI v1 / Evidence Pipeline Runner dry-run v1 / working-copy DB smoke v1 / scheduler approval checklist v1 / Live vs Research Gap linkage v1 / Signal Decay Monitor v1 / Decision Quality Review v1 / Evidence Review Dashboards read-only UI pack v1 / Evidence Review UI smoke checklist / multi-day dry-run record scaffold / safe scheduled CMD wrappers / V1.3 weekly evidence operations package / V1.4 weekly review history / V1.5 data credibility gate / V1.6 cross-sectional factor pipeline 已建立；每日 05:30 Codex read-only 摘要、Evidence Review UI smoke 與 multi-day dry-run evidence 可開始背景累積。V1.1 已補推薦 Profile 可見性、推薦回放語意與 Profile replay comparison service；V1.2 已補 replay 訓練 / 驗證分離、rolling risk、microstructure preflight 與 relative attribution；V1.3 已補 manual approval summary、weekly review CLI、signal decay manual lifecycle candidate 與 action item planning；V1.4 已補 weekly review history repository、CLI save/list 與 Research Lab `覆盤歷史` 唯讀子頁；V1.5 已補 Data Source Capability Registry、corporate action / adjusted price policy、governed microstructure source metadata 與 centralized Evidence Source Coverage Service；V1.6 已補 daily factor snapshot DTO / repository / pipeline、concept basket available-date gate、integer factor rank / quantile 與 read-only attribution summary CLI。why-not / liquidity optional payload 缺口現在是 warnings / V1.7 negative evidence 後續，不再是 durable source blocking gap；watchlist / portfolio 仍需真實 workflow 與樣本累積。完整 Daily Decision / Market Watch 合併仍留待 V2.0 評估；下一步是用 weekly review + history 實際累積數週覆盤證據，並準備 V1.7 Negative Evidence、V1.8 Portfolio Sandbox 與 V1.9 Read-only Agent / MCP。Production scheduler implementation 仍需 durable source coverage、人工 approval 與明確設計後才可進行。
...
- **Evidence Source Coverage Service**：新增 `EvidenceSourceCoverageService`，讓 CLI、pipeline runner 與 scheduler readiness evaluator 共用同一份 source coverage 分級。Durable source 缺口仍是 blocking gaps；why-not / liquidity optional payload 缺口是 warnings / `dry_run_only`，完整 negative evidence 留待 V1.7。
