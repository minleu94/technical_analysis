...
2. 每日先看 `決策工作台` 的 status strip、今日待判讀、Evidence mode / data quality、Daily Checklist 與 warnings；需要細節時再下鑽 Daily Decision Desk、Market Watch / Smart Money、Evidence Review 或 Portfolio。
...
2026-07-07 Phase 2 Workbench MVP shell 已完成：Qt 主 UI 新增 `決策工作台`，資料只經 `WorkbenchSourceService` / `WorkbenchDashboardDTO`，第一版只讀呈現 status strip、今日待判讀、Evidence mode / data quality、Daily Checklist 與 warnings / degraded source。`WorkbenchSourceService` 可帶 optional replay JSON summary；UI 不直接讀 replay DB 或執行 replay，只揭露 simulated scheduler、source gap、payload gap、outcome maturity、benchmark coverage、missing industry benchmark 與 pending future-data 限制。此 shell 不重算 scoring、portfolio、backtest、lifecycle，不產生買賣建議，不解除 weekly history `0/3`、multi-day dry-run `1/3` 或 scheduler approval gate。
