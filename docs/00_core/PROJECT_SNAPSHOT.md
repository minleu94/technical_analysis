...
2026-07-06 V2.0 Phase 1 read-only Workbench prototype 已完成：新增 `WorkbenchDashboardDTO`、`WorkbenchReadOnlyComposer`、`workbench_replay_summary.py` 與 `scripts/inspect_v2_workbench_prototype.py`。CLI 只支援 `--sample`，可選讀 `_reference_fix` replay JSON summary，輸出 JSON / Markdown 的「今日待判讀」、Evidence mode 與 Daily Checklist prototype；它不讀正式 DB、不寫 evidence、不掛主 UI、不建立 scheduler、不下單、不套用 lifecycle action，也不改 `ScoringEngine`、Portfolio 或回測結果。Phase 2 MVP、Phase 0 weekly / multi-day gate 與 Phase 5 scheduler approval 仍未完成。
