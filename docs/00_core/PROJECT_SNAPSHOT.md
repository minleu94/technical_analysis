...
- legacy `scripts\maintain_ml_direct_v3_refresh_chain.py` 已新增 prospective schema guard：看到 prospective ledger／Rule／PIT wrapper（含 `.jsonl.gz` 與大型 `.json` bounded header）時，one-shot 直接 blocked，watch 模式只低 CPU 等待並記錄 `prospective_only_inputs_detected_legacy_watcher_blocked`；不會將 prospective path 交給歷史 Direct/OOC 重建器，也不會輸出 path／secret。guard regression=`26 passed`，py_compile／mypy=`0 issues`。
...
- PFS-01 現已提供 `scripts/publish_prospective_formal_clock.py --fixture-only`：它只把 owner 明確提供的未來交易日、官方日曆證據、seed 與 frozen identities 寫成 canonical planned clock manifest，採 create-only；不自動選 activation、不能回填過去日期、不設定正式 path、不建立 transitions／Rule／PIT，也不啟動 watcher。正式起算日仍未由 owner 提供，故 `formal_oos_allowed=false` 不變。
