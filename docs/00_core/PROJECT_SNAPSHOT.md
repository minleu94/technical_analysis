...
Post-V1 evidence-driven 增量已建立 Evidence Event Store v1、Forward Outcome Calculator v1 與 Evidence Importers / Capture Pipeline v1：可 append-only 保存 Recommendation、Watchlist Trigger、Portfolio Alert、Risk Prompt 等事件，並以 SQLite `daily_prices` 計算 5 / 10 / 20 / 60 交易日 close-to-close forward research outcome。Recommendation importer 可從 persisted `RecommendationResultDTO` 擷取；Watchlist / Portfolio Alert / Risk Prompt importer v1 讀取注入的 DTO / snapshot provider，CLI 無 provider 時會 fail-closed 回報 `source_unsupported`，不偽造事件。這仍只是 evidence collection layer；Forward Performance Dashboard、read model、樣本累積與投資有效性結論尚未完成。
...
2. **下一階段主線：Evidence-Driven baldr**：Evidence Event Store v1 / Forward Outcome Calculator v1 / Evidence Importers v1 已建立；下一步優先建立 Forward Performance read model 與 Dashboard，再接更完整的 Daily Decision Desk durable snapshot source、Live vs Research Gap Dashboard、Signal Decay Monitor 與 Decision Quality Review，追蹤 Watchlist Trigger、Recommendation、Why Not / Liquidity Gate、Portfolio Alert 與 Strategy Lifecycle 判斷是否真的改善決策。
...
Post-V1 evidence 增量狀態（2026-07-02）：`app_module/evidence_event_*`、`app_module/evidence_capture_service.py`、`app_module/evidence_event_importers.py`、`app_module/forward_performance_service.py`、`data_module/evidence_event_migration.py` 與最小 CLI 已建立，可在受控 SQLite schema 下保存 evidence events / outcomes。CLI 預設 dry-run，只有 `--confirm` 寫入；Recommendation 可讀 persisted result，Daily Decision Desk 類來源仍需 durable snapshot provider wiring。此狀態不代表任何事件類型已累積足夠樣本，也不代表 alpha 成立；後續必須累積 forward outcomes 並建立 dashboard / review 才能判讀效果。
