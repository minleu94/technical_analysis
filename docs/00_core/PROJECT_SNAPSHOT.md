...
- legacy `scripts\maintain_ml_direct_v3_refresh_chain.py` 已新增 prospective schema guard：看到 prospective ledger／Rule／PIT wrapper（含 `.jsonl.gz`）時，one-shot 直接 blocked，watch 模式只低 CPU 等待並記錄 `prospective_only_inputs_detected_legacy_watcher_blocked`；不會將 prospective path 交給歷史 Direct/OOC 重建器，也不會輸出 path／secret。guard regression=`25 passed`，py_compile／mypy=`0 issues`。
