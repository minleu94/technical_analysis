...
- `PFS-01`～`PFS-05` 已完成：clock／Portfolio producer／clock-bound HMAC Rule publisher／prospective PIT sector custody／inference-level calibration policy 均通過 focused QA；PFS-05 新增 8 個 tests，固定 identity vs isotonic、prior-fold fit lineage、ECE `500 bp`、逐 horizon conservative max 與 `rebalance_worthwhile` 雙類別 coverage。PFS-04／PFS-05 仍只產生受控 fixture 的 prospective sidecar／shadow audit，不設定正式 path、不寫 `D:`、不啟動 watcher；下一個可執行 slice 是 `PFS-06` capture-only readiness 與 heavy-rebuild guard。預設仍把 cutoff=`2026-08-13T08:30:00+08:00` 的現有 candidate 綁為未來 clock 的 frozen challenger；clock 啟動後不得用已消費的 Formal OOS 期間重訓或替換 model／calibration，重型 watcher 維持停止。
