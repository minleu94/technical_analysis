...
> **Gate 2–7 新判讀**：人工 blanket gate 已終止，改用 append-only machine evidence／policy decision。weekly sidecar 已由排程累積 4 個不同真實週期，`evidence:weekly-history-3` 以 `4/3` 自動完成。14 個 registry items 現為 8 個 complete、1 個 in progress（ML revalidation）、5 個 `insufficient_evidence`（forward、paper elapsed、exit outcome、shadow days、nonzero-alpha formal gates）。不足證據不再阻擋 Rule production，但也不會被偽簽成 accepted。完整狀態見 [Gate 2–7 External Validation Register](../06_qa/GATE_2_TO_7_EXTERNAL_VALIDATION_REGISTER.md)。
