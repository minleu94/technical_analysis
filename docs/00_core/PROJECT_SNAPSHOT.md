...
- Readiness UI 的 bounded projection 現在也保留各 lane 的固定進度摘要：P0 的機器證據／accepted／route probe、Evidence 的 weekly／dry-run／Formal credit、Paper 的 snapshot／benchmark／cost／fills、Formal／ML 的 `ready／total` input，以及 Runtime／performance／Update History 的 host probe／canary／排程／freshness；Evidence 另外顯示 sidecar 中 `pending_human_review` 的期數，讓 `weekly 0/3` 與「已有資料但待人工審核」分開閱讀。阻擋欄以中文意義加原始 machine token 顯示，不複製大型 details、不改變 gate。最新 host P0 + normalized MOPS candidate 重算 artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_program_readiness\program_readiness_after_latest_host_p0_20260828.json`（status=`action_required`；SHA-256=`19049F03B64D3165281FC7506879CBAE24FAC05BB382CB42B575EC15E15C8A2D`），P0=`13/13` machine evidence、`accepted=0`、route=`14/27` attempted，Evidence=`weekly 0/3`／`pending 8 期`／dry-run=`3/3`，Paper=`21` snapshot／benchmark、cost=`0`，Formal=`0/3`；Runtime／Update History ready，正式寫入與 scheduler／broker／Formal 仍關閉。
