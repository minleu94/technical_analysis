...
- `PFS-01`～`PFS-04` 已完成：clock／Portfolio producer／clock-bound HMAC Rule publisher／prospective PIT sector custody 均通過 focused QA；PFS-04 新增 8 個 tests，支援 `.json`／`.jsonl`／`.jsonl.gz`，並嚴格驗證 accepted source、license、publication、available/effective time、universe 與 canonical hash。PFS-04 仍只產生受控 fixture 的 prospective sidecar，不設定正式 path、不寫 `D:`、不啟動 watcher；下一個可執行 slice 是 `PFS-05` inference-level calibration policy。預設仍把 cutoff=`2026-08-13T08:30:00+08:00` 的現有 candidate 綁為未來 clock 的 frozen challenger；clock 啟動後不得用已消費的 Formal OOS 期間重訓或替換 model／calibration，重型 watcher 維持停止。
