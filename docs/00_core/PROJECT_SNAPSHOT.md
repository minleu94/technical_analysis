...
- 2026-08-28 最新全量 pytest 以 `-o addopts=` 通過 `3747 passed / 1 skipped / 26 warnings`（`533.67s`）；已包含 Registry clone transaction、storage retention inventory、monthly-revenue candidate projection 與 weekly review CLI UTF-8 guard。warnings 主要是 joblib physical-core fallback、研究回測同日成交假設與 pytest cache 權限提示；工程回歸通過不會解除 production scheduler、正式 ACL、owner decision 或資料 gate。
