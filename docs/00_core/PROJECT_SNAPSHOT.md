...
3. Month 5 Fundamental Layer preflight 已開始：已完成 `docs/03_data/FUNDAMENTAL_SOURCE_INVENTORY.md`，確認既有 `financial_data/` 只可作 raw candidate source，正式 SQLite 已新增三張 fundamental schema 空表；並新增 raw 月營收唯讀正規化契約、受治理公告日 / available_date mapping 契約、公告日 / available_date 初版政策、月營收公告日 mapping CSV loader 與 `TWStockConfig.monthly_revenue_availability_file` 預設路徑、正式 mapping dry-run 驗證入口 `data_module/fundamental_availability_entrypoint.py` 與 CLI `scripts/validate_monthly_revenue_availability.py`、TWSE OpenAPI 月營收 availability 候選產生器 `scripts/build_monthly_revenue_availability.py`、月營收 normalized backfill workflow `scripts/backfill_monthly_revenue_fundamentals.py`、Fundamental SQLite read provider `data_module/fundamental_sqlite_provider.py`、候選 SQLite schema dry-run 模組、暫時 connection / 正式 DB working copy dry-run report API、Fundamental SQLite 受控 migration service/CLI、Revenue Factor Pack v1 adapters、valuation data layer v1、Abnormal Fundamental diagnostics，缺 explicit `available_date` 不產生 normalized record / factor record，未來 `available_date` 由既有 `FactorGate` 跳過。Revenue Factor Pack 目前只從已正規化月營收 records 產生 YoY、MoM、3M trend 與 new high factor records；缺 baseline 只輸出 diagnostics，不給中性分數，也不接 `ScoringEngine`。valuation data layer 只建立受治理 `ValuationObservation` 與同產業整數基點分位，缺分位交由既有 presentation policy 回 diagnostics。SQLite read provider 只讀 `available_date <= decision_date` 的月營收與估值 records，不讀 raw CSV、不寫 DB。Abnormal Fundamental diagnostics 只輸出 Research metadata 與 Daily Decision Desk risk prompts，不改寫財報、不自動扣分，也不產生買賣建議。驗證入口只讀取使用者提供的 mapping 檔或 `TWStockConfig.monthly_revenue_availability_file`，輸出 diagnostics / Markdown 摘要，不建立、不改寫正式 mapping 檔、raw CSV 或 SQLite。2026-06-16 已在正式 `twstock.db` 複本上驗證候選 schema 只新增三張 fundamental 表、不修改既有五張核心表；同日依使用者確認正式套用 schema migration，備份檔為 `D:/Min/Python/Project/FA_Data/meta_data/backup/twstock_fundamental_schema_20260616_022301.db`，正式 DB 目前只有三張空 fundamental 表，尚未回填資料。估值呈現政策 v1 已建立 presentation-only 邊界，僅輸出相對估值區間、policy_version、quality 與 diagnostics，不輸出目標價、合理價、上漲空間或交易建議。TWSE OpenAPI 真實 dry-run 目前只回最新月資料，與本機歷史 raw 月營收期間暫無交集，因此尚未寫入正式 mapping；backfill 正式路徑 dry-run 也因 mapping 缺檔 fail-closed，尚未寫入 records。下一步是取得可追溯的歷史公告日 mapping，再由人工確認回填 normalized fundamental data。
