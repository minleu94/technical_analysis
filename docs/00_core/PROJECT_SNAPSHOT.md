...
> **Terra Development Dataset V0（2026-07-13）**：owner 已決議 2025 永久為 `seen_oos`／development-only，新的獨立 dataset generator 僅唯讀 core price／technical／market／industry tables，強制 T-1、252 日 observed universe、2025 fit／2026 evaluation-only、append-only external output 與 `research_only_degraded` corporate coverage。bounded real-data smoke 已確認正式 DB SHA-256／mtime 不變；`formal_oos_allowed=false`、production alpha=0，且不改 Rule-only、Score、Recommendation、Portfolio、Exit 或 scheduler。這是 development artifact 工程能力，不是 formal OOS、model promotion 或投資有效性證據。
