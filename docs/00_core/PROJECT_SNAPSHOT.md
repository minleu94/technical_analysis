...
- 新增唯讀 `scripts/plan_prospective_formal_clock.py` 與 `data_module/prospective_clock_planner.py`：只消費明確的 `official-trading-calendar-bundle.v1`，驗證台北當日、至少一個 preparation-day window、TWSE／TPEX 共同交易、owner decision timestamp 與既有 clock date，產出 `candidate_ready`／`blocked` proposal。它不下載日曆、不建立 clock、不自動選 same-day override、不改受控 path；proposal hash 只能交給 owner 審閱，不能當 Formal input。
...
- 測試 inventory 機器重算（2026-08-28）：新增 P0 audit CLI CP1252 console guard、scheduled-task registration inspector（含 CP1252 `--help` guard）、technical process-pool／worker recovery acceptance、P0 audit → candidate intake projection、freshness ACL-safe status/log route、machine evidence handoff projection／intake envelope guard、explicit freshness readiness projection、freshness probe ACL fail-soft regression、PowerShell freshness wrapper canonical delegation、owner packet route evidence、Formal candidate inventory 與月營收 snapshot 選擇／候選狀態測試並完成 inventory 登錄後為 `648/648`、`3674 collected`，inventory audit 的 machine-checkable blockers=`0`；本節較早的 `3614`／`3612 collected` 與 `639/639`／`638/638`／`3604`、`637/637`／`3596`、`3608 collected` 讀數屬前序中間基準。
- 2026-08-28 最新全量 pytest 以 `-o addopts=` 通過 `3683 passed / 1 skipped / 26 warnings`（`540.83s`）；已包含 broker real HTTP canary、availability merge 與月營收候選狀態測試。沒有新增 failure；warnings 主要是 joblib physical-core fallback、研究回測同日成交假設與 pytest cache 權限提示。此前 `3674 passed`／`3668 passed` 為前序基準。
