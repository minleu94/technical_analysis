...
2. Month 3 Portfolio Replay 可信度：推薦組合回放已能產生 factor metadata，並開始輸出 `portfolio_credibility` manifest 揭露現金帳、再平衡、未成交、Liquidity / Gap 與同日收盤成交假設限制；完整現金帳、權重、再平衡、未成交與 Liquidity / Gap 模型仍需補強，避免後續策略生命週期建立在不完整組合績效上。
