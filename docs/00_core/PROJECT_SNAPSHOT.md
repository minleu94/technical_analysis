...
2026-07-06 V2.0 Phase 1 read-only Workbench prototype 與 Phase 1.5 / Phase 2 前置 formal read-only source adapter 已完成：新增 `WorkbenchDashboardDTO`、`WorkbenchReadOnlyComposer`、`workbench_replay_summary.py`、`WorkbenchSourceService` 與 `scripts/inspect_v2_workbench_prototype.py`。CLI 保留 `--sample`，也可用受控 `--db-path` / `--decision-date` 讀取 existing read-only sources：Pre-V2 readiness、Daily Decision durable snapshot、AgentEvidenceAccess summary 與可選 `_reference_fix` replay JSON summary。missing DB / missing table / degraded source 只回 diagnostics 與 warnings，不建立 schema、不寫 evidence DB、不掛主 UI、不建立 scheduler、不下單、不套用 lifecycle action，也不改 `ScoringEngine`、Portfolio 或回測結果。Phase 2 MVP、Phase 0 weekly history `0/3`、multi-day dry-run `1/3` 與 Phase 5 scheduler approval 仍未完成。
