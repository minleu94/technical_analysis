...
3. Month 5 Fundamental Layer preflight 已開始：已完成 `docs/03_data/FUNDAMENTAL_SOURCE_INVENTORY.md`，確認既有 `financial_data/` 只可作 raw candidate source，正式 SQLite 已新增三張 fundamental schema；並新增 raw 月營收唯讀正規化契約、受治理公告日 / available_date mapping 契約、公告日 / available_date 初版政策、月營收公告日 mapping CSV loader 與 `TWStockConfig.monthly_revenue_availability_file` 預設路徑、正式 mapping dry-run 驗證入口 `data_module/fundamental_availability_entrypoint.py` 與 CLI `scripts/validate_monthly_revenue_availability.py`、TWSE OpenAPI 月營收 availability 候選產生器 `scripts/build_monthly_revenue_availability.py`、月營收 normalized backfill workflow `scripts/backfill_monthly_revenue_fundamentals.py`、valuation metrics backfill workflow `scripts/backfill_valuation_metrics.py`、Fundamental SQLite read provider `data_module/fundamental_sqlite_provider.py`、Fundamental factor service `app_module/fundamental_factor_service.py`、候選 SQLite schema dry-run 模組、暫時 connection / 正式 DB working copy dry-run report API、Fundamental SQLite 受控 migration service/CLI、Revenue Factor Pack v1 adapters、valuation data layer v1、Abnormal Fundamental diagnostics，缺 explicit `available_date` 不產生 normalized record / factor record，未來 `available_date` 由既有 `FactorGate` 跳過。Revenue Factor Pack 目前只從已正規化月營收 records 產生 YoY、MoM、3M trend 與 new high factor records；缺 baseline 只輸出 diagnostics，不給中性分數，也不接 `ScoringEngine`。valuation data layer 只建立受治理 `ValuationObservation` 與同產業整數基點分位，缺分位交由既有 presentation policy 回 diagnostics。valuation metrics backfill 可由 `daily_prices.本益比` 與官方更新後的 `companies.csv` 產業 mapping 產生 P/E valuation records；2026-06-16 正式路徑使用最新 P/E 日 `2026-06-15`，來源 1,090 筆，831 筆可正規化，259 筆 diagnostics，已依使用者確認正式 apply，`fundamental_valuation_metrics` count 為 831。TPEX daily price backfill 已新增受控 CLI，2026-06-16 對正式 `daily_prices` 補入 877 筆上櫃四碼普通股日價，`3207` 已有 `20260616` 日價且 0 duplicate primary keys；此流程只寫 `daily_prices`，不寫 company registry 或 fundamental tables。SQLite read provider 只讀 `available_date <= decision_date` 的月營收與估值 records，不讀 raw CSV、不寫 DB；Fundamental factor service 串接 provider、revenue/valuation adapters 與 FactorGate，但不接 `ScoringEngine`。Abnormal Fundamental diagnostics 只輸出 Research metadata 與 Daily Decision Desk risk prompts，不改寫財報、不自動扣分，也不產生買賣建議。驗證入口只讀取使用者提供的 mapping 檔或 `TWStockConfig.monthly_revenue_availability_file`，輸出 diagnostics / Markdown 摘要，不建立、不改寫正式 mapping 檔、raw CSV 或 SQLite。2026-06-16 已在正式 `twstock.db` 複本上驗證候選 schema 只新增三張 fundamental 表、不修改既有五張核心表；同日依使用者確認正式套用 schema migration，備份檔為 `D:/Min/Python/Project/FA_Data/meta_data/backup/twstock_fundamental_schema_20260616_022301.db`，正式 DB 目前已有三張 fundamental 表；`fundamental_monthly_revenues` 與 `fundamental_statement_items` 仍未回填，`fundamental_valuation_metrics` 已正式寫入 831 筆 P/E records。估值呈現政策 v1 已建立 presentation-only 邊界，僅輸出相對估值區間、policy_version、quality 與 diagnostics，不輸出目標價、合理價、上漲空間或交易建議。TWSE OpenAPI 真實 dry-run 目前只回最新月資料，與本機歷史 raw 月營收期間暫無交集，因此尚未寫入正式 mapping；月營收 backfill 正式路徑 dry-run 也因 mapping 缺檔 fail-closed，尚未寫入 records；service 正式 DB snapshot 目前可讀 valuation records，月營收仍回 missing diagnostics。下一步是取得可追溯的歷史公告日 mapping，並在使用者確認後才可正式 apply 月營收回填；TPEX official daily close quotes 已納入日常每日股價更新管線，會輸出 `DATA_ROOT/daily_price_tpex/YYYYMMDD.csv` 並 upsert 到 `daily_prices`，TPEX endpoint timeout 會成為 warning 而不阻斷其他資料同步；歷史 TPEX 缺漏仍需另行 dry-run plan 與人工確認，目前 `3207` 只有已正式補入的 `20260616` 日價。SQLite Inspector 已修正日期預設 / 欄寬與重複顯示欄名造成的 `PandasTableModel` Series ambiguity；`broker_flows` 同步已準備受控 schema upgrade，首次同步舊 DB 時會先備份，再把唯一鍵升級為 `(分點名稱, 證券代號, 日期, trade_type)`，允許同日同分點同股票的買超 / 賣超 records 共存。

## 高風險區（改動需謹慎）

Month 5 月營收 availability mapping 最新補充（2026-06-16）：已新增 `data_module/monthly_revenue_availability_history.py` 與 `scripts/build_monthly_revenue_availability_history.py`，支援 `--start-period 2020-01`、`--end-period 2026-05`、`--markets twse,tpex`、`--stock-code`、`--mops-html-dir` 與候選 CSV output。TWSE `/opendata/t187ap05_L` 與 TPEX `/openapi/v1/mopsfin_t187ap05_O` 最新月來源都有 `出表日期`，樣本 `2330`、`9935`、`3207` 已驗證；但 OpenAPI 未提供歷史 period query，MOPS historical 自動化查詢目前被安全頁或 404 擋下。`--mops-html-dir` 可讀人工保存且含 `出表日期` 的 MOPS 官方 HTML，source 為 `mops.monthly_revenue_announcement`，缺 `出表日期` 或公司列時 fail-closed diagnostics。本機 raw 月營收期間為 `2014-04..2024-04`，與最新月 `2026-05` 來源無交集；`2020-01..2026-05` dry-run 結果為 `requested_periods=77`、`fetched_periods=1`、`matched_raw_monthly_revenue_rows=0`、`missing_availability_rows=76902`、`duplicate_mapping_rows=0`。正式 mapping 寫入與 `fundamental_monthly_revenues` apply 仍需人工確認。
