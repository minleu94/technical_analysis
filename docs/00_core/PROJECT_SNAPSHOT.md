...
2026-07-07 V2.0 Phase 1 read-only Workbench prototype、Phase 1.5 / Phase 2 前置 formal read-only source adapter 與 Phase 2 Qt MVP shell 已完成第一版：新增 `WorkbenchDashboardDTO`、`WorkbenchReadOnlyComposer`、`workbench_replay_summary.py`、`WorkbenchSourceService`、`scripts/inspect_v2_workbench_prototype.py`、`ui_qt/views/workbench_view.py` 與 `ui_qt/models/workbench_table_models.py`。CLI 保留 `--sample`，也可用受控 `--db-path` / `--decision-date` 讀取 existing read-only sources；Qt 主 UI 新增 `決策工作台` 分頁，第一版呈現 status strip、今日待判讀、Evidence mode / data quality、Daily Checklist 與 warnings / degraded source。UI 只透過 `WorkbenchSourceService` / `WorkbenchDashboardDTO`，不直接讀 SQLite、不讀 replay DB、不寫 DB、不建立 scheduler、不下單、不套用 lifecycle action，也不改 `ScoringEngine`、Portfolio 或回測結果。若 DTO 帶 `_reference_fix` replay JSON summary，data quality 會揭露 `simulated_scheduler`、source gap、payload gap、outcome maturity、benchmark coverage、missing industry benchmark 與 pending future-data。完整 operating loop、舊 Tab drill-down、Phase 0 weekly history `0/3`、multi-day dry-run `1/3` 與 Phase 5 scheduler approval 仍未完成。
...
2. 每日先看 `決策工作台` 的中文優先 status strip、今日待判讀、Evidence mode / data quality、Daily Checklist 與 warnings；需要細節時用 Workbench 的 read-only drill-down 切到 Daily Decision Desk、Evidence Review 或 Portfolio，或再進 Market Watch / Smart Money。
...
2026-07-07 Phase 2 Workbench MVP shell 已完成並推進第一段 operating loop：Qt 主 UI 新增 `決策工作台`，資料只經 `WorkbenchSourceService` / `WorkbenchDashboardDTO`，中文優先呈現 status strip、今日待判讀、Evidence mode / data quality、Daily Checklist、warnings / degraded source，並提供舊 Daily Decision / Evidence Review / Portfolio 的 read-only drill-down。若預設 `_reference_fix` replay JSON summary 存在，UI 會自動傳入 `WorkbenchSourceService`，但不直接讀 replay DB 或執行 replay；目前 summary 顯示 118 天、118,056 events、472,224 outcomes，成熟 outcomes 380,736，pending future-data 91,488，market benchmark coverage 380,736/380,736，industry benchmark coverage 2,245/380,736，`source_missing_screening_matrix` 為 118/118 days。這代表方向性 evidence pipeline 正在變好，但仍不構成 production readiness 或投資有效性。此 shell 不重算 scoring、portfolio、backtest、lifecycle，不產生買賣建議，不解除 weekly history `0/3`、multi-day dry-run `1/3` 或 scheduler approval gate。
