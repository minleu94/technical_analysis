...
2. **下一階段主線：Evidence-Driven baldr**：Evidence Event Store v1 / Forward Outcome Calculator v1 / Evidence Importers v1 / E2E smoke / Forward Performance Read Model v1 / Daily Decision Desk durable snapshot source / source coverage inspection v1 / Forward Performance Dashboard read-only UI v1 / Evidence Pipeline Runner dry-run v1 / working-copy DB smoke v1 / scheduler approval checklist v1 / Live vs Research Gap linkage v1 / Signal Decay Monitor v1 已建立；下一步優先接 Decision Quality Review、Signal Decay read-only UI 與 Live vs Research Gap read-only UI，追蹤 Watchlist Trigger、Recommendation、Why Not / Liquidity Gate、Portfolio Alert 與 Strategy Lifecycle 判斷是否真的改善決策。
