...
Post-V1 evidence 增量狀態（2026-07-07）：`app_module/evidence_event_*`、`app_module/evidence_capture_service.py`、`app_module/evidence_event_importers.py`、`app_module/forward_performance_service.py`、`app_module/forward_performance_read_model.py`、`app_module/forward_performance_dashboard_*`、`app_module/evidence_pipeline_runner.py`、`app_module/evidence_scheduler_readiness.py`、`app_module/decision_desk_snapshot_*`、`data_module/evidence_event_migration.py`、`ui_qt/views/forward_performance_view.py` 與 CLI 已建立，可在受控 SQLite schema 下保存 evidence events / outcomes / Daily Decision Desk durable snapshot，於 Research Lab `Forward Evidence` 分頁唯讀檢查已成熟 outcome 的樣本、pending / missing、benchmark / industry 缺口、quality 與 warnings，並用 `scripts/run_evidence_pipeline.py` 手動模擬每日 evidence pipeline。CLI 預設 dry-run，只有 `--confirm --db-path` 寫入 working-copy DB；`scripts/smoke_evidence_pipeline_working_copy.py` 可在 working-copy DB 重複 confirm smoke 並檢查 source DB read-only 與 idempotency；`scripts/evaluate_evidence_scheduler_readiness.py` 會回傳 readiness、blocking gaps、manual checks 與 `production_scheduler_allowed=false`。Recommendation 可讀 persisted result，Daily Decision Desk 類來源可讀 durable snapshot；Why Not / Liquidity exclusion payload 為 optional / partial，舊結果缺 payload 時只 diagnostic。此狀態不代表任何事件類型已累積足夠樣本，也不代表 alpha 成立；scheduler readiness 最高只到 `ready_for_manual_confirm`，production scheduler 仍未啟用，Live vs Research Gap linkage 與 Signal Decay Monitor 尚未完成。
