...
3. Month 6 尚未開始實作；下一步是開 Strategy Lifecycle 與 Portfolio Feedback 的 scope / contract：先定義 Promote / demote / retire 規則、StrategyDriftDetector 輸入輸出、Portfolio post-trade attribution 與 Live vs research gap report 的證據欄位，再決定 UI 或服務實作。此階段不得為了生命週期功能直接修改 scoring、回測績效或 fundamental factor 權重。

## 高風險區（改動需謹慎）

Month 5 closeout 狀態（2026-06-17）：Fundamental Layer v1 已達成「保守接入、可追溯、可診斷、不污染 ScoringEngine」的驗收目標，可關閉 Month 5 並進入 Month 6 規劃。後續若要把任何 fundamental factor 用於策略升降級或推薦權重，必須另走 Month 6 strategy lifecycle gate，並重新做 Look-ahead bias 與金融數值邊界自查。
