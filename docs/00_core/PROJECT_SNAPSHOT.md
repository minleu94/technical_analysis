...
3. Month 5 Fundamental Layer preflight 已開始：已完成 `docs/03_data/FUNDAMENTAL_SOURCE_INVENTORY.md`，確認既有 `financial_data/` 只可作 raw candidate source，正式 SQLite 尚無 fundamental tables；並新增 raw 月營收唯讀正規化契約、受治理公告日 / available_date mapping 契約、公告日 / available_date 初版政策、候選 SQLite schema dry-run 模組、暫時 connection / 正式 DB working copy dry-run report API、最小 `fundamental.revenue_yoy` adapter contract 測試，缺 explicit `available_date` 不產生 normalized record / factor record，未來 `available_date` 由既有 `FactorGate` 跳過。2026-06-16 已在正式 `twstock.db` 複本上驗證候選 schema 只新增三張 fundamental 表、不修改既有五張核心表；下一步是正式公告日資料源、估值政策與 AbnormalFundamentalFlag 規則。
