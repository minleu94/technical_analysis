...
- 2026-08-28 全量 pytest 以 `-o addopts=` 通過 `3664 passed / 1 skipped / 26 warnings`（`560.58s`）；freshness probe ACL fail-soft、freshness readiness projection、Evidence Workbench pending-period 與 formal cutoff 診斷納入後的完整回歸沒有新增 failure。PowerShell wrapper 的 canonical delegation 以 targeted suite 驗證；warnings 仍是既有 joblib physical-core fallback、研究回測同日成交假設與 pytest cache 權限提示。`3663 passed` 為前序中間結果。
