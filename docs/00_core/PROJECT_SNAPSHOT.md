...
2. **下一階段主線：Evidence-Driven baldr + V1.1 / V1.2 / V1.3 / V1.4 v1 已完成**：Evidence Event Store v1 / Forward Outcome Calculator v1 / Evidence Importers v1 / E2E smoke / Forward Performance Read Model v1 / Daily Decision Desk durable snapshot source / source coverage inspection v1 / Forward Performance Dashboard read-only UI v1 / Evidence Pipeline Runner dry-run v1 / working-copy DB smoke v1 / scheduler approval checklist v1 / Live vs Research Gap linkage v1 / Signal Decay Monitor v1 / Decision Quality Review v1 / Evidence Review Dashboards read-only UI pack v1 / Evidence Review UI smoke checklist / multi-day dry-run record scaffold / safe scheduled CMD wrappers / V1.3 weekly evidence operations package / V1.4 weekly review history 已建立；每日 05:30 Codex read-only 摘要、Evidence Review UI smoke 與 multi-day dry-run evidence 持續背景累積。V1.1 已補推薦 Profile 可見性、推薦回放語意與 Profile replay comparison service；V1.2 已補 replay 訓練 / 驗證分離、rolling risk、microstructure preflight 與 relative attribution；V1.3 已補 manual approval summary、weekly review CLI、signal decay manual lifecycle candidate 與 action item planning；V1.4 已補 weekly review history repository、CLI save/list 與 Research Lab `覆盤歷史` 唯讀子頁。2026-07-03 已修正 batch / CLI Daily Decision Desk snapshot wiring 與 numpy scalar JSON 序列化，working-copy confirm smoke 可寫入 `risk_prompt` evidence 且 idempotency passed；同日受控 tmp run 已保存 working-copy Recommendation result 並驗證 `recommendation,risk-prompt` requested sources 可 idempotent confirm。剩餘 source gaps 為 why-not / liquidity exclusion payload、watchlist 與 portfolio 實際資料。完整 Daily Decision / Market Watch 合併仍留待 V2.0 評估；下一步是用 weekly review + history 實際累積數週覆盤證據。Production scheduler implementation 仍需 source gaps 修正、人工 approval 與明確設計後才可進行。
...
Post-V1 safe scheduled wrapper 狀態（2026-07-12）：`scripts/scheduled/` 已新增 CMD wrapper + Windows `schtasks.exe` 註冊路徑，以避開 PowerShell `.ps1` 被 local execution policy 擋住的問題；不得使用 `Set-ExecutionPolicy`。Windows Task Scheduler 已建立 `baldr-data-update-quick-daily`（每日本機時間 04:20，非 UI 快速資料更新）、`baldr-data-freshness-check-daily`（每日本機時間 05:00，只讀 freshness）與 `baldr-evidence-pipeline-dry-run-daily`（每日本機時間 05:15，只跑 dry-run report）。`baldr-evidence-working-copy-smoke-manual` 只保留 manual-only script，不建立每日自動 task。Codex app 已另建 `baldr scheduled evidence morning report` automation（約 05:30），只讀 Task Scheduler 狀態、latest status、最新 report 與必要 log，產生繁體中文摘要；它不重新執行 data update / freshness / evidence pipeline、不建立或修改 Windows task。這些 wrapper 會寫 `<OUTPUT_ROOT>/scheduled/...` 的 status / log / report；data update quick task 會寫市場資料 CSV / SQLite，但不寫 production evidence DB、不跑 UI、不讀 UI state、不做 portfolio / lifecycle action。Production write-mode evidence schedule 仍未啟用，後續仍需 multi-day dry-run record、人工 UI smoke 與 explicit approval。
