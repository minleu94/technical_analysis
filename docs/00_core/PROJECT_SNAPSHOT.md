...
- `PFS-01`、`PFS-02` 與 `PFS-03` 已完成：clock／Portfolio producer／clock-bound HMAC Rule publisher 均通過 focused QA；PFS-03 新增 6 個 tests，並通過 PFS-01／Rule service 30 passed、PFS-02／formal ledger／turnover 11 passed、`py_compile` 與 mypy `0 issues`。PFS-03 只產生受控 fixture 的 prospective wrapper，不設定正式 path、不寫 `D:`、不啟動 watcher；下一個可執行 slice 是 `PFS-04` prospective PIT sector source／manifest custody。預設仍把 cutoff=`2026-08-13T08:30:00+08:00` 的現有 candidate 綁為未來 clock 的 frozen challenger；clock 啟動後不得用已消費的 Formal OOS 期間重訓或替換 model／calibration，重型 watcher 維持停止。
