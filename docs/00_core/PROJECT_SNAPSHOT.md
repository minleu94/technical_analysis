...
**目前產品 Gate**：Gate 0 Safe Refactor Closeout 尚未正式完成。`SAFE_REFACTORING_MASTER_REPORT.md` 仍是 Active engineering companion；完成行為等價、golden baseline、Recommendation / Backtest / Portfolio / Scheduler / DTO diff 與 rollback point 後，應停止無止盡重構，轉入 Gate 1 Daily Usable Advice。新 Product Roadmap 與 Target Architecture 已建立，但本次只改文件，不代表 Advice、Portfolio Coach、Exit Engine、P0 ingestion 或 ML production 已實作。
...
**Evidence 時間型 Gate 狀態校正（2026-07-08）**：multi-day dry-run record 已為 `3/3 ready`；weekly evidence operations history 仍為 `0/3 waiting_for_time`，manual review / action-item rhythm 與 explicit production approval 仍未完成，`production_scheduler_allowed=false`。本節後續日期較早的 `1/3` 記載只保留為歷史進度，不代表目前狀態。
...
2026-07-07 V2.0 Phase 1 read-only Workbench prototype、Phase 1.5 / Phase 2 前置 formal read-only source adapter 與 Phase 2 Qt read-only operating loop 已完成：新增 `WorkbenchDashboardDTO`、`WorkbenchReadOnlyComposer`、`workbench_replay_summary.py`、`WorkbenchSourceService`、`scripts/inspect_v2_workbench_prototype.py`、`ui_qt/views/workbench_view.py` 與 `ui_qt/models/workbench_table_models.py`。CLI 保留 `--sample`，也可用受控 `--db-path` / `--decision-date` 讀取 existing read-only sources；Qt 主 UI 新增 `決策工作台` 分頁，並已從中文 read-only shell + 舊頁 drill-down 推進到 background evidence feed / read-only Action Items / Operating Loop。畫面呈現 status strip、今日待判讀、背景證據流、只讀 Action Items、操作節奏、Evidence mode / data quality、Daily Checklist 與 warnings / degraded source。背景證據流只彙整既有 DTO/service payload（Daily Decision snapshot、Evidence Review readiness、Portfolio alerts、可選 replay summary diagnostics）；Action Items 只顯示人工待處理事項，每列帶 source trace、degraded reason 與 drill-down target，`write_intent=false`，不建立 repository、不寫 DB、不套用 lifecycle。2026-07-07 follow-up 已把 Action Items 推進成更像人工處理佇列的掃描體驗：每列由 DTO payload 提供 severity、queue group、source label 與 sort rank，composer 依嚴重度 / 佇列 / 來源排序，Qt model 顯示佇列與來源欄位，空狀態與降級狀態文案都明確保留只讀、非建議與不補值語氣；drill-down target 維持 Daily Decision、Evidence Review、Portfolio 舊頁導向。Phase 2C/2D 再新增 `WorkbenchOperatingLoopStep`，把今日待判讀、背景證據流、Action Items、daily checklist、weekly history、multi-day dry-run、manual review note 與 scheduler gate 串成只讀操作節奏：每天打開時可辨識今天要看什麼、哪些要人工處理、哪些只是等待真實時間累積；所有 step 都是 `write_intent=false`，不標記完成、不寫 DB、不啟用 scheduler。UI 只透過 `WorkbenchSourceService` / `WorkbenchDashboardDTO`，不直接讀 SQLite、不讀 replay DB、不寫 DB、不建立 scheduler、不下單、不套用 lifecycle action，也不改 `ScoringEngine`、Portfolio 或回測結果。若 DTO 帶 `_reference_fix` replay JSON summary，data quality 會揭露 `simulated_scheduler`、source gap、payload gap、outcome maturity、benchmark coverage、missing industry benchmark 與 pending future-data。Phase 2 UI / read-only operating loop 可收口；Phase 0 weekly history `0/3`、multi-day dry-run `1/3`、V2.2 真實 evidence operating loop 與 Phase 5 scheduler approval 仍未完成。
...
1. **Gate 0 Safe Refactor Closeout**：完成 protected contract diff、golden baseline 與 rollback point；重構 closeout 前不把新產品能力混入行為等價計畫。
2. **Gate 1 Advice Contract 設計準備**：以 `PRODUCT_ROADMAP_POST_REFACTOR.md` 定義 Guided / Professional 共核、bounded actions、Why / Why Not / Risk / Evidence、`NO_NEW_POSITION` 與 Portfolio target/current/gap；尚未進入程式實作。
3. **維持真實 Evidence 與資料邊界**：weekly evidence operations history 仍需真實時間累積；multi-day dry-run、historical replay、candidate source readiness 與 V3 engineering candidate 不折抵正式 effectiveness、P0 source acceptance、production scheduler 或 ML Gate。

## 2026-07-08 本週優先事項（歷史記錄）
