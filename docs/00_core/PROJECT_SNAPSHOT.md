...
- `PFS-01`～`PFS-09` 已完成：clock／Portfolio producer／clock-bound HMAC Rule publisher／prospective PIT sector custody／inference-level calibration policy／capture-only readiness／frozen activation contract／shadow maturity gate／frozen-candidate OOS evidence contract 均通過 focused QA；PFS-09 新增 7 個 tests，固定雙 replay identity、maturity hash、calibration／PSI custody 與 no-fit／no-retrain／no-post-outcome-selection。PFS-04～PFS-09 仍只產生受控 fixture 的 prospective sidecar／shadow audit／readiness／activation contract／maturity report／OOS evidence package，不設定正式 path、不寫 `D:`、不啟動 watcher；下一個可執行 slice 是 `PFS-10` promotion review package。預設仍把 cutoff=`2026-08-13T08:30:00+08:00` 的現有 candidate 綁為未來 clock 的 frozen challenger；clock 啟動後不得用已消費的 Formal OOS 期間重訓或替換 model／calibration，重型 watcher 維持停止。
