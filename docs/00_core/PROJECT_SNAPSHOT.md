...
Month 5 基本面 factor layer 收尾（2026-06-17）：季度財報已接入 `FundamentalSQLiteProvider` 與 `FundamentalFactorService`，新增 EPS、gross margin、operating margin、ROE、non-operating income ratio factor adapters；各指標只輸出 factor records / diagnostics，不輸出 score、不接 `ScoringEngine`。正式 DB `decision_date=2026-06-30` inspection 結果為 factor records 14,840、diagnostics 812；statement factors 分別為 EPS 1,411、gross margin 1,368、operating margin 1,374、ROE 1,277、non-operating income ratio 1,261。PB / PS 已新增 source policy inspection，Month 5 僅 P/E ready，P/B 與 P/S 分別回 pending diagnostics，等待下一階段決定 book value / share count、market cap / TTM sales 政策。
