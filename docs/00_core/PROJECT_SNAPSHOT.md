...
- Runtime 先前在受限 sandbox 看到的既有檔案 `Permission denied` 已由一般 host context 重跑排除；對正式 `config.log` 與 Research Registry 的零位元 existing-file write-handle probe 通過，overall=`ready`、`write_probe=os.access_plus_existing_handle`。另已補上受控 staging probe：以正式 `ResearchRunRepository` schema 在隔離暫存 DB 執行 insert／讀回／rollback／再讀取，輸出 `registry_transaction_succeeded`；它仍不碰正式 Registry，也不代表正式 ACL 已核准。觀測 JSON 保留於 OS TEMP；QA Equal Weight preview 位於 `output/qa/readiness_refresh_20260828/`，正式 Paper Equal Weight ledger 已另存於 `output/paper_portfolio/paper_equal_weight_benchmark.sqlite`。
