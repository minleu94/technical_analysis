...
- **目標閉環 4：每日決策工作台（Daily Decision Desk）** ✅ 已接上首頁 v1
  ...
  - 目前主 UI 已接上「每日決策」工作區，各 section 已具備 snapshot 顯示框架；Market Regime、Market Breadth v1、Sector Rotation v1、Watchlist Trigger v1 與 Portfolio Alert 已接主 UI。Market Breadth v1 由 SQLite `daily_prices` 推導多方 / 空方 / 持平、成交量擴散與新高新低等 metadata；Sector Rotation v1 由 SQLite `industry_indices` 推導領先 / 落後產業、5 / 20 日變化與輪動強度；Watchlist Trigger v1 由 `WatchlistService` 與 SQLite `technical_indicators` 共同推導，可計算出個股強度 score_bp (RSI * 100) 與風險警示 risk_alert (偏離 RSI > 80 / < 20 或跌破 lowerband)。若指定日無資料會採用最近可用交易日，並在 `warnings` 以 fallback 揭露（quality 降級為 `DEGRADED`）。Portfolio Alert v1 已接 `PortfolioService`、`PortfolioConditionMonitor` 與 `PortfolioChipService`，可把條件監控與籌碼風險彙總成每日持倉警示；若籌碼資料缺失、估算或不可用，會透過 `quality / warnings` 降級揭露，不補值。
...
3. Month 4 Daily Decision Desk 已接上主 UI：`DecisionDeskSnapshot` 已進入首頁，Market Regime、Market Breadth v1、Sector Rotation v1、Watchlist Trigger v1、Portfolio Alert v1 已接真實 service，缺口以 `quality / warnings` 顯示，缺值不補值。
