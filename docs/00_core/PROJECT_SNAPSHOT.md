...
- `PFS-01`～`PFS-06` 已完成：clock／Portfolio producer／clock-bound HMAC Rule publisher／prospective PIT sector custody／inference-level calibration policy／capture-only readiness 均通過 focused QA；PFS-06 新增 6 個 tests，固定三項 input 必須同時 ready，且 heavy rebuild 永遠不由 readiness 啟動。PFS-04～PFS-06 仍只產生受控 fixture 的 prospective sidecar／shadow audit／readiness，不設定正式 path、不寫 `D:`、不啟動 watcher；下一個可執行 slice 是 `PFS-07` frozen identities 與 owner activation。預設仍把 cutoff=`2026-08-13T08:30:00+08:00` 的現有 candidate 綁為未來 clock 的 frozen challenger；clock 啟動後不得用已消費的 Formal OOS 期間重訓或替換 model／calibration，重型 watcher 維持停止。
