...
- 2026-08-28 最新全量 pytest 以 `-o addopts=` 通過 `3674 passed / 1 skipped / 66 warnings`（`537.05s`）；已包含 scheduler inspector CP1252 `--help` guard、availability builder snapshot 選擇回歸與月營收候選狀態測試。沒有新增 failure；warnings 仍主要是既有 subprocess CP1252 decode、joblib physical-core fallback、研究回測同日成交假設與 pytest cache 權限提示。此前 `3668 passed`／`3665 passed` 為前序基準。
