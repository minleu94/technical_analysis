...
- 2026-08-28 全量 pytest 以 `-o addopts=` 通過 `3668 passed / 1 skipped / 26 warnings`（`542.69s`）；Formal candidate inventory 與 freshness probe ACL fail-soft、freshness readiness projection、Evidence Workbench pending-period、formal cutoff 診斷、PowerShell wrapper canonical delegation 納入後的完整回歸沒有新增 failure。之後補上的 scheduler inspector CP1252 `--help` guard 另有 `3 passed / 1 warning` targeted regression，尚未重新跑全量；warnings 仍是既有 joblib physical-core fallback、研究回測同日成交假設與 pytest cache 權限提示。`3665 passed` 為前序中間結果。
