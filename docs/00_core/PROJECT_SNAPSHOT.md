...
2. **下一階段主線：Evidence-Driven baldr + V1.1 / V1.2 / V1.3 / V1.4 v1 已完成**：Evidence Event Store v1 / Forward Outcome Calculator v1 / Evidence Importers v1 / E2E smoke / Forward Performance Read Model v1 / Daily Decision Desk durable snapshot source / source coverage inspection v1 / Forward Performance Dashboard read-only UI v1 / Evidence Pipeline Runner dry-run v1 / working-copy DB smoke v1 / scheduler approval checklist v1 / Live vs Research Gap linkage v1 / Signal Decay Monitor v1 / Decision Quality Review v1 / Evidence Review Dashboards read-only UI pack v1 / Evidence Review UI smoke checklist / multi-day dry-run record scaffold / safe scheduled CMD wrappers / V1.3 weekly evidence operations package / V1.4 weekly review history 已建立；每日 05:30 Codex read-only 摘要、Evidence Review UI smoke 與 multi-day dry-run evidence 持續背景累積。V1.1 已補推薦 Profile 可見性、推薦回放語意與 Profile replay comparison service；V1.2 已補 replay 訓練 / 驗證分離、rolling risk、microstructure preflight 與 relative attribution；V1.3 已補 manual approval summary、weekly review CLI、signal decay manual lifecycle candidate 與 action item planning；V1.4 已補 weekly review history repository、CLI save/list 與 Research Lab `覆盤歷史` 唯讀子頁。完整 Daily Decision / Market Watch 合併仍留待 V2.0 評估；下一步是用 weekly review + history 實際累積數週覆盤證據。Production scheduler implementation 仍需 blocking gaps 修正、人工 approval 與明確設計後才可進行。
