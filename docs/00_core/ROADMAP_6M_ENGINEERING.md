...
- 推薦組合回放已具備 `portfolio_credibility` 限制揭露、缺價型 `unfilled_orders`、entry-day 成交量參與率檢查、cash-gated holding creation、optional bps execution costs 與 optional full-lot sizing；後續補權重、零股、價差、完整撮合與 Gap 模型。
...
2. 設計 Portfolio Replay 可信度補強：`portfolio_credibility` manifest 已開始揭露限制，缺價推薦已記錄為 `unfilled_orders`，提供 `max_participation_rate` 時會記錄 entry-day 成交量不足的 `liquidity_limited`，建立 holding 前也會用可用現金 gate 並記錄 `cash_limited`，提供 fee / tax / slippage bps 時會套用 execution costs，提供 `lot_size` 時會套用整股 sizing；後續補權重、零股、價差、完整撮合與 Gap 實際模型。
