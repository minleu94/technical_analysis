...
- 推薦組合回放已具備 `portfolio_credibility` 限制揭露、缺價型 `unfilled_orders` 與 entry-day 成交量參與率檢查；後續補現金帳、權重、再平衡現金重用、完整撮合與 Gap 模型。
...
2. 設計 Portfolio Replay 可信度補強：`portfolio_credibility` manifest 已開始揭露限制，缺價推薦已記錄為 `unfilled_orders`，並在提供 `max_participation_rate` 時記錄 entry-day 成交量不足的 `liquidity_limited`；後續補現金帳、權重、再平衡現金重用、完整撮合與 Gap 實際模型。
