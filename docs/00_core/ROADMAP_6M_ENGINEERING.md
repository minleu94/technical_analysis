...
- Post-V1 第一批 evidence 增量已完成 Evidence Event Store v1 / Forward Outcome Calculator v1 / Evidence Importers v1 / E2E smoke / Forward Performance Read Model v1 / Evidence Source Persistence v1；這是 forward evidence 的資料底座、source capture 起點、durable Daily Decision Desk snapshot source 與唯讀彙總層，不是 Forward Performance Dashboard 完成，也不是投資有效性證明。
...
1. V1 release baseline 已完成；Post-V1 Evidence Event Store v1 / Forward Outcome Calculator v1 / Evidence Importers v1 / E2E smoke / Forward Performance Read Model v1 / Evidence Source Persistence v1 已落地。下一步優先建立 Forward Performance Dashboard read-only UI 與 scheduler dry-run design；Live vs Research Gap Dashboard 仍是下一個 evidence 主線。
