...
- 2026-07-09：完成 Signal Decay Monitor v1，新增 decay DTO / repository / service / capture / inspect CLI 與 lifecycle proposed payload；decay observation 是 evidence，不是自動 lifecycle action，也不證明任一訊號有效或失效。
- 2026-07-10：完成 Decision Quality Review v1，新增 review DTO / repository / service / capture / inspect CLI、review item detection、integer bp process score 與 action item；Decision Quality 是流程 evidence，不是投資能力或責備判斷。
- 2026-07-11：完成 Evidence Review Dashboards read-only UI pack v1，將 Research Lab evidence 檢查整合為 `Evidence Review` 分頁，內含 Forward Evidence、Live vs Research Gap、Signal Decay 與 Decision Quality；Dashboard 只讀已保存 evidence / observation / review，不寫 evidence、不建立 scheduler、不自動 lifecycle action、不宣稱 alpha。
- 2026-07-12：新增 Evidence Review manual smoke checklist、multi-day dry-run record template 與 scheduler approval SOP；這些是正式 scheduler 前的人工 QA scaffold，不代表 production scheduler 已啟用，也不代表 alpha 或策略有效。
- 2026-07-12：新增 safe scheduled dry-run wrappers、register / unregister scripts、morning check guide 與 scheduler QA；每日自動範圍只包含 read-only data freshness check 與 evidence pipeline dry-run，working-copy smoke 預設 disabled / manual-only，production write-mode scheduler 仍未啟用。
- 2026-07-12：因 PowerShell `.ps1` 被 local execution policy 擋住，新增 CMD wrapper + Windows `schtasks.exe` 實際註冊路徑；每日本機時間 05:00 執行 freshness check、05:15 執行 evidence dry-run，仍不建立 production confirm schedule。
- 2026-07-12：新增 Codex app read-only morning report automation；每日本機時間約 05:30 只讀 Windows Task Scheduler 狀態、latest status、最新 report 與必要 log，產生繁體中文摘要，不重新執行 pipeline、不建立或修改 Windows task。
