...
4. V1.6 Cross-sectional Factor Pipeline v1 已完成：daily factor snapshot DTO / repository / migration、`FactorService` / `FactorGate` pipeline、integer rank / quantile、sector / concept metadata、concept available-date gate 與 read-only attribution summary CLI 已落地；不改 `ScoringEngine`、不把 rank 當推薦、不啟用 production scheduler。V1.7 準備接續 Screening Matrix 與 Negative Evidence；why-not / liquidity payload 持久化從 V1.5 warning 邊界移交至 V1.7，三大法人、信用交易與 TDCC 只能在 source / available_date / quality / missing policy 完整後接入。
