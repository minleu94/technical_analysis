...
1. V1 release baseline 與 V1.1 workflow bridge v1 已完成；Evidence dry-run / 05:30 read-only 摘要 / Evidence Review UI smoke / multi-day dry-run record 繼續背景執行，用來累積資料可信度與 blocking gaps。
2. 下一個工程焦點轉向 V1.2 Research Credibility & Execution Model：Profile / replay 比較需補獨立驗證期間、walk-forward、execution / microstructure credibility、benchmark / industry attribution 與 OOS 判讀。
3. Month 6.1 / V1.3 仍保留為 lifecycle / feedback 的人工審核深化：Manual Approval Workflow、Strategy Evidence Explainability、demote / retire manual review、Portfolio Review Dashboard 深化與 QA checklist。
