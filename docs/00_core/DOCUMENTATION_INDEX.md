...
| `scripts/build_performance_canary_owner_packet.py` | 將 technical／worker／broker／Direct-OOC storage／retention 的 TEMP evidence 整理成 candidate-only owner review packet；不啟動 worker、不刪除或搬移 run、不開 production pool。 |
| `scripts/qa_research_registry_production_canary.py` | Research Registry production transaction／rollback canary；預設唯讀，需 owner approval／無並行 writer acknowledgement／explicit confirm，先 snapshot 到 OS TEMP，唯一 marker rollback 後以 hash／row count／quick-check 驗證無 durable change。 |
...
| [PROSPECTIVE_FORMAL_CALENDAR_CAPTURE_2026_08_28.md](../06_qa/PROSPECTIVE_FORMAL_CALENDAR_CAPTURE_2026_08_28.md) | 2026-09-01..02 官方 TWSE／TPEX bounded calendar capture、response hashes、candidate bundle 與 planner／Formal 邊界。 |
