...
- Research Console 排錯：未設定 `RESEARCH_CONSOLE_PROJECTION`、檔案不存在、JSON/schema 不合法時，頁面仍可開啟並顯示 `Missing` 或 `degraded`；若 projection 宣稱 formal OOS、非零 alpha、promotion eligible 或任何 apply flag，service 會以 `projection_boundary_violation` fail-closed，不套用該內容。啟動與重新載入只讀 JSON，不掃描 development／正式資料目錄、不建立 SQLite 連線、table、directory 或 artifact。
