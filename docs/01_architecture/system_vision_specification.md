...
| Evidence Event Store / Forward Outcome Calculator / Importers / Read Model / Source Persistence / Forward Performance Dashboard | v1 已完成 | SQLite evidence tables 可保存事件與 close-to-close forward outcomes；Recommendation persisted importer、durable Daily Decision Desk snapshot source、Watchlist / Portfolio Alert / Risk Prompt importers 已建立；Why Not / Liquidity exclusion payload 為 optional / partial；Forward Performance Read Model 可唯讀彙總 ready / pending / missing outcomes；Research Lab `Forward Evidence` 分頁可檢查 sample、pending、missing、quality、warnings、benchmark / industry 缺口 | 工程契約、capture service、dry-run / confirm CLI、E2E smoke、read model service / CLI、snapshot repository / CLI、source coverage CLI、dashboard service / Qt UI 與 focused tests 已建立；仍需 scheduler dry-run design | 只能證明事件、snapshot、outcome 與 summary 可被保存、擷取、計算、彙總與檢查；尚不能證明任一訊號有效。 |
...
| Forward Performance Dashboard | 證據缺口 | P1 | Research / App | 資料底座、importer v1、E2E smoke、dashboard read model v1、durable DDD snapshot source 與 Research Lab read-only UI v1 已完成；scheduler 僅能進入 dry-run design / ready-for-design，不是 production-ready | 仍無法證明 Watchlist Trigger、Recommendation、Why Not、Liquidity Gate 或 Portfolio Alert 是否有效。 | 累積事件後 5 / 10 / 20 / 60 日 forward return、benchmark excess return、industry / concept excess return、誤報率與樣本數；正式 scheduler 前需另做 dry-run scheduler design。 |
