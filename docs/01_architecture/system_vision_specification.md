...
| Evidence Event Store / Forward Outcome Calculator / Importers | v1 已完成 | SQLite evidence tables 可保存事件與 close-to-close forward outcomes；Recommendation persisted importer 與 Watchlist / Portfolio Alert / Risk Prompt DTO importers 已建立；benchmark / industry 缺資料以 warnings 揭露 | 工程契約、capture service、dry-run / confirm CLI 與 focused tests 已建立；仍需 dashboard read model 與 durable DDD snapshot source | 只能證明事件與 outcome 可被保存、擷取與計算；尚不能證明任一訊號有效。 |
...
| Forward Performance Dashboard | 證據缺口 | P1 | Research / App | 資料底座與 importer v1 已完成；dashboard read model 未完成；DDD 類 CLI capture 仍缺 durable snapshot source | 仍無法證明 Watchlist Trigger、Recommendation、Why Not、Liquidity Gate 或 Portfolio Alert 是否有效。 | 接 dashboard read model 與 durable DDD snapshot source，追蹤事件後 5 / 10 / 20 / 60 日 forward return、benchmark excess return、industry / concept excess return、誤報率與樣本數。 |
...
2026-07-01 第一增量已先完成 Evidence Event Store v1 / Forward Outcome Calculator v1，可保存 events 與 close-to-close forward outcomes。2026-07-02 第二增量已完成 Evidence Importers / Capture Pipeline v1：Recommendation 可讀 persisted result，Watchlist Trigger / Portfolio Alert / Risk Prompt 可讀注入式 DTO provider；CLI 預設 dry-run，只有 `--confirm` 寫入。尚未建立 dashboard read model，也尚未讓 DDD 類 source 具備 durable snapshot repository；目前不能用來宣稱 alpha 或任何事件類型有效。
