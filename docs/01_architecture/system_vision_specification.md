...
| Evidence Event Store / Forward Outcome Calculator / Importers / Read Model / Source Persistence / Forward Performance Dashboard / Pipeline Runner / Working-copy Smoke / Live Research Gap Linkage | v1 已完成 | SQLite evidence tables 可保存事件與 close-to-close forward outcomes；Recommendation persisted importer、durable Daily Decision Desk snapshot source、Watchlist / Portfolio Alert / Risk Prompt importers 已建立；Why Not / Liquidity exclusion payload 為 optional / partial；Forward Performance Read Model 可唯讀彙總 ready / pending / missing outcomes；Research Lab `Forward Evidence` 分頁可檢查 sample、pending、missing、quality、warnings、benchmark / industry 缺口；manual runner 可串接 source coverage、snapshot capture、event capture、outcome calculation、summary 與 diagnostics report；working-copy smoke 可重複 confirm 並檢查 idempotency；Live vs Research Gap linkage 可保存 portfolio source trace 與 Evidence Event / Outcome gap observation | 工程契約、capture service、dry-run / confirm CLI、E2E smoke、read model service / CLI、snapshot repository / CLI、source coverage CLI、dashboard service / Qt UI、pipeline runner CLI、working-copy smoke CLI、readiness evaluator、approval checklist、gap repository / service / CLI 與 focused tests 已建立；production scheduler 仍未啟用 | 只能證明事件、snapshot、outcome、summary、runner diagnostics、working-copy smoke 與 gap observation 可被保存、擷取、計算、彙總與檢查；尚不能證明任一訊號有效或完整實帳歸因。 |
