...
Paper fills 的受控輸入由 `app_module/paper_trade_import_service.py` 與 `scripts/append_paper_trade_csv.py` 提供，刻意不重用手動 `TradeImportService` 的 Portfolio DTO 寫入路徑。CSV 預覽要求完整 execution 欄位與可驗證的狀態／數量／價格／Decimal 成本／turnover／execution gap，確認後才以 `PaperTradeLedgerRepository.append_many()` atomic append；source hash 會綁入 `source_event_id`，固定 `source_type=paper_trade_import`、`research_only=true`、`broker_order_allowed=false` 與 `auto_rebalance_allowed=false`。Paper UI 只在確認後寫成本 ledger，寫入後再以既有 readiness／weekly evidence read model 重新讀取，不修改手動 Portfolio、snapshot 或正式市場 DB。
