...
- `data_module/monthly_revenue_availability_history.py` / `scripts/build_monthly_revenue_availability_history.py`（Month 5 月營收公告日 historical dry-run builder；讀 TWSE/TPEX 官方 `出表日期`、人工提供 JSON 或人工保存的 MOPS 官方 HTML，輸出候選 mapping / diagnostics，不寫正式 mapping 或 SQLite）
...
- 2026-06-16：新增 TPEX daily price 日常 adapter 與歷史 dry-run planner 架構邊界，確認每日股價 sync 同時讀取 TWSE `daily_price/` 與 TPEX `daily_price_tpex/`，並 upsert `daily_prices`。
