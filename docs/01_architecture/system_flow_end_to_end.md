...
**功能**：
- 數據更新（每日/大盤/產業）
- 市場觀察（強勢股/弱勢股、強勢產業/弱勢產業、市場 Regime）
- 推薦分析（策略配置、執行、結果顯示、產業篩選、加入觀察清單）
- 策略回測（單檔/批次回測、參數最佳化、Walk-forward、策略預設、Promote 機制）
- 觀察清單管理（跨 Tab 共用、新增/移除/清空、選股清單管理）

**位置**：`ui_qt/main.py`（推薦使用 PySide6 版本）
