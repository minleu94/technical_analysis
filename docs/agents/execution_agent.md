...
- 策略、回測、推薦、資金、倉位、風控與績效等核心計算嚴禁新增裸 `float` 計算；實作前必須完成 Look-ahead bias 自查
- **必須使用繁體中文**（所有文檔、對話、回答、註解都必須使用繁體中文，禁止使用簡體中文）
- 保持向後相容性
- 確保代碼品質
- 同步更新文檔與測試；文檔判讀採 Scoped SSOT（Snapshot / 6M Roadmap / V2.1-V4.0 Roadmap / Roadmap Hub / Architecture / Index）
- **不主動重構或最佳化未指定的代碼**
- **不補齊未明確要求的步驟**
...
- [ ] 檢查回歸問題
- [ ] 驗證邊界情況
- [ ] UI 修改後已執行 `scripts/qa_validate_update_tab.py`
- [ ] UI 修改後已執行型態檢查

### 文檔更新
- [ ] 更新相關文檔
- [ ] 依 `docs/00_core/DOC_COVERAGE_MAP.md` 檢查 Snapshot / 6M Roadmap / V2.1-V4.0 Roadmap / Roadmap Hub / Architecture / Index 是否需要同步
- [ ] 更新 API 文檔（如有變更）
- [ ] 更新使用指南（如有新功能）
- [ ] 記錄變更日誌
...
- 2026-07-06：補上 V2.0 之後長期版本階梯 companion 的文檔同步檢查與參考入口。
