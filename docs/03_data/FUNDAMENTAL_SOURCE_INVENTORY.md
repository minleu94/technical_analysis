...
2026-08-05 以 Owner 授權的 bounded research acquisition 已取得 17 個新的 MOPS 2025-Q1 immutable numeric／publication candidates（DEV-82 的 7 個、DEV-84 的 1215、DEV-85 的 5 個、DEV-86 的 4 個）；連同既有候選共 35 個 identity，聚合後為 1,540 個 PIT-eligible rows、85 bp coverage（canonical denominator 179,271，距固定 8,000 bp 門檻 7,915 bp）。DEV-84 的 1217、1218、1219、1220 及 DEV-86 的 1256 仍因 timeout／DNS diagnostics fail-closed；DEV-85 五個新身份均取得成功，DEV-86 其餘四個成功；DEV-83 的 EZSearch F26–F29 仍全部 `invalid_response`、0 events／0 projections。未猜測原因、未補值或重試成形式資料；所有 artifacts 留在 TEMP research root，未寫正式 SQLite、未 materialize、未進 fit/training，也不構成 source acceptance。
