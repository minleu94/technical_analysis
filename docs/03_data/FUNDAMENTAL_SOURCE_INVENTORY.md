...
2026-08-05 以 Owner 授權的 bounded research acquisition 已取得 13 個新的 MOPS 2025-Q1 immutable numeric／publication candidates（DEV-82 的 7 個、DEV-84 的 1215、DEV-85 的 5 個）；連同既有候選共 31 個 identity，聚合後為 1,389 個 PIT-eligible rows、77 bp coverage（canonical denominator 179,271，距固定 8,000 bp 門檻 7,923 bp）。DEV-84 的 1217、1218、1219、1220 仍分別因 MOPS timeout、doc.twse DNS failure、listing timeout、MOPS timeout fail-closed；DEV-85 五個新身份均取得成功；DEV-83 的 EZSearch F26–F29 仍全部 `invalid_response`、0 events／0 projections。未猜測原因、未補值或重試成形式資料；所有 artifacts 留在 TEMP research root，未寫正式 SQLite、未 materialize、未進 fit/training，也不構成 source acceptance。
