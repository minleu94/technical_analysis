...
1. **備份**：執行前會自動備份現有文件，備份位置在 `meta_data/backup/` 目錄；同一來源同一天只保留最新一份，且最多保留最新 5 個日期版本；備份來源與既有大檔清理候選見 [BACKUP_RETENTION_AUDIT_2026_07_06.md](BACKUP_RETENTION_AUDIT_2026_07_06.md)
2. **時間**：重建所有數據可能需要較長時間，取決於數據量
3. **磁碟空間**：確保有足夠的磁碟空間存儲備份和臨時文件；備份清理只作用於 `meta_data/backup/` 內完全符合來源前綴的舊備份，不會刪除正式資料或不同用途的標籤備份
