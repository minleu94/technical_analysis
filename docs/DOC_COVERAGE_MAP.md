...
1. **`docs/DEVELOPMENT_ROADMAP.md`** - 最高權威
...
2. **`docs/PROJECT_SNAPSHOT.md`** - 次級權威
   - 從 Roadmap 抽出的短版入口
   - 必須與 Roadmap 的當前狀態一致

3. **`docs/DOCUMENTATION_INDEX.md`** - 索引權威
...
5. **其他專項文檔** - 功能特定權威
   - `docs/UI_FEATURES_DOCUMENTATION.md` - UI 功能說明
   - `docs/USER_GUIDE.md` - 使用者指南
   - `docs/system_architecture.md` - 系統架構
   - `docs/STRATEGY_DESIGN_SPECIFICATION.md` - 策略設計規格
...
**必須更新（Must）：**
- `docs/UI_FEATURES_DOCUMENTATION.md` - UI 功能說明
- `docs/PROJECT_SNAPSHOT.md` - 「現在的工作模式」段落（如影響使用流程）
- `docs/DOCUMENTATION_INDEX.md` - 若新增文檔，需更新索引

**應該更新（Should）：**
- `docs/USER_GUIDE.md` - 使用者指南（如有操作流程變更）
- `PROJECT_NAVIGATION.md` - 「功能導航」段落（如新增功能）
- `docs/DEVELOPMENT_ROADMAP.md` - Living Section（如影響 Phase 狀態）
...
- `docs/system_architecture.md` - 系統架構文檔（如架構變更）

**應該更新（Should）：**
- `docs/DEVELOPMENT_ROADMAP.md` - Living Section（如影響 Phase 狀態）
- `docs/PROJECT_SNAPSHOT.md` - 「高風險區」段落（如涉及高風險模組）
...
**必須更新（Must）：**
- `docs/BACKTEST_LAB_FEATURES.md` - 回測功能文檔（如回測邏輯變更）
- `docs/USER_GUIDE.md` - 使用者指南（如操作流程變更）
- `docs/PROJECT_SNAPSHOT.md` - 「高風險區」段落（這些都是高風險區）

**應該更新（Should）：**
- `docs/STRATEGY_DESIGN_SPECIFICATION.md` - 策略設計規格（如策略邏輯變更）
- `docs/DEVELOPMENT_ROADMAP.md` - Living Section（如影響 Phase 狀態）
- `docs/DOCUMENTATION_INDEX.md` - 若新增文檔，需更新索引
...
- `docs/DOCUMENTATION_INDEX.md` - 文檔索引（如新增 Agent 文檔）

**應該更新（Should）：**
- `docs/PROJECT_SNAPSHOT.md` - 「Tech Lead 的預設任務」段落（如 Tech Lead 變更）
...
**必須更新（Must）：**
- `docs/DEVELOPMENT_ROADMAP.md` - Living Section（Phase 狀態、Next、Blockers）
  - **Living Section 範圍**：見該文件的「📍 Living Section 定義」段落
- `docs/PROJECT_SNAPSHOT.md` - 所有相關段落（當前狀態、工作模式、優先事項）
- `docs/DOCUMENTATION_INDEX.md` - 若索引內進度文字過期，需更新

**應該更新（Should）：**
- `README.md` - 專案主文檔（如 Phase 狀態變更）
- `docs/CURRENT_STATUS.md` - 當前開發狀態（如存在）
...
**必須更新（Must）：**
- `docs/data_collection_architecture.md` - 資料收集架構（如資料架構變更）
- `docs/DATA_FETCHING_LOGIC.md` - 資料獲取邏輯（如資料獲取邏輯變更）
- `docs/daily_data_update_guide.md` - 每日資料更新指南（如更新流程變更）
...
**必須更新（Must）：**
- `docs/STRATEGY_DESIGN_SPECIFICATION.md` - 策略設計規格（如策略規格變更）
- `docs/DOCUMENTATION_INDEX.md` - 若新增策略文檔，需更新索引

**應該更新（Should）：**
- `docs/USER_GUIDE.md` - 使用者指南（如影響策略使用）
- `docs/BACKTEST_LAB_FEATURES.md` - 回測功能文檔（如策略回測變更）

**可選更新（Nice-to-have）：**
- `docs/DEVELOPMENT_ROADMAP.md` - Living Section（如為重大策略變更）
...
**必須更新（Must）：**
- `docs/DOCUMENTATION_INDEX.md` - 文檔索引（必須反映最新結構）

**應該更新（Should）：**
- `docs/PROJECT_SNAPSHOT.md` - 「指定權威文件」段落（如權威文件變更）
...
2. **Snapshot / Index / Roadmap 一致性檢查通過**
   - `docs/PROJECT_SNAPSHOT.md` 與 `docs/DEVELOPMENT_ROADMAP.md` 的 Living Section 一致
   - `docs/DOCUMENTATION_INDEX.md` 與 `docs/DEVELOPMENT_ROADMAP.md` 的 Living Section 進度描述一致
   - 若發現不一致，必須修正（以 Roadmap 的 Living Section 為準）
   - **Living Section 定義**：見 `docs/DEVELOPMENT_ROADMAP.md` 的「📍 Living Section 定義」段落

3. **相關使用指南已更新**
   - 若變更影響使用者操作流程，`docs/USER_GUIDE.md` 必須更新
   - 若變更影響 UI 功能，`docs/UI_FEATURES_DOCUMENTATION.md` 必須更新
...
2. **架構文檔已更新**
   - 若變更影響系統架構，`docs/system_architecture.md` 或 `PROJECT_NAVIGATION.md` 已更新
