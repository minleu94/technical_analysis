...
**Status:** Started 2026-06-16. Availability policy exists in `data_module/fundamental_availability.py` and `tests/test_fundamental_availability.py`; raw monthly revenue normalization contract exists in `data_module/fundamental_data.py` and `tests/test_fundamental_data.py`; candidate SQLite schema dry-run, temporary-connection report API, and formal-DB working-copy report API exist in `data_module/fundamental_schema.py` and `tests/test_fundamental_schema.py`; minimal `fundamental.revenue_yoy` adapter contract and tests exist in `decision_module/factors/fundamental_adapters.py` and `tests/test_fundamental_factor_adapters.py`. Missing `available_date` emits diagnostics without a normalized record or factor record; future `available_date` is skipped by `FactorGate` under `MissingPolicy.SKIP`. Formal `twstock.db` working-copy dry-run passed on 2026-06-16: existing core tables were preserved, three candidate fundamental tables were created on the copy only, and the temporary copy was cleaned up after the SQLite handle-release regression fix.
