...
- [ ] 執行 `python -m pytest --collect-only -q -o addopts=`。
- [ ] 將錯誤依無效 API、錯誤模組路徑、固定資料、外部服務與 package collision 分類。
- [ ] 確認每個錯誤都有「遷移、隔離或刪除」唯一處置。
...
- [ ] 先執行個別測試，確認 legacy import failure。
- [ ] 將 `DataConfig` 改為 `TWStockConfig` 並使用 `tmp_path`。
- [ ] 將 `DataProcessor` 期待拆成 `DataLoader` 與 `TechnicalIndicatorCalculator` 的合成資料測試。
- [ ] 移除固定正式資料路徑、`input()` 與人工輸出依賴。
- [ ] 執行修改後測試確認通過。
...
- [ ] 將扁平 import 改為正式子套件入口。
- [ ] 將需要真實檔案或繪圖人工判讀的腳本移至 manual 分類。
- [ ] 保留合成 OHLCV 的 Pattern 與 Technical Analyzer 行為測試。
- [ ] 執行分析測試確認通過。
...
- [ ] 將真實 TWSE/API、視覺化、人工輸入與本機資料診斷移出自動 collection。
- [ ] 在 README 記錄用途、前提與手動執行方式。
- [ ] 執行 `pytest --collect-only` 確認不再有 collection error。
...
- [ ] 記錄現行 API 與 legacy test 處置。
- [ ] 執行全量 pytest、mypy、float boundary、py_compile。
- [ ] 執行 `git diff --check` 與 staged diff review。
- [ ] Commit 並 push 目前分支。
