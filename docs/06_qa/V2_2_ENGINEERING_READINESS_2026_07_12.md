...
- `--save-history` 可在明確指定且經人工核准的 working-copy DB append-only 保存 weekly review snapshot；`--list-history` 在 config 初始化前以 SQLite read-only mode 檢查既有 history，缺 DB / table 只輸出 diagnostics，不建立 DB parent、log directory、schema 或 index。
...
- 2026-07-12：re-review P1 將 `--list-history` 移至 config 初始化前的純路徑解析，避免 `--data-root` 造成 QA 目錄或 log 副作用。
