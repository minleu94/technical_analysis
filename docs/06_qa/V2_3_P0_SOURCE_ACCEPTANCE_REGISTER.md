...
## 13. 2026-08-05 Owner formal development-plan activation（append-only）

- **Owner 決議**：owner=`archi` 確認已審閱既有 P0/P13、PIT、ML custody、weekly custody 與 Formal capture blockers，並將它們正式納入 `V4.0 Evidence Accumulation Track` 的 development plan；decision=`owner:v4-evidence-accumulation-development-plan:20260805-r1`，owner instruction SHA-256=`9533c3bef602937af9b6225a807075ea58f247ac352d8397492ea6acbaea9806`。
- **已授權的工程範圍**：P0/P13 官方 artifact 的 bounded、foreground、唯讀、TEMP research acquisition；MOPS numeric PIT lineage／coverage validation；ML TEMP PIT publication 與 sidecar custody diagnostics；sanitized Research Console projection；weekly custody/manual-review hardening；以及 Formal `manual_observed` capture readiness tooling。這是 development plan activation，不是 Formal evidence artifact。
