...
- 本次相關 repo 變更已分批提交：`44cb70a`（readiness contract 與 regression test）、`3857a5f`（clock QA failure record）、`bfecb18`（weekly status addendum）；未 push。回滾程式只需逐一 `git revert 44cb70a`，文件則依 review 分別 revert `3857a5f`／`bfecb18`；不使用 reset 或 checkout 覆寫其他工作。
