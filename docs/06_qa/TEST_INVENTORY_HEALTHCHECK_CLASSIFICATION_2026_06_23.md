...
- pytest support 檔：`1`
- 預設不收集檔：`30`
- `pytest --collect-only -q -o addopts=`：`3664 tests collected`

2026-08-28 全量 pytest 已以 `-o addopts=` 完成：`3663 passed, 1 skipped, 26 warnings`（`564.24s`；freshness readiness projection、Evidence Workbench pending-period 與 formal cutoff 診斷納入後的完整回歸）。
