...
> Latest follow-up after the prospective clock planner slice: filesystem／inventory=`653/653`、`3719 tests collected`；前一輪段落中的 `652/652`／`3702`／`3691`／`3696` 僅作歷史基準，以下 current count 以本行與後續欄位為準。
...
Current filesystem Python files: `653`

最新機器重算（2026-08-28）以 `pytest --collect-only -q -o addopts=` 實際收集 `3719 tests collected`；下方歷史段落中的 `3702`／`3684`／`3683`／`3691`／`3696` 為前一輪中間讀數。

- 預設 pytest 可收集測試檔：`615`
- pytest support 檔：`1`
- 預設不收集檔：`30`
- `pytest --collect-only -q -o addopts=`：`3719 tests collected`
