...
- pytest support 檔：`1`
- 預設不收集檔：`30`
- `pytest --collect-only -q -o addopts=`：`3661 tests collected`

2026-08-28 全量 pytest 已以 `-o addopts=` 完成：`3660 passed, 1 skipped, 26 warnings`（`576.78s`；新增 intake envelope guard 測試後的完整回歸）。
