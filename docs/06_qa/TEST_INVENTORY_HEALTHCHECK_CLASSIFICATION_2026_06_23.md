...
- pytest support 檔：`1`
- 預設不收集檔：`30`
- `pytest --collect-only -q -o addopts=`：`3657 tests collected`

2026-08-28 全量 pytest 已以 `-o addopts=` 完成：`3656 passed, 1 skipped, 26 warnings`
（`563.37s`；未指定 JUnit 輸出）。下方較早的 `3613 passed`／`3607 passed`／`3606 passed`／`3603 passed`／`3592 passed` 是前一輪中間基準，
不取代本次完整回歸結果。
