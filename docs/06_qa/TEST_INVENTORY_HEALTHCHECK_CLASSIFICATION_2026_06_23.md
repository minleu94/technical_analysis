...
`637/637`，缺漏路徑、過時路徑與 collection errors 均為 `0`。新增納管的測試主要是
...
Current filesystem Python files: `637`

- 預設 pytest 可收集測試檔：`606`
- pytest support 檔：`1`
- 預設不收集檔：`30`
- `pytest --collect-only -q -o addopts=`：`3596 tests collected`

| 分類 | 數量 |
|---|---:|
| `general-unit-keep-in-pytest` | 142 |
