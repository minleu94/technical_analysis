...
本次 technical production canary guard 新增 7 個定向測試，P0 license evidence candidate capture 新增 5 個定向測試，月營收 availability candidate projection 新增 6 個定向測試；`pytest --collect-only` 已更新為 `3748`，並已完成耗時的全量回歸。
...
Current filesystem Python files: `656`

本次機器重算（2026-08-28）以 `pytest --collect-only -q -o addopts=` 實際收集 `3748 tests collected`；前一輪 `3726`／`3719`／`3702`／`3684`／`3683`／`3691`／`3696` 僅作歷史中間讀數。
