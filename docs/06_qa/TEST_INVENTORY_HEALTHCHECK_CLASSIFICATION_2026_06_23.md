...
> 機器重算（2026-08-27）：新增 `tests/test_update_status_history.py` 後，實際為 `3603 tests collected`、filesystem／inventory `638/638`；current category `general-unit-keep-in-pytest=143`。以下 current count 欄位已同步，前述段落中的 `3596`／`637` 為補登錄前的中間讀數。
...
- pytest support 檔：`1`
- 預設不收集檔：`30`
- `pytest --collect-only -q -o addopts=`：`3603 tests collected`
