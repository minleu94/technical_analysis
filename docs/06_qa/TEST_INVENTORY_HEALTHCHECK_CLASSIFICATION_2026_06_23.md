...
> 機器重算（2026-08-28）：新增 `tests/test_render_p0_owner_decision_packet.py` 並完成 inventory 登錄後，實際為 `3608 tests collected`、filesystem／inventory `639/639`；current category `general-unit-keep-in-pytest=143`。以下 current count 欄位已同步，前述段落中的 `3596`／`637` 為補登錄前的中間讀數，`3603`／`3604`／`3607` 為前序中間讀數。
...
2026-08-28 全量 pytest 已以 `-o addopts=` 完成：`3607 passed, 1 skipped, 26 warnings`
（`518.78s`；未指定 JUnit 輸出）。下方較早的 `3606 passed`／`3603 passed`／`3592 passed` 是前一輪中間基準，
不取代本次完整回歸結果。
