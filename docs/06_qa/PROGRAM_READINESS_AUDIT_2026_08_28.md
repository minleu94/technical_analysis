...
- 最新全量回歸（Registry clone transaction、storage retention inventory、monthly-revenue candidate projection 與 weekly review CLI UTF-8 guard 修正後）：`3747 passed / 1 skipped / 26 warnings`（`533.67s`；未指定 JUnit 輸出）；這只更新工程回歸證據，不會把 production scheduler、正式 ACL、owner decision 或資料 gate 誤標成 ready。
