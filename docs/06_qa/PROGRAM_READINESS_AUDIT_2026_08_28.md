...
- freshness probe／PowerShell wrapper post-change regression：`3664 passed / 1 skipped`；`run_daily_data_freshness_check.ps1` 已以正式資料唯讀＋TEMP status/log 實際執行 `status=passed`、daily／technical latest=`20260828`，並與 `.cmd` 共用同一個 canonical probe。
