...
| 效能工程 | technical full-batch、isolated CSV／SQLite writer、real indicator process-pool staging 與 synthetic bounded contract 已量測 | full-batch read／calculate／aggregate、CSV serialization、SQLite lock/retry、real calculator process pool（bounded in-flight／retry／parent single writer）與 synthetic queue／cancel checks 均有 artifact；production worker 仍為 1 | 券商來源 rate limit／retry／Selenium 邊界；worker crash recovery／長時間取消與 production single-writer integration | 可以；先完成 worker recovery／integration，再分開驗收 broker fetch，不能直接拉高 thread 數 |
...
5. 已完成 technical indicator 各階段耗時、writer contention 與 real staging bounded process-pool 初步證據；下一步補 worker recovery／integration，再實作 broker bounded fetch＋single-writer queue。
