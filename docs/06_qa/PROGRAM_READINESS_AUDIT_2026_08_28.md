...
- 既有 live P0 audit 已另以 `scripts\build_p0_intake_from_audit.py` 轉成 13 列 candidate intake；輸出 `C:\Users\archi\AppData\Local\Temp\technical_analysis_p0_audit\p0_candidate_intake_from_audit_20260828.json`，SHA-256=`E1E9BFAE3DEACD2FBB715BD1F056E6A302C6A002424C2298E711517F2F1D063`。唯讀 validator 結果為 `deferred`、`valid=13`、`owner_review_ready=0`；轉接器只帶入 machine row counts／payload hash，沒有填入或推導 owner／license／publication／PIT 決議，`downstream_eligibility` 仍為 `none`。
