...
這仍不是完整產品 closeout。Data Update 尚未把 route／fallback／publication／治理狀態完整串進同一個使用者畫面；Paper 成本後週報與 ML Formal lane 也尚未可計算。正確做法是繼續完成可工程化部分，同時把外部輸入與時間證據獨立追蹤，不再把兩者統稱為「功能沒做完」。
...
| Paper Portfolio | 只有 snapshot、週報不可算 | 21 筆 Paper snapshot；QA staging Equal Weight ledger 21 筆，benchmark reader=`ready` | 真實 fill／partial-fill／reject／override、Decimal 成本、turnover、execution gap；正式 Paper Trade Ledger 缺失 | 可以；benchmark 工程已通，execution evidence 不可推造 |
...
| Data Update 顯示 | 卡片／頁面狀態容易互相矛盾 | fail-closed 顯示、台灣市場日期、候選分頁與 inline summary 已有回歸保護 | live audit 尚未自動成為 UI 的 current projection；route used、fallback reason、publication/PIT class、last successful capture 與 governance decision 仍需統一投影 | 可以；這是下一個明確 UI／read-model slice |
...
4. **Runtime write-handle 不是 SQLite transaction。** 現有 probe 已排除單純 ACL blocker，但還不能宣稱 Registry schema／transaction／rollback 全部實寫成功。

## 持續推進順序

1. 將 live P0 audit、actual route、fallback reason、publication/PIT class 與 owner decision 投影接進 Data Update／Research Console，同時保留 candidate-only 安全邊界。
2. 以 5 組 owner packet 完成 13 項 source 的 license/use-case/reviewer 決議；可先 `limited`，不必等待全部來源一次 accepted。
3. 將 QA Equal Weight builder 納入明確受控的 Paper benchmark 建置流程；由真實 paper execution producer 或使用者提供完整 fills CSV，建立 Paper Trade Ledger 後才計算成本後週報。
