...
由於本輪只更新 P0 machine evidence，尚未以這份新 audit 重算 unified readiness；現行 readiness artifact 仍維持前節所記錄的 `action_required` 與原有 Evidence／Paper／Formal／performance blockers。下一步仍是 owner／license／publication／PIT 複核，以及對既有 raw rows 建立可逐列追溯的 available-date mapping；不能用這批 availability candidate 直接回填或解除任何 gate。
