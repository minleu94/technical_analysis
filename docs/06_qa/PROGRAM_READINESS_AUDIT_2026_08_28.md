...
## Evidence pending review inventory（2026-08-28 host read-only）

直接以 SQLite `mode=ro`／`PRAGMA query_only=ON` 讀取 `D:\Min\Python\Project\FA_Data\output\scheduled\v2_2_weekly_collection\evidence_scheduler.db`，目前共有 9 筆 collection，狀態全部是 `pending_human_review`：`2026-07-06..07-12`、`07-13..07-19`、`07-20..07-26`、`07-27..07-29`、`07-27..08-02`、`08-03..08-09`、`08-10..08-16`、`08-17..08-23`、`08-24..08-28`。這些列是實際 sidecar append 結果，不是 replay 或 projection；它們仍需具名 owner／reviewer 逐期核准，核准前 `weekly=0/3`、`formal_credit_authorized=false`、`production_scheduler_allowed=false` 都維持不變。

## 2026-08-29 Evidence continuation（host append）

以來源 SQLite 的唯讀連線觀察到最新交易日 `2026-08-28` 後，已在正式 output 下的 append-only sidecar 新增 collection=`ewc_0cde0f7a8154a7a1`，期間為 `2026-08-24..2026-08-28`、`last_trading_date=2026-08-28`、狀態=`pending_human_review`。sidecar SHA-256=`897B38A32E0F7CF2D4CF3A9CF822DF0BB7F18305495B7683F6C3C0761FC6913E`；同期間 report SHA-256=`49BBE6ECF08A313731EDACEB3F75537F8945705FDC1C1FE341730B87BDBFEFB0`。接著以唯讀 exporter 產生 9 筆 owner/reviewer approval input：`C:\Users\archi\AppData\Local\Temp\technical_analysis_program_readiness\evidence_weekly_approval_input_20260829.json`，packet status=`needs_named_owner_reviewer`、`candidate_only=true`、`write_performed=false`、`formal_credit_authorized=false`；這次只讓真實週期繼續累積，不把 pending 轉成 weekly 或 Formal credit。

重新載入 approved weekly projection、上述 sidecar、Paper／Formal／Runtime／performance 與 scheduler artifacts 的唯讀 unified readiness 為 `C:\Users\archi\AppData\Local\Temp\technical_analysis_program_readiness\program_readiness_after_evidence_week_with_owner_packet_20260829.json`（status=`action_required`；SHA-256=`47032C9ADF79BDD8EE6A585DF40459342CC56051927BE6FD33CA304DF8DBEAA6`）。Evidence read model 為 owner-approved projection=`3/3`、pending sidecar=`9`，但 `formal_credit_authorized=false`；P0=`action_required`、Paper=`partial`、Formal/ML=`action_required`、Runtime／Update History=`ready`、performance=`partial`，沒有因新週期而開啟任何正式 writer、Formal、scheduler 或 broker。
...
| Evidence Gate | weekly `0/3` | owner-approved weekly projection=`3/3`；multi-day dry-run=`3/3`；Pre-V2=`ready`；另以正式 SQLite 唯讀連線收集 `2026-08-24..2026-08-28` 一筆 `pending_human_review` sidecar；readiness 與 Workbench 都明確輸出 `formal_credit_authorized=false`，並揭露 pending 期段 | 另有 9 個 pending-human-review sidecar 期間；此 projection／sidecar 都不授予 Formal credit 或 production scheduler | 可以；Gate 顯示已修正，後續只累積真實週期與審核 |
...
| Runtime | 只有 `os.access` 提示 | `scripts/inspect_runtime_environment_readiness.py --confirm-write-probe` 已能在明確 TEMP staging 以正式 Registry schema 完成 insert／讀回／rollback／清除；2026-08-28 18:44 UTC 的實際 host context 重新檢查 `config.log`／Research Registry write-handle 均成功；另以正式 Registry read-only snapshot clone 通過 schema／quick_check／insert／rollback，統一 readiness=`ready`。早先 `codexsandboxoffline` 沙盒 token 的 `PermissionError` 只作環境觀測，不再當成產品 ACL blocker | 正式 Registry production writer 尚未實寫；若要宣稱 production lock／atomic commit，仍需 owner 在正式流程核准並留存 rollback evidence | 可以；staging transaction、正式既有檔案 handle 與正式 schema clone 均已驗證，後續只需依正式 writer 流程補 transaction／rollback evidence，不必修改 ACL |
| 效能工程 | technical full-batch、isolated CSV／SQLite writer、real indicator process-pool staging、broker bounded fetch 離線 acceptance 與 synthetic contract 已量測；guarded technical production canary entry 已備妥 | full-batch read／calculate／aggregate、CSV serialization、SQLite lock/retry、real calculator process pool（bounded in-flight／retry／parent single writer）、worker process crash recovery／queued cancellation、broker parser／rate-limit／retry／duplicate／failure contract、synthetic queue／cancel checks 與單次 MoneyDJ real HTTP canary 均有 artifact；technical batch 的 production feature flag／scheduler wiring 已接上但預設關閉，broker pool 仍關閉 | 真實來源長期 rate-limit、Selenium driver 重建、technical production backup／rollback 與 owner-approved canary 實跑；staging recovery／單次 HTTP 不等同 production proof | 可以；先由 owner 使用 guarded entry 核准 technical backup／rollback 後做單次 canary，再評估 broker bounded pool，不能直接拉高 thread 數 |
| Data Update 顯示 | 卡片／頁面狀態容易互相矛盾 | fail-closed 顯示、台灣市場日期、候選分頁、inline summary、P0 13 列唯讀 projection 與 Research Console 共用欄位已接上；`data-update-timeline.v1` 明確顯示排程 run、最後成功完成時間、12 個步驟結果與 freshness；runner／UI 已接 `data-update-status-history.v1` append-only 歷史；P0 fallback attempted／date mismatch／network error 已保留並以不同文字呈現；待更新卡片會顯示新鮮度基準日／資料最新日；2026-08-28 quick run=`20260828-29472` terminal=`passed`、12/12，Data Update QA=`23 passed / 0 failed / 4 skipped`；unified readiness 新增明確 `--freshness-status-path`，可分開投影 freshness 的 status／checked_at／warnings／errors | 核心 SQLite 已追上 `2026-08-28`，隔離 TEMP freshness probe 已觀察 `passed`，但正式 `data_freshness/latest_status.json` 仍是 `2026-08-27`；沙盒 probe 對正式 output 的 `PermissionError` 已由 host-context existing-handle recheck 排除為 token 限制；host scheduler 已確認 13/13、action wiring 相符，尚待下一個自然週期產生最新 freshness／history 並維持 production governance 關閉；ML Direct/OOC 另有容量 preflight blocker | 可以；資料更新本身已恢復，先觀察 host scheduler 下一個真實 terminal／freshness history，再處理 Direct/OOC 容量與正式 Registry transaction evidence；不回放補歷史或繞過 candidate-only 邊界 |
...
排程分頁的初始 JSON 預覽也已修正語意：`data_freshness/latest_status.json` 只代表單一 freshness 工作，不再顯示成整體 Scheduler 的 `Simulated/Waiting for time`。按「檢查此資料源狀態」後才會以明確 `scheduled/*/latest_status.json` 產生唯讀彙總；每日資料更新 task 的註冊／執行與 Evidence／ML 生產寫入授權分開判讀，`production_scheduler_allowed=false` 不變。
...
本輪再補上 lane 進度投影：P0 顯示 `machine evidence／source`、`accepted／limited` 與 route probe `attempted／total`；Evidence 顯示 weekly／dry-run `observed／required`、sidecar `pending_human_review` 期數與 Formal credit；Paper 顯示 snapshot／benchmark／cost／fills；Formal／ML 顯示 owner-controlled input `ready／total`；Runtime、performance、Update History 顯示 host probe、canary／容量、排程與 freshness。阻擋欄先顯示中文意義並保留原始 token，避免已有資料時仍被誤讀成「完全沒有功能」；projection 只取固定 allowlist 計數／狀態，不複製大型 details，也不改變任何 gate。最新 host artifact 的 Evidence 進度為 `weekly 0/3`、`pending_human_review=8`、dry-run `3/3`；8 期仍不計 Gate credit，下一步是 owner/reviewer 審核，而不是回放或自動升格。
...
時間軸的判定也會把已提供的 freshness observations 與 quick-run `target_date` 做一致性核對；日價／技術指標日期、quick status／檢查日／預期日或原始日檔缺漏時，會標為 `degraded` 並留下明確 diagnostics。舊 artifact 沒有這些 optional 欄位時仍維持相容，不會以缺欄位自行猜測資料已更新。
...
不寫 task，也不會把缺 task 自動註冊。沙盒觀察 artifact 為
...
目前 host-context artifact 為 `C:\\Users\\archi\\AppData\\Local\\Temp\\technical_analysis_program_readiness\\scheduled_task_status_host_20260828.json`，`available=13/13`、`configuration_ready=true`、`all_actions_match=true`，SHA-256=`14C1F4C59611231961F62105C4608AE3FDBA4A8BE2D48ABA3A5E70281E8C9845`。以該 artifact 及 host runtime context 重算的 unified readiness 為 `C:\\Users\\archi\\AppData\\Local\\Temp\\technical_analysis_program_readiness\\program_readiness_host_runtime_scheduler_20260828.json`（`update_history=ready`、整體仍 `action_required`，SHA-256=`7C3612120555FF28FFC9A45091E925B6E25D2DB2A310A53E1F50D9FB713EFF87`）。
...
read/write handle probe 無診斷。沙盒帳號的拒絕只保留在執行環境紀錄，不再被 unified
readiness 當成正式 ACL 缺口；最新 host-context readiness artifact 為
`C:\Users\archi\AppData\Local\Temp\technical_analysis_program_readiness\program_readiness_host_runtime_20260828.json`，SHA-256=`F6B2C244E9EE7E10F7E7B46418CBC4CEE38CF3BDAD63BF3F3259D3DCECF47A3F`。
...
4. **Runtime staging／clone transaction 不是正式 Registry 實寫。** Staging probe 會在非正式暫存 DB 使用正式 Registry schema；新增的 snapshot probe 再以正式 DB 的 read-only clone 驗證現有 schema、`quick_check`、insert／讀回／rollback 與 source hash 不變。兩者都排除了「程式完全沒有 transaction 路徑」的疑問，但仍不能宣稱正式 Registry 的 ACL／鎖定／production 實寫已成功。
...
6. 已完成 technical indicator 各階段耗時、writer contention、real staging bounded process-pool、worker recovery／取消 acceptance、parent single-writer integration staging，以及 production batch feature flag／scheduler wiring；guarded canary 入口已備妥但尚未執行，下一步是由 owner 核准 backup／rollback 後做單次 technical canary，再觀察正式排程。Direct/OOC 先以 `inspect_ml_storage_retention.py` 完成容量／retention inventory；在 20 GiB headroom 與 owner retention 決策前，不重跑 worker、不刪除 immutable run。
...
- host-context bounded live audit continuation（2026-08-28；13 個 payload 均有 hash，12/13 network observed、1/13 MOPS artifact verified）：`C:\Users\archi\AppData\Local\Temp\technical_analysis_p0_audit\p0_evidence_20260828_continuation_live_host.json`（SHA-256=`212C5C55FF276CCE7198DC4875EFEB5E34B99C32F2031EF2F254484CBAF44EFC`）；raw=`91,127`、accepted=`89,750`、blocked=`1,377`、quarantine=`0`。11 個來源的剩餘 blocker 是 `official_publication_timestamp_missing`，1 個是 `decision_time_availability_not_proven`；這改善 machine evidence，但不改 owner／license／downstream eligibility gate。
...
- 載入上述 host runtime／scheduler／Direct storage，並改用最新 host P0 live audit 後的 unified readiness：`C:\Users\archi\AppData\Local\Temp\technical_analysis_program_readiness\program_readiness_host_runtime_scheduler_storage_v3_p0_live_20260828.json`（status=`action_required`；SHA-256=`B125B1C653F6F6AE1F63E3D21B3F5D78D9DBF2C574AE43B5BA6CA3EABC1D9068`）；lane 狀態為 `p0=action_required`、`evidence=partial`、`paper=partial`、`formal_ml=action_required`、`runtime=ready`、`performance=partial`、`update_history=ready`。P0 的 machine evidence 已更新為 12/13 observed，但 owner／license／publication／Formal／Paper blockers 仍保留。
...
- host live audit 的最新 candidate intake／validator：`C:\Users\archi\AppData\Local\Temp\technical_analysis_p0_audit\p0_candidate_intake_continuation_live_host.json`（SHA-256=`8401CEEEA39D18CF1AE0D6EA3BEB96C736345BE9D626B3DA19B8C218E0497114`）；validator=`deferred`、`valid=13`、`owner_review_ready=0`，report SHA-256=`1F77970B9FFD0FFC539323E544E4A0A80532240F58BA9903705DD7101389F8A3`。對應 owner packet 為 `C:\Users\archi\AppData\Local\Temp\technical_analysis_p0_audit\p0_owner_packet_continuation_live_host.md`；仍只重排 machine evidence，不自動接受來源。
- host context 另完成 Phase 3C 候選回補（`2026-08-24..2026-08-28`）：5/5 交易日、institutional／credit 10 個日 checkpoint 與 TDCC 最新週 snapshot 共 11/11 成功，TEMP candidate DB 共 `126,014` 列（institutional=`110,894`、credit=`11,073`、TDCC=`4,047`），所有 rows 仍為 `quality=degraded`，沒有寫入正式 SQLite。summary=`C:\Users\archi\AppData\Local\Temp\technical_analysis_p0_audit\backfill_reports\phase3c_backfill_summary_20260828_154340.json`（SHA-256=`0762A4A0C369FB0666878DD1747829725148B58527186D10B9284DED20A39146`），candidate DB SHA-256=`BA7A6D75119CE3DC8CEF0BEF0D916C5384FEAFF85E9C5411937E7C42D415EE93`。這是多日機器候選 evidence，不解除 publication/PIT、license、owner acceptance 或 downstream gate。
- host scheduler 產生正式 freshness 後的 current unified readiness：`C:\Users\archi\AppData\Local\Temp\technical_analysis_program_readiness\program_readiness_current_20260828.json`（status=`action_required`；SHA-256=`987BD522124F8A56A43A25AA7F7BC07F88183AF3D5C729C3D83694E66587BDFB`）；`freshness_projection=status=passed`、daily／technical latest=`2026-08-28`、`runtime=ready`、`update_history=ready`。Evidence 因目前未提供 owner-approved projection 仍為 `waiting_for_external_input`，其餘 P0／Paper／Formal／performance blockers 維持不變；artifact 只寫入 TEMP，未改正式資料或啟用 scheduler／broker。
...
- Technical production canary guarded entry：`scripts/qa_technical_indicator_production_canary.py`；預設只讀 production state，要求 owner token、無並行 writer acknowledgement 與 explicit confirm，並在建立 backup 前唯讀檢查預設 20 GiB headroom；不足時回報 `production_canary_storage_preflight_blocked`，不建立 backup／啟動 writer。容量通過後才會建立 SQLite online backup／單股 CSV backup，再執行 bounded parent-only writer；驗證失敗會嘗試 rollback。2026-08-28 尚未執行 production canary，因此 readiness 仍保留 `technical_production_single_writer_canary_not_completed`。
...
## MOPS EZSearch chunked retry candidate（2026-08-28）

前一輪整月 query 已確認 SII 的 `invalid_response` 主要是單一 query 觸及 MOPS 1,000 列上限，另有少量暫時網路錯誤；因此使用 `--query-window-days 7 --query-retries 2` 對同一 `2026-05-01..2026-05-31`、`sii,otc`、`F26–F29` 範圍重跑。40 個 query（每個 market／item 五段窗口）全部成功，得到 `7,473` events／`7,473` projections；duplicate、future、invalid、failed 均為 `0`，其中上市 `4,069`、上櫃 `3,404`，四項分布為 F26=`1,869`、F27=`1,869`、F28=`1,869`、F29=`1,866`。本輪沒有需要 retry 的窗口，但每段 manifest 都保留 query 起訖日，後續重試會另留 `attempt_count`／`retry_error_codes`。

候選 artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_chunked_retry\mops_statement_availability_host_20260828_chunked_retry.json`（SHA-256=`44F169A3D6CAD796E47262BD0F8C91A3637B8627AFFBFC631F1903E317D5D926`），candidate mapping=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_chunked_retry\mops_statement_availability_host_20260828_chunked_retry.csv`（SHA-256=`1C966610D7DF145DB14E470D875EC7A3D4B6354CD9745DFA63677C44D2B31022`）；`validate_statement_availability.py` 結果為 `valid=true`、accepted=`7,473`、diagnostics=`0`。這仍是 research candidate，不代表 numeric statement lineage、source acceptance、license／owner approval 或 Formal credit。

以該 mapping 對現有 `D:\Min\Python\Project\FA_Data\financial_data` 全量 raw rows 做 backfill dry-run，raw=`1,645,555`、normalized=`0`、diagnostics=`1,645,555`，主要診斷為既有歷史列沒有 explicit `available_date`；候選公告資料雖涵蓋少量 `2023-Q2..2025-Q4` 舊期申報與大量 `2026-Q1`，仍不足以替全量 raw universe 建立完整 mapping。此輪沒有寫正式 mapping、SQLite 或任何下游資料，且沒有把 backfill 失敗轉成成功。

## MOPS chunked candidate 接回 P0 audit（2026-08-28）

以上 `7,473` 筆 availability candidate 已在 host context 接回 P0 source-evidence audit，使用同一份 MOPS validator-normalized row list；audit 的 13 個 source row 均有 machine evidence，machine status=`1 verified / 12 degraded / 0 missing`，route summary=`13 observed / 14 not_attempted`。MOPS 季報列保留 validator 的 raw／accepted=`7,473 / 7,473`、quarantine／blocked=`0 / 0` 與 payload hash；這只證明該 candidate artifact 的 row conservation，不代表整個歷史 raw universe 已具備 available-date。

本輪 audit=`C:\Users\archi\AppData\Local\Temp\technical_analysis_p0_audit\p0_evidence_20260828_chunked_mops_host.json`（SHA-256=`4AABBC7C2CABCA487943BE563C7C9D5F530460DBBC4CFBB6C983442FD9A6A757`）。由此 audit 產生的 candidate intake=`C:\Users\archi\AppData\Local\Temp\technical_analysis_p0_audit\p0_candidate_intake_20260828_chunked_mops_host.json`（SHA-256=`8878DBAB0B446BF31C329AE32ADC77AC61F3075F85AD244CC5DAD2D7C6EF8CAA`），13 列 validator 仍為 `deferred`、owner-review-ready=`0`；Owner packet=`C:\Users\archi\AppData\Local\Temp\technical_analysis_p0_audit\p0_owner_packet_20260828_chunked_mops_host.md`（SHA-256=`A7C44F17035222675C482B5E897E9707C054E18C069B69F570C51E6AD6B0C183`）。三份檔案都在 TEMP，沒有寫正式 Registry、SQLite、mapping、scheduler 或 broker。

補齊既有 technical／broker baseline、host runtime／Registry clone、scheduler、freshness 與 Direct storage artifact 後，已用這份新 audit 重算 unified readiness：`C:\Users\archi\AppData\Local\Temp\technical_analysis_program_readiness\program_readiness_after_chunked_mops_host_full_20260828.json`（status=`action_required`；SHA-256=`56CE1B1D67B8CC1F25AC7B633E59A6ABA8B498FA871F4B290E1F954B090AF8F1`）。P0 仍為 `action_required`、Evidence=`weekly 0/3`／pending sidecar 仍待審核、Paper 週報仍因成本帳缺漏 `not_computable`、Formal input=`0/3`；Runtime／Update History 為 `ready`，performance 只保留 Direct storage 與 technical production single-writer canary blockers。這只是把最新 machine evidence 反映到聚合報告，不會用 availability candidate 直接回填或解除任何 gate；下一步仍是 owner／license／publication／PIT 複核與逐列 available-date mapping。

## 歷史 MOPS 2024-Q1 availability candidate 與 bounded backfill（2026-08-28）

為了確認現有 raw 財報不是只能靠單一補資料方法，本輪再以官方 MOPS EZSearch 查詢 `2024-05-01..2024-05-31`、`sii,otc`、`F26,F27,F28,F29`，同樣採 7 天窗口與最多 2 次 error-only retry。40/40 query 成功，取得 `6,933` 個 valid events／projections；另有 8 個歷史列缺少 `CTIME`，現在由 parser 逐列 quarantine、保留錯誤摘要與 bounded sample，artifact 整體標為 `degraded`（CLI return code=`3`），不會因少數壞列丟掉同一回應中的有效列。

artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_2024q1\mops_statement_availability_host_2024q1.json`（SHA-256=`84D906913853F2CC64A37EE2EC9EF2714CD0E1C07BF0E624FFC788F03E3E670F`），candidate mapping=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_2024q1\mops_statement_availability_host_2024q1.csv`（SHA-256=`6B9A4AA614A4114FF5C4F180A7CBDD01D41B4461D96208A7A44CD507301C51EB`）。mapping validator=`valid=true`、accepted=`6,933`、diagnostics=`0`；其中 `2024-Q1` projection=`6,821`，其餘為歷史修訂／跨期列，仍不能自行推導成完整 raw coverage。

以這份 mapping 對現有 `D:\Min\Python\Project\FA_Data\financial_data` 全量 raw rows 做唯讀 backfill dry-run，結果為 raw=`1,645,555`、normalized=`496`、diagnostics=`1,645,059`，且全部屬 `fundamental_availability.missing_available_date`。CLI 現在只顯示 20 筆 bounded diagnostics，另列出完整計數與 `missing_availability_count`；`ready_for_apply=false`，沒有寫入正式 availability mapping、`fundamental_statement_items` SQLite、Registry 或下游 factor。這個結果把缺口從「parser 失敗」縮小為「仍缺逐列公告／可得日 coverage」，後續可持續按歷史月份、雙市場與 statement type 分段取得並 reconciliation。

## 多窗口 MOPS candidate 累積與 raw 對應率（2024-03／04／05）

為驗證不同公告窗口可以累積而不是互相覆蓋，另完成兩個相鄰官方查詢：2024-03（主要對應 `2023-Q4`）40/40 query 成功，valid=`5,984`、invalid=`4`（同樣是缺 `CTIME`），validator accepted=`5,984`；artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_2023q4\mops_statement_availability_host_2023q4.json`（SHA-256=`2C371F4FB69E46039ED4928D5289032E1E702BC28A66F5347D2FC336C3E0BF6E`），mapping SHA-256=`BC23140D8F97EE808830ACE4BFD45517E0B18CFD14B7C319AFA7B9E0CFD83B96`。單獨 backfill dry-run normalized=`33,653`、missing availability=`1,611,902`。

2024-04（主要對應 `2024-Q1` 延後公告）40/40 query 成功，valid=`844`、invalid=`4`，validator accepted=`844`；artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_2024q1_april\mops_statement_availability_host_2024q1_april.json`（SHA-256=`916636C5DB430CDB31265BBB7B599B84D64470DCD969D61A59776292C3DDC032`），mapping SHA-256=`D047297B8E661F5D7405FE7D619236C52C15D995D6C0A1352E4ACFFB7B002490`。單獨 backfill dry-run normalized=`1,586`、missing availability=`1,643,969`。
...
## 歷史 MOPS coverage 擴充至 2022-Q3（2026-08-28）

沿用相同的雙市場、F26–F29、7 天窗口與 bounded retry，完成下列公告月份的唯讀 candidate：

- 2023-05（主要對應 `2023-Q1`）：40/40 query 有資料、`6,773` events／projections、invalid=`4`（缺 `CTIME`），validator accepted=`6,773`，單獨 backfill normalized=`38,594`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_2023q1\mops_statement_availability_host_2023q1.json`（SHA-256=`C49A756171E53AB3D52881B7882C116F2AC5DBAF517BD32CD7D9083552A511B5`），mapping SHA-256=`C6D12EE14221D2F9F97AB00EA29BD4DBEF85D135C2E2C51CFF6929FD23CB1E29`。
- 2023-08（主要對應 `2023-Q2`）：40/40 query 有資料、`7,096` events／projections、invalid=`0`，validator accepted=`7,096`，單獨 backfill normalized=`38,651`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_2023q2\mops_statement_availability_host_2023q2.json`（SHA-256=`1A5B207F0CD604D04FB6BA41724B69DCB0AE5E6387C2C9CAD899BA0E97261E0E`），mapping SHA-256=`05C4B3FE378A19D2E79D074DFB458CFBAE4AD776A92D75C7ABA22D1BCBE589A1`。
- 2023-11（主要對應 `2023-Q3`）：40/40 query 有資料、`6,997` events／projections、invalid=`0`，validator accepted=`6,997`，單獨 backfill normalized=`40,227`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2023q3\mops_statement_availability_host_2023q3.json`（SHA-256=`8EBF23E246BA6072DA62F58BC206E84B08E94C7198D3A6B4E75B17AFD490B5B9`），mapping SHA-256=`0E981497361D718E99277D5271F6385FFB27A253B4ED6E3AD7EFC941DF71B44E`。
- 2023-03（主要對應 `2022-Q4`）：40/40 query 有資料、`5,972` events／projections、invalid=`4`（缺 `CTIME`），validator accepted=`5,972`，單獨 backfill normalized=`34,056`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_2022q4\mops_statement_availability_host_2022q4.json`（SHA-256=`5FADF9EDC0FBB9F84BDFFDC302EF5BD0E347BA88A2F61FD704F1BE220A2FF975`），mapping SHA-256=`93CF9E0A5AF6CE7EF4EC14B240F2741E0554B1514AC56ED8209239E1C63B0BCD`。
...
## 歷史 MOPS coverage 擴充至 2021-Q4（2026-08-28）

2022-03（主要對應 `2021-Q4`）40/40 query 有資料，取得 `5,966` events／projections、invalid=`0`，validator accepted=`5,966`；單獨 backfill normalized=`34,131`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260828_2021q4\mops_statement_availability_host_2021q4.json`（SHA-256=`C9689E29731785305A89F9DDD4630BD6093F757C669D1A8DBE6C3EDB13F13F17`），mapping SHA-256=`A1B50B051B8D1F3DB10E7A08036ECD9823DD82417BC6DF9EAC3B508CFA8C47B6`。

加入既有 2022-Q1～2024-Q1 candidate 與 retroactive baseline 後，hybrid planner dry-run 為 raw／normalized=`1,645,555 / 1,645,555`、diagnostics=`0`、`availability_file_count=12`，quality=`observed 335,620`／`degraded 1,309,935`。這仍是 candidate coverage 增量，沒有正式 mapping／SQLite apply；下一段可重跑窗口為 2021-Q3（2021-11）。

## 歷史 MOPS coverage 擴充至 2021-Q3（2026-08-28）
...
## 歷史 MOPS coverage 擴充至 2021-Q1（2026-08-28）

2021-05（主要對應 `2021-Q1`）36 個 query 有資料、4 個 query 為官方 `status=fail` 無資料，取得 `6,349` events／projections、invalid=`0`，validator accepted=`6,349`；單獨 backfill normalized=`36,851`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2021q1\mops_statement_availability_host_2021q1.json`（SHA-256=`9D2289F0039EA863152AF342E44F33CC2417B4027AA9D126F7BDAD2A16852003`），mapping SHA-256=`F856B2B0835B41F66CBCEAAB3FCB7801EEB47EF5DA401CEA978DD0ACA866537C`。

加入既有 2021-Q2～2024-Q1 candidate 與 retroactive baseline 後，hybrid planner dry-run 為 raw／normalized=`1,645,555 / 1,645,555`、diagnostics=`0`、`availability_file_count=15`，quality=`observed 449,464`／`degraded 1,196,091`。這仍未寫正式 mapping／SQLite，也不解除 P0 acceptance、license／owner decision、PIT 或 Formal gate；下一段可重跑窗口為 2020-Q4（2021-03）。

## 歷史 MOPS coverage 擴充至 2020-Q4（2026-08-28）

2021-03（主要對應 `2020-Q4`）40/40 個 7 天 query 有資料，取得 `5,989` events／projections、invalid=`0`，validator accepted=`5,989`；單獨 backfill normalized=`34,838`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2020q4\mops_statement_availability_host_2020q4.json`（SHA-256=`D36980F9876D41A6E5EBC747E3886542FF9E65C40EB62AEBB23311B4C3DBCA1B`），mapping SHA-256=`E3C190B5C23CF646AB9869764797B1022A755DECD6CB29BD1CE97B9C8CFC841F`。

加入既有 2021-Q1～2024-Q1 candidate 與 retroactive baseline 後，hybrid planner dry-run 為 raw／normalized=`1,645,555 / 1,645,555`、diagnostics=`0`、`availability_file_count=16`，quality=`observed 484,302`／`degraded 1,161,253`。這仍未寫正式 mapping／SQLite，也不解除 P0 acceptance、license／owner decision、PIT 或 Formal gate；下一段可重跑窗口為 2020-Q3（2020-11）。

## 歷史 MOPS coverage 擴充至 2020-Q3（2026-08-28）

2020-11（主要對應 `2020-Q3`）40/40 個 7 天 query 有資料，取得 `6,373` events／projections、invalid=`0`，validator accepted=`6,373`；單獨 backfill normalized=`37,109`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2020q3\mops_statement_availability_host_2020q3.json`（SHA-256=`A5F471979E3E0669B7F4694A1EBD54FA84BF43F9460ABB5F86F5BD1234AB88A3`），mapping SHA-256=`03EC9F3E42E984ABB9E31EE129EB39033125B58058CE98EEAE741EA1F0D1C48A`。

加入既有 2020-Q4～2024-Q1 candidate 與 retroactive baseline 後，hybrid planner dry-run 為 raw／normalized=`1,645,555 / 1,645,555`、diagnostics=`0`、`availability_file_count=17`，quality=`observed 521,411`／`degraded 1,124,144`。這仍未寫正式 mapping／SQLite，也不解除 P0 acceptance、license／owner decision、PIT 或 Formal gate；下一段可重跑窗口為 2020-Q2（2020-08）。

## 歷史 MOPS coverage 擴充至 2020-Q2（2026-08-28）

2020-08（主要對應 `2020-Q2`）40/40 個 7 天 query 有資料，取得 `6,610` events／projections、invalid=`0`，validator accepted=`6,610`；單獨 backfill normalized=`37,671`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2020q2\mops_statement_availability_host_2020q2.json`（SHA-256=`44EED6503FBD93C76E54D71CD16DA85A77638C2835D65C6B4DD3380968C72B88`），mapping SHA-256=`250FF987FCB0DCF4054DC58DD806DADF5C813F0213831FC570F897A0A7AE6C29`。

加入既有 2020-Q3～2024-Q1 candidate 與 retroactive baseline 後，hybrid planner dry-run 為 raw／normalized=`1,645,555 / 1,645,555`、diagnostics=`0`、`availability_file_count=18`，quality=`observed 559,082`／`degraded 1,086,473`。這仍未寫正式 mapping／SQLite，也不解除 P0 acceptance、license／owner decision、PIT 或 Formal gate；下一段可重跑窗口為 2020-Q1（2020-05）。

## 歷史 MOPS coverage 擴充至 2020-Q1（2026-08-28）

2020-05（主要對應 `2020-Q1`）40/40 個 7 天 query 有資料，取得 `6,094` events／projections、invalid=`0`，validator accepted=`6,094`；單獨 backfill normalized=`35,560`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2020q1\mops_statement_availability_host_2020q1.json`（SHA-256=`7AAFD7CB0C9A130AA5771C9FC4DF963366D64B00B27AAE738E6D904C1EF37855`），mapping SHA-256=`C5C2EAB94E02E145446B5CCE10937436498993A3B8B2535FA896C410685A432A`。

加入既有 2020-Q2～2024-Q1 candidate 與 retroactive baseline 後，hybrid planner dry-run 為 raw／normalized=`1,645,555 / 1,645,555`、diagnostics=`0`、`availability_file_count=19`，quality=`observed 594,642`／`degraded 1,050,913`。這仍未寫正式 mapping／SQLite，也不解除 P0 acceptance、license／owner decision、PIT 或 Formal gate；下一段可重跑窗口為 2019-Q4（2020-03）。

## 歷史 MOPS coverage 擴充至 2019-Q4（2026-08-28）

2020-03（主要對應 `2019-Q4`）40/40 個 7 天 query 有資料，取得 `5,864` events／projections、invalid=`0`，validator accepted=`5,864`；單獨 backfill normalized=`36,343`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2019q4\mops_statement_availability_host_2019q4.json`（SHA-256=`8E77D19D22AC985837F67C37D5043D19F0ECC39DC9EF2986CA636C320CF41304`），mapping SHA-256=`A6F5AB9DA1AD7D9D66BC4AEBFC648521001BE9249014200E0C2DAF22C897306A`。

加入既有 2019-Q4～2024-Q1 candidate 與 retroactive baseline 後，hybrid planner dry-run 為 raw／normalized=`1,645,555 / 1,645,555`、diagnostics=`0`、`availability_file_count=20`，quality=`observed 630,985`／`degraded 1,014,570`。這仍未寫正式 mapping／SQLite，也不解除 P0 acceptance、license／owner decision、PIT 或 Formal gate；下一段可重跑窗口為 2019-Q3（2019-11）。

## 歷史 MOPS coverage 擴充至 2019-Q3（2026-08-28）

2019-11（主要對應 `2019-Q3`）40/40 個 7 天 query 有資料，取得 `6,444` events／projections、invalid=`0`，validator accepted=`6,444`；單獨 backfill normalized=`39,958`。artifact=`C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2019q3\mops_statement_availability_host_2019q3.json`（SHA-256=`EF17BA6EAF4E27D85D726B7B1F4D3DB3A2596B3A0FC1E3B5A971124986241F5F`），mapping SHA-256=`ECFC12D9C5928DC598D50B9099A0C98EE438BDB9FEAE909F8804FB031A4D49D7`。
...
## 歷史 MOPS coverage 擴充至 2016-Q4／Q3（2026-08-29）

沿同一個官方 MOPS EZSearch 路徑再往前取得兩個公告窗口，仍採雙市場／四個
statement item、7 天分段與最多 2 次 error-only retry：

- 2017-03（主要對應 `2016-Q4`）：40 個 query 有資料、40 個官方無資料、真正失敗=`0`，
  取得 `5,644` events／projections、invalid=`0`，artifact 狀態=`ready`；artifact=
  `C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2016q4_host\mops_statement_availability_host_2016q4.json`
  （SHA-256=`301EDA79A4629A306066A5E730277938B5510F190AF634446783D8821A026486`），mapping SHA-256=
  `9F2B2E8DA6AF22C6AD773986E52D8BFC50E3AF147FB8DD2CBC03B0DF7B87DE65`；validator `valid=true`、
  accepted=`5,644`、diagnostics=`0`。
- 2016-11（主要對應 `2016-Q3`）：36 個 query 有資料、44 個官方無資料、真正失敗=`0`，
  取得 `5,948` events／projections、invalid=`0`，artifact 狀態=`ready`；artifact=
  `C:\Users\archi\AppData\Local\Temp\technical_analysis_mops_statement_availability_20260829_2016q3_host\mops_statement_availability_host_2016q3.json`
  （SHA-256=`F81E442745C50D3D90ED7CF5F44E007A1AD672C6E95570BC0351B6ECF0A04D41`），mapping SHA-256=
  `E1CD1AA23DB53CAA06BF2AB497CE6171F9B48127B62249D0B17E2CBBE3A7D3EB`；validator `valid=true`、
  accepted=`5,948`、diagnostics=`0`。

將這兩份 candidate 與既有 31 份 candidate 及 retroactive baseline 一起做唯讀 hybrid
planner（共 `32` 份 candidate 加 1 份 baseline），結果為
`availability_file_count=33`、raw／normalized=`1,645,555 / 1,645,555`、
diagnostics／missing availability=`0`、quality=`observed 1,123,851`／`degraded 521,704`、
`ready_for_apply=true`。這是可追溯的官方公告 coverage 增量與完整 schema 合併證明，仍是
