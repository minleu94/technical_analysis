...
此 owner 決議只建立後續工程方向，不產生 snapshot、elapsed day、source acceptance、Formal OOS 或 promotion credit。既有 `2014–2026` Direct／OOC 留在 research/history；`formal_oos_allowed=false`、production alpha 0、broker disabled 與 append-only external Gate 規則不變。`PFS-01`～`PFS-08` 已完成 strict clock／fixture-only capture／prospective PIT custody／inference calibration policy／capture-only readiness／frozen activation contract／shadow maturity gate focused QA；下一個獨立 slice 是 `PFS-09` frozen-candidate Formal OOS replay 與 calibration／PSI，且不得啟動現行 heavy watcher 代替它。

## Forward Clock current projection（2026-08-06）

| Clock | 今日狀態 | 可引用證據 | 缺失／degraded／failure | 下一步 |
|---|---|---|---|---|
| Development | `DEV-119-foreground-rule-only-decision-producer-20260806` | 新增 owner foreground-only Rule-only decision producer：唯讀 `daily_prices`、嚴格取決策日前完整交易日、20 日 Decimal／整數 bp 排名、canonical SHA-256 lineage、既有 Rule Champion HMAC attestation 與 `manual_observed` constructor payload；focused contracts `30 passed` | 未在 automation 中執行 live capture；無正式 market DB／Recommendation／Portfolio／Exit／Score／lifecycle／evidence ledger 寫入，無 scheduler、training、promotion、unblind 或 blend；controlled runtime key 仍需 owner 本機設定；P0／ML raw PIT coverage blockers 未變 | owner 於綁定 session 的真實台北盤中手動執行新 CLI；若 source、時間或簽章前置不合格，保持 fail-closed 與 snapshot count=`0` |
| Formal evidence | `rule_only_20260807_snapshot_pending` | owner=`archi` 的 Rule-only lane `holdout_id=formal-rule-only-20260807-r1` 已以 registry v3 綁定第一個未使用 session=`2026-08-07`；新增 producer 只會使用 whitelist 子集 `daily_prices`，輸出含 hash-addressed lineage、HMAC-attested Rule Champion 與 `manual_observed` payload | 本次 snapshot count=`0`；outcome revision count=`0`；matured denominator／formal credit／consumption increment=`0`；missing=`manual_observed_snapshot_missing`；現時尚無真實盤中輸出；歷史 invalid artifact 不回填；formal readiness 仍固定 `false` | 只能在真實台北 09:00–13:30、同一綁定 session 手動產生 source，然後以既有 capture CLI append TEMP/shadow；不得改時間、replay、fixture、scheduled output 或同日 invocation 取得 elapsed day／credit |

截至本次 **2026-08-06** projection refresh，`DEV-119` 已補上真實盤中手動 Rule-only decision source 的缺口，但尚未也不得由 automation 代為產生 live snapshot；`DEV-102-datagov-history-target-detail-probe-20260805` 與 `DEV-105-ml-raw-publication-refresh-current-sqlite-20260805` 的既有證據仍適用；DEV-79 至 DEV-119 的工程安全約束仍全部適用：
