...
|---|---|---|---|---|
| Development | `rule_only_observation_lane_binding_hardened` | 新增 hash-addressed observation-lane contract、append-only v3 binding CLI 與唯讀 preflight；舊 v2 binding 不覆寫。owner=`archi` 決議 `decision:formal.rule_only_candidate_exclusion:20260728-r1`，content hash `sha256:961f425ceae517fe6ba96363657ae4608d4a78f76792453c2c92e2c26ead602c`；明確白名單僅 `daily_prices`、`industry_indices`、`market_indices`、`technical_indicators` | 富邦及所有尚未正式接受的 P0 candidate source 均從此 Formal lane 排除；富邦 shadow／候選計算仍可用，`fubon_shadow_usable=true`、`fubon_formal_credit_allowed=false` | 2026-07-29 真正 decision-time 才人工保存 Rule-only `manual_observed` artifact；不得預先建立、replay 或回填 |
...
截至本次 **2026-07-28** projection refresh，owner=`archi` 已核准新的 Rule-only／candidate-source-exclusion observation lane。決議 artifact serialized SHA-256 為 `BFB319F1129373B339F1A6CCD473A0F34DC833E2AF1CEF5A7559D477539C1072`；automation 在綁定前重新讀取 registry，確認 `2026-07-29` 未出現於任何既有 binding／consumption record，才 append `holdout-consumption-registry.v3` record。唯讀 preflight 現只回報 `manual_observed_snapshot_missing`，這是正確的等待狀態，不是人工審核 blocker。富邦依既有授權繼續供 decision-time／historical-PIT shadow 與 candidate Score／Recommendation／Portfolio／Exit 計算，但不進新 lane 的 typed source lineage；舊 2026-07-21 Fubon shadow snapshot 維持零 retroactive credit。V2.2 approved weekly history `3/3 ready` 不變；Formal Week 3 尚未自然結束，故不新增週報 revision；2026 年 7 月尚未自然結束，故未新增月度 Gate Review。
