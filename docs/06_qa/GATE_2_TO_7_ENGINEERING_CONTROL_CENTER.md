...
|---|---|---|---|---|
| Development | `DEV-104-ml-raw-publication-source-drift-diagnostic-20260805` | current configured SQLite `D:\Min\Python\Project\FA_Data\sqlite\twstock.db` SHA=`sha256:31f4ca4b8789805239884da22a6a36cbebf482d0726fcdcbc6f52e1842113414`，而 immutable raw publication `pit-413ff34c1d2c6cc9fdaa2849` 記錄的 source fingerprint 是 `sha256:23ad4fb019ac76d1eca0ea36a6b33e4d7719e7530a9bab1187d565a332480665`；size 同為 3,308,916,736 bytes，但 DB mtime 晚 341.041629 秒。query-only／immutable SQLite 下，11 symbols／2024–2026 policy 的 11 個 table scoped source row counts 與 raw manifest 全部相同；TEMP result SHA=`sha256:45cda32d7dd9a2705ff8b80beed685683d5d196dc3cd4ba01a7e38305989c004`、manifest SHA=`sha256:925e48f6855baf3dba01e5c99331bbc85eb178c850fcf750175a14f059cd051e`、self-check SHA=`sha256:f9dee8ed46187799f61ac484c30fd21d2708f54b304866c3787e12562b021090`，diagnostic checks PASS | fingerprint mismatch 是 custody failure；row count／date bounds 相同不足以證明 value equivalence，既有 raw publication 不可直接供 downstream assembly 或 training。這不是 numeric backfill，也不是 source acceptance；本次沒有寫 DB、沒有重跑 scheduler、沒有 materialize | 保留 DEV-104 TEMP drift report；numeric backfill rows=`0`、candidate delta=`0`、aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp`。下一步只能以 current DB 在 TEMP 建立新的 controlled raw PIT publication，再獨立驗證 hash／shards／lineage；GPU、fit、training、promotion 仍不在 scope |
...
22. **ML raw source-drift boundary** DEV-104 發現 current SQLite source fingerprint 與既有 immutable raw publication 的 recorded fingerprint 不同，雖 bounded source row counts／日期範圍相同，仍不能宣稱 value equivalence；current DB mtime 晚於 publication，既有 raw snapshot 不得進 downstream assembly 或 training。這只建立 source-freshness blocker，沒有回補正式 DB、沒有建立 candidate、沒有 materialize、沒有 scheduler rerun；下一步必須先在 TEMP 以 current DB 產生新的 controlled raw PIT publication 並重新做 custody 驗證。
