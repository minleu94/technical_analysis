...
|---|---|---|---|---|
| Development | `DEV-81-mops-numeric-pit-bounded-coverage-expansion-20260805` | DEV-80 的 11 個候選保持有效；本次前景 batch 嘗試 10 個 2025-Q1 上市候選，7 個取得並通過 immutable raw/candidate 驗證，3 個因 `doc.twse.com.tw` DNS failure fail-closed；累計 `candidate_count=18`、`eligible_rows=820`、`denominator=179,271`、`coverage=45 bp`、`gap=7,955 bp`；aggregate hash=`sha256:541766187370d381facac4d10d87d14d6cb3ea89a15a8fdbaec3173aca2c1e66` | 7 個新增候選只在 TEMP；1109、1110、1213 未產生可用候選；coverage 仍遠低於固定 8,000 bp，且 license、quality/PIT、具名 reviewer 與 applying registry revision 仍缺；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 依 Owner 授權繼續逐來源 bounded acquisition；保留 DNS diagnostics，不把重試或缺失數值轉成形式證據、不補值、不 materialize；下一批只有在新輸入／可取得 artifact 明確存在時才執行，仍不得寫正式 DB、feature、fit、training 或 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-81-mops-numeric-pit-bounded-coverage-expansion-20260805` 完成一個可獨立驗證的 research-only 回補增量；DEV-79 與 DEV-80 的工程安全約束仍全部適用：
1. **Bounded acquisition** 前景取得 7/10 個 2025-Q1 MOPS 數值／公告時間候選；1109、1110、1213 的 DNS failure 保留為 diagnostics，不以重試或補值掩蓋。
2. **Aggregate coverage** 18 個 immutable candidates 經 validator 聚合為 820 個 PIT-eligible rows、45 bp coverage；固定 8,000 bp 門檻仍未達成，未建立 feature materialization。
