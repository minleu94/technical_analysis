...
|---|---|---|---|---|
| Development | `DEV-100-datagov-history-targeted-filter-probe-20260805` | 以 DEV-96 官方 metadata 的 target `nid=94829`、標題 `上市公司資產負債表(金融業)` 建立新的 `fulltext` filter；用 wire-preserving client 依 module exact contract 執行一次 `POST /api/front/dataset/simple-list`，HTTP `200`、wire 88 bytes、SHA=`sha256:50db2ebc4928de9a3caea28868e062aff1ca2217e78e210b6a0771dc9c90f65c`，`search_count=0`、`search_result=[]`。request body SHA=`sha256:f6cd7f0c0576485cc79b505618abd2fea50814aee2df3044c7200a6811359d92`；result SHA=`sha256:a360666d5b604985e6d81e30723ee7f73b3a12dca5a05d454b33906792d6b9f5`、manifest SHA=`sha256:008b8585a97970d7c300206c70328cab4d341be55d353c98d139d6c4564e5991`；candidate／aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp` | exact title filter 的空結果只對該 filter 成立，不代表未查詢的 token、page 或 detail route 沒有歷史；本次沒有 numeric、announcement、available、revision 或 correction lineage。wire bytes 已保留，session cookie 僅在 metadata redacted；coverage、license／quality／rollback、具名 reviewer acceptance 與 dataset-specific lineage 仍缺失，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP request/response/result/manifest；不把 `search_count=0` 當成全域歷史不存在、不補值、不建立 candidate、不寫正式 DB／feature／fit／training；下一步 fresh preflight 後使用另一個 module-supported fulltext token 或明確 detail route 做一次 wire-preserving probe，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-100-datagov-history-targeted-filter-probe-20260805` 完成一個可獨立驗證的 research-only official dataset-filter capability probe；DEV-79 至 DEV-99 的工程安全約束仍全部適用：
