...
|---|---|---|---|---|
| Development | `DEV-97-datagov-historical-index-capability-probe-20260805` | 對上一輪從 dataset page 發現的官方 `https://data.gov.tw/datasets/history` 執行一個 bounded read-only GET：HTTP `200`、1,008,456 bytes、raw SHA=`sha256:15d9c5c691cdb3e71554422a3c442294bd35fff8021e3f62cdc73dacd2abad4e`。server-rendered 內容是通用歷史資料篩選 UI（含開始／結束日期與提供機關欄位）並顯示 `No data`；沒有 `dataset_id=94829`、目標財報標題、歷史 record、版本／resource link 或 dataset-specific detail route；result SHA=`sha256:3c09b5888ad6744c234d22d2729625d6b23ec834c5b1ee2b3ecdaccfd9c1af51`、manifest SHA=`sha256:910163ed7629031f202784cdf50d9c1a22295ac1a861f6fa3abd30d40430f249`；沒有下載 numeric payload、未建立 candidate，cumulative aggregate 維持 `candidate_count=47`、`matching_decision_row_count=5,363`、`eligible_rows=1,987`、`denominator=179,271`、`coverage=110 bp`、`gap=7,890 bp` | 這只能證明本次抓到的歷史索引頁沒有 server-rendered dataset-specific result，不能宣稱後端或其他日期篩選結果不存在；沒有歷史 bytes、announcement／available_at、revision 或 correction lineage，仍不是 2025-Q1 PIT artifact。coverage、license／quality／rollback 與具名 reviewer acceptance 仍缺失，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP raw index/result/manifest；不把 `No data` 或目前 CSV 當歷史、不補值、不建立 candidate、不 materialize 或寫正式 DB／feature／fit／training；下一步只接受直接可引用的 dataset-specific history/detail response、歷史檔案／公告可得 sidecar 或 owner 授權 PIT export，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-97-datagov-historical-index-capability-probe-20260805` 完成一個可獨立驗證的 research-only official history-index capability probe；DEV-79 至 DEV-96 的工程安全約束仍全部適用：
