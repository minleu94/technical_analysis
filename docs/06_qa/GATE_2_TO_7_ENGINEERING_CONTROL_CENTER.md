...
## Current Forward Clock projection（2026-08-28）

| Clock | 今日狀態 | 可引用證據 | 缺失／degraded／failure | 下一步 |
|---|---|---|---|---|
| Development | `DEV-138-20260828-activation-lineage-projection-refresh / blocked_zero_partial` | create-only `clock:prospective:20260828:v1` 的 clock file hash=`sha256:85d50c270d9a10098e4e99d4531b0b3ce96eb096797977ab64349bc164c90161`；既有官方 TWSE／TPEX raw hashes 分別為 `sha256:9e6f88f30420552e435951193dc44094d726412c8d917d8a975581d8ae083c97`／`sha256:4f055c3c035d84c75b299211f36ef2dbe01b700c58c2115aeb6630c7628ce835`，PIT staging 維持 `1,932/1,932`。單次 runner 於 `2026-08-28T09:01:27+08:00` 唯讀確認 2026-08-27 T-1=`1,961` rows 後執行一次，因 strict readiness status contract 缺陷在 publish 前 fail closed；Rule／Portfolio／PIT manifests=`0/3`、strict readiness 不存在、`.activation_staging` 已清除，沒有 partial publication。修正已由 commit `44cb70a` 保存；本次只做唯讀 lineage 驗證與 sanitized projection refresh，沒有重跑 runner | failure=`staged strict readiness did not reach ready after all three manifests`；三個 producer 的當次 staged payload 雖各自通過 validator，消耗的一次性 clock 不可因修正而重跑或手動發布。受控三個 `BALDR_ML_FORMAL_*` path 仍指向較舊的 `clock-20260819` 且檔案缺失，正式 readiness=`0/3`；readiness 不會自動 discovery／升格 `clock-20260828`。當前 clock 已過 activation boundary，唯讀 clock inspector 因無 same-day pre-open override 維持 blocked | 等待新的可引用 owner 決議建立仍在未來的 create-only clock，並由 owner-controlled formal publication 在真實 boundary 自然產生三份 input；不得重跑 8/28 one-shot、改日期、補發布、回填、把 prospective wrapper 改名，或啟動 watcher、Direct／OOC、training、promotion、broker |
| Formal evidence | `formal-rule-only-20260807-r1 / zero_credit` | owner decision 與四筆 consumption-registry binding 維持有效且未消耗；formal session=`2026-08-07`，snapshot=`missing`，唯一 blocker=`manual_observed_snapshot_missing`，`formal_readiness=false`。2026-08-28 prospective `manual_observed` 經唯讀檢查為 structurally valid，但屬另一個 Development clock/session，對本 lane 明確回報 `manual_observed_session_mismatch` | 本次 snapshot count、outcome revision、matured denominator、Formal credit、consumption 與 elapsed-day increment 全部為 `0`；missing=`genuine decision-time Rule-only manual_observed artifact for 2026-08-07`；degraded=`none for the latest bound Rule-only lane`；capture failure=`2026-08-28 prospective artifact belongs to a different session and cannot be reused`。歷史 `manual_observed_20260729.json` 亦不重用、不回填 | 不重綁或消耗 2026-08-07，不把 prospective clock、one-shot、staging／dry-run、development invocation、replay、fixture 或事後審核換算為 observed day。新 holdout 仍須先有可引用 owner 決議時間 artifact，且綁定前重新檢查 registry |

截至 **2026-08-28 14:33（Asia/Taipei）**，Formal Week 7（2026-08-24 至 2026-08-30）仍在自然進行中，因此不追加提前或第二份 weekly revision；Formal Week 1～6 各維持唯一 revision。August 尚未完成，因此不新增當月 Gate Review。8/28 one-shot 已消耗且 fail closed，修正後仍不重跑；其 prospective artifact 與 isolated dry-run 都只屬 Development clock，不產生 Formal observed day 或 credit。安全旗標維持 `formal_oos_allowed=false`、`formal_evidence_credit_authorized=false`、`production_blend_alpha_bp=0`、Rule-only Formal path、`promotion_eligible=false`、`broker_order_allowed=false`。
