...
## Forward Clock current projection（2026-07-28）

| Clock | 今日狀態 | 可引用證據 | 缺失／degraded／failure | 下一步 |
|---|---|---|---|---|
| Development | `development_ml_data_inventory_and_experiment_runner_added` | DEV-70 完成欄位級 Development Data Inventory CLI (`inspect_development_ml_data_inventory.py`) 與隔離 ML 實驗 CLI (`run_development_ml_experiment.py`)，盤點 20 個 Core 特徵、4 個標籤、MOPS 秒級發布時間軸 (Availability Gate)、Broker/Fundamental 族群與 5 個 TWSE 微觀結構源；支援 Feature Pack (`core_20`, `technical_only`, `price_only`, `price_technical`)、模型家族與超參數 Grid 實驗，且輸出皆位於 TEMP 根目錄 | 欄位級別明確標示 `included_in_current_fit`, `included_evaluation_only`, `candidate_research_only`, `excluded_by_policy`, `blocked_missing_artifact`, `blocked_pit_contract`；MOPS 標為 Availability Gate (`is_numeric_model_feature=false`)，不偽造財務比率特徵；所有實驗結論限定於 `research_observation`/`continue_shadow`/`insufficient_evidence`；正式白名單維持固定，`formal_oos_allowed=false`，`production_blend_alpha_bp=0` | 可建立並執行新隔離 Development Experiments (Feature Ablation, Model Comparison, Grid)，等待真實 2026-07-29 決策時點進行 Formal Rule-only 觀測 |
| Formal evidence | `rule_only_lane_bound_waiting_for_20260729_observation` | `holdout_id=formal-rule-only-20260729-r1` 已在 owner 決議後綁定首個未使用完整交易時段 `2026-07-29`；registry SHA-256 `7972DD62E2175022E24DF6FAE4F80CD85F47BFF8704548E1A8BBEA3B7B52CF09`；preflight=`owner_attested_lane_binding_valid`；既有 2026-07-21 shadow snapshot 與 Formal Week 2 revision 保持不變 | 本次 snapshot count=`0`；既有 shadow `manual_observed` count=`1`；outcome revision count=`0`；matured denominator increment=`0`；formal credit increment=`0`；missing=`2026-07-29 decision-time Rule-only manual_observed artifact`；degraded=`none for the new lane`；capture failure=`none in this invocation` | 等待真實 2026-07-29 決策時點；通過 source whitelist、timestamp 與 session equality 後才可追加 shadow artifact，仍不自動取得 Formal credit |

截至本次 **2026-07-28** projection refresh，DEV-70 已完成開發階段欄位級 Data Inventory 與 ML Experiment Runner。現有 20 個 Core Features 與 4 個 Core Labels 清晰劃分訓練狀態；MOPS 發布時間軸標為 Availability Gate 絕不偽造數值財報；DEV-69 TWSE 微觀結構源與 Broker/Fundamental 顯式說明候選遮蔽原因而非模糊禁止。開發者可透過 `run_development_ml_experiment.py` 在 TEMP 中執行純開發 ML 實驗，結果不影響 Formal/Production 路徑。owner=`archi` 之前已核准新的 Rule-only／candidate-source-exclusion observation lane，`2026-07-29` 已綁定首個未使用完整交易時段。唯讀 preflight 現只回報 `manual_observed_snapshot_missing`，這是正確等待狀態。富邦依既有授權繼續供 shadow 計算，但不進 Formal credit。V2.2 approved weekly history `3/3 ready` 不變；Formal Week 3 尚未自然結束，故不新增週報 revision；2026 年 7 月尚未自然結束，故未新增月度 Gate Review。
