...
|---|---|---|---|---|
| Development | `DEV-107-ml-sidecar-custody-lineage-audit-20260805` | 唯讀核對 current official-market publication 與既有 sidecar 綁定 revision：current run manifest=`sha256:c7883ae5928f12b2bc6ec569f3ae5434c9fe74d0e552ed1b9b8681d4835ab321`、canonical=`sha256:4fe0eb31abc835e5a56a73650816e41d5d1485d3c55141d44de8a6f1970e3418`，既有 sidecar 綁定舊 manifest=`sha256:d9011aadde6902b04481252ffad445604366c576d0b9840a59e528492de2c69b`、canonical=`sha256:40436b3cc4ae639cbb6d3165800ec0dd168b3dd8575138bd04c3764644634d3f`；causal ledger／portfolio training 仍 `training_as_of=2026-07-30T08:30:00+08:00` 且未對齊 DEV-105 current TEMP raw；sector membership hash/count=`zero/0`、portfolio ledger=`absent`（cash-only fallback）。TEMP result=`sha256:85ee5a9dc7290b4f613893b8948cf91a60051ab4f828bce64fe6e739439f69a9`、manifest=`sha256:3ba19e28bbdf3bf2e5787b2c43cac1c1297064b0356234f6bef670d7572f0642`、self-check=`sha256:6b8813859e710df15b10c20127bb8f41a12fe8ba25d285871376a57be847f859`；`checks=24`、validation PASS | current event revision 與 sidecar binding 不一致；causal／portfolio sidecar stale、sector zero、ledger absent；scheduled shadow input／evidence 為 machine background，`manual_observed`／Formal 不成立 | 全部仍為 TEMP research-only；numeric backfill rows=`0`、candidate／aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp`；未寫正式 DB、未 materialize、未 fit／training／promotion、未重跑 scheduler、GPU 不在 scope。下一個合格 increment 只能處理 owner/reviewer/source-acceptance/applying revision 與 current raw／event publication 對齊；若輸入 hash 不變則 no_op |
