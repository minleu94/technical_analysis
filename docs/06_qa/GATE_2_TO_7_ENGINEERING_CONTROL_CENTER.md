...
|---|---|---|---|---|
| Development | `DEV-105-ml-raw-publication-refresh-current-sqlite-20260805` | current configured SQLite `D:\Min\Python\Project\FA_Data\sqlite\twstock.db` file SHA=`sha256:31f4ca4b8789805239884da22a6a36cbebf482d0726fcdcbc6f52e1842113414`（file identity only）；以 exporter 同一公式重算 logical source fingerprint=`sha256:08ea4743b54e82da5a3d343cf73de9a15dafa5f76c1224933315ae55e2669160`，既有 publication `pit-413ff34c1d2c6cc9fdaa2849` 記錄=`sha256:23ad4fb019ac76d1eca0ea36a6b33e4d7719e7530a9bab1187d565a332480665`；eligibility hash 與 size=`3,308,916,736` bytes 相同，mtime 晚 341.041629 秒。以 current DB 在 TEMP 建立 fresh publication `pit-1b53e73127a5246d3255ee95`：9 shards、186,710 raw rows；`all_field_enriched`／`core_long_history` 各 52 features／67,902 rows／424,932 values／3 shards，`research_shadow_all_fields` 32 features／50,906 rows／379,675 values／3 shards。publication manifest hash=`sha256:11429185f8ef08a61618beb059b81a3330a4c26bdf840a92d28708f693c25717`、file SHA=`sha256:8cbb0a4d0fd3a846472700fef38698d6d9fe2ca0185a4c85a30d312f9c7e2ac`；TEMP validation result=`sha256:8f1b647efbc86f02383c3b7dd4b713102006c91276436b0c85b1239a4c8d27a5`、manifest=`sha256:b9db3c527f1b94a1332cd34ff502134e1d87747357a14c493cc81c959a0e24e3`、self-check=`sha256:3412a403b6e3a0a4ecf24da5dc8a5bde108b4e7ede54e46da7e88990987fda32`；query-only／ro／production_action_allowed=false | custody、manifest／compressed-content hash、row/value/mask/schema/conservation 與 lineage checks PASS。formal-labelled raw datasets 各有 15,211 missing、421,512 stale values 且沒有 announced／first_seen／effective／revision；shadow 有 14,348 quality-blocked values、21 announced、14,377 first_seen／revision rows，全部 formal_training_eligible=false。相較舊 publication，formal rows 各增加 15,029、values 各增加 92,603，shadow rows／values 增加 14,117；這是 raw snapshot refresh，不是 formal numeric backfill、source acceptance 或 direct training input | 保留 DEV-105 TEMP publication；numeric backfill rows=`0`、candidate delta=`0`、aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp`。下一步只能做獨立 delta／as-of decision-row assembly blocker audit，仍不寫正式 DB、不 materialize、不 fit／training、不 promotion；GPU 不在 scope |
...
截至本次 **2026-08-04** projection refresh，`DEV-102-datagov-history-target-detail-probe-20260805` 的 official target-detail capability probe 與 `DEV-105-ml-raw-publication-refresh-current-sqlite-20260805` 的 current SQLite TEMP raw PIT refresh 均已完成；DEV-79 至 DEV-101 的工程安全約束仍全部適用：
