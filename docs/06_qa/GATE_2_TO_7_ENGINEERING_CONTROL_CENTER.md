...
|---|---|---|---|---|
| Development | `DEV-92-twse-quarterly-ratio-alternate-source-capability-probe-20260805` | 針對 DEV-91 缺口身份以官方 TWSE OpenAPI `t187ap14_L` 做五請求能力探測；baseline 與 3 個 query 變體均回傳同一份 82 rows、期間 `115-Q2`，未命中 2025-Q1 目標；`roc_q1` 變體 read timeout；未產生 candidate，cumulative aggregate 維持 `candidate_count=47`、`matching_decision_row_count=5,363`、`eligible_rows=1,987`、`denominator=179,271`、`coverage=110 bp`、`gap=7,890 bp`；probe result SHA=`sha256:245d2983d408664a7bb7440b14a5f9e702d96614e09843793fc0583bb13e0a7d` | 官方替代端點目前只可觀測最新 `115-Q2`，歷史 query 未改變內容且一個變體 timeout；沒有可引用的 2025-Q1 數值或公告日，不能補值、不能把最新值當 PIT；coverage 仍低於固定 8,000 bp，license、quality/PIT、具名 reviewer 與 applying registry revision 仍缺；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP raw/result 與 DEV-91 failure diagnostics；不重試相同 query、不建立 candidate、不 materialize 或寫正式 DB／feature／fit／training；下一步只接受新的可追溯官方歷史 artifact 或授權 PIT 匯出，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-92-twse-quarterly-ratio-alternate-source-capability-probe-20260805` 完成一個可獨立驗證的 research-only alternate-source blocker probe；DEV-79 至 DEV-91 的工程安全約束仍全部適用：
