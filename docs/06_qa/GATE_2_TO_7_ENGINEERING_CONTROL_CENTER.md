...
|---|---|---|---|---|
| Development | `DEV-85-mops-numeric-pit-bounded-coverage-expansion-20260805` | DEV-84 的 26 個候選保持有效；本批新嘗試 1225、1227、1229、1231、1232 五個 2025-Q1 上市身份，5/5 取得並通過 immutable raw/source/candidate/PIT lineage 驗證；累計 `candidate_count=31`、`matching_decision_row_count=3,630`、`eligible_rows=1,389`、`denominator=179,271`、`coverage=77 bp`、`gap=7,923 bp`；aggregate hash=`sha256:63aed9d8b5d6188366019ccc18aa7aa4a5c92583c87d4302964b65e0a8ee270f` | 本批無新的 fetch failure；DEV-84 的四個 timeout／DNS diagnostics 仍保留且未重跑；coverage 仍遠低於固定 8,000 bp，且 license、quality/PIT、具名 reviewer 與 applying registry revision 仍缺；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留五個新 candidate／manifest hashes 與 cumulative aggregate；不把 research candidate materialize 或送入正式 DB／feature／fit／training；下一批只選新的未嘗試身份或新的可驗證官方 artifact，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-85-mops-numeric-pit-bounded-coverage-expansion-20260805` 完成一個可獨立驗證的 research-only numeric PIT acquisition 增量；DEV-79 至 DEV-84 的工程安全約束仍全部適用：
1. **Bounded numeric acquisition** 新嘗試 1225、1227、1229、1231、1232；5/5 通過 official MOPS numeric／listing raw hash 與 PIT lineage 驗證，沒有猜測或補值。
2. **Aggregate coverage increased but remains blocked** 31 個 immutable candidates 聚合為 1,389 個 PIT-eligible rows、77 bp coverage；固定 8,000 bp 門檻仍未達成，未建立 feature materialization。
