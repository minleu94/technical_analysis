...
|---|---|---|---|---|
| Development | `DEV-90-mops-numeric-pit-bounded-coverage-expansion-20260805` | DEV-89 的 47 個候選保持有效；本批新嘗試 1409、1410、1413、1414、1416 五個 2025-Q1 上市身份，0/5 取得 candidate bundle，未產生新候選；重算 cumulative aggregate 仍為 `candidate_count=47`、`matching_decision_row_count=5,363`、`eligible_rows=1,987`、`denominator=179,271`、`coverage=110 bp`、`gap=7,890 bp`；aggregate hash=`sha256:b63ef5b52c0009bf72fa4af757127e3e9b633acb2aa6039ac61566441366e583` | 1409、1410、1416 `doc.twse.com.tw` DNS resolution failure；1413 listing row 不符合唯一 consolidated report contract；1414 MOPS numeric request read timeout；未補值、未重試既有 failure、未產生 revision；coverage 仍遠低於固定 8,000 bp，且 license、quality/PIT、具名 reviewer 與 applying registry revision 仍缺；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 DEV-90 五筆 failure diagnostics 與 unchanged cumulative aggregate；不把 research candidate materialize 或送入正式 DB／feature／fit／training；下一批只選新的未嘗試身份或新的可驗證官方 artifact，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-90-mops-numeric-pit-bounded-coverage-expansion-20260805` 完成一個可獨立驗證的 research-only numeric PIT acquisition 增量；DEV-79 至 DEV-89 的工程安全約束仍全部適用：
1. **Bounded numeric acquisition** 新嘗試 1409、1410、1413、1414、1416；0/5 產生 candidate bundle，DNS、listing contract 與 timeout 均 fail-closed，沒有猜測或補值。
