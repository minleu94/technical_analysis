...
|---|---|---|---|---|
| Development | `DEV-101-datagov-history-short-token-probe-20260805` | 以官方 module 支援的新 `fulltext` token `資產負債表` 執行一次 wire-preserving `POST /api/front/dataset/simple-list`，HTTP `200`、wire 1,514 bytes、SHA=`sha256:28b1521348c5c3fff2aa046937bedbc9ec353935b74e688b4d0793a4ee8998c8`，`search_count=4`、4 rows；返回 nids `166165, 24175, 117702, 152394`，target `nid=94829` 與 target title 均命中 0。request body SHA=`sha256:637181805b81ef0fda9da7d7c5f98d87177e76f0d841f181bfe2b78906ece921`；result SHA=`sha256:c4f1ad68acfb8dfd166f66248145d3cdf1ac01179d4d2ca977c60f7d80522397`、manifest SHA=`sha256:4d3893ce97968d1e4d1fec40d5d9bb0bd711aa26b129b4668ae19c50de006d3c`；candidate／aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp` | token search 回傳四筆與 target 無關的歷史索引；這不是全域歷史否定，也沒有 numeric、announcement、available、revision 或 correction lineage。wire bytes 已保留，session cookie 僅在 metadata redacted；coverage、license／quality／rollback、具名 reviewer acceptance 與 dataset-specific lineage 仍缺失，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP request/response/result/manifest；不把 fulltext 命中或 `unpubdate` 當 PIT、不補值、不建立 candidate、不寫正式 DB／feature／fit／training；下一步 fresh preflight 後測試明確 nid-oriented filter 或 dataset detail route，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-101-datagov-history-short-token-probe-20260805` 完成一個可獨立驗證的 research-only official short-token capability probe；DEV-79 至 DEV-100 的工程安全約束仍全部適用：
