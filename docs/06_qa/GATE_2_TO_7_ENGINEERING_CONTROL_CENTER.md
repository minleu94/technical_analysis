...
|---|---|---|---|---|
| Development | `DEV-79-mops-engineering-fail-closed-hardening` | `resolve_mops_numeric_pit_source_mapping`、aggregate conservation/hash validator、TEMP-root containment、atomic aggregate/package publication、registered applying-revision gate、candidate lineage selection 與 hash-addressed resume 已完成獨立 hardening；4 份既有 immutable candidates 仍為 `candidate_count=4`、`eligible_rows=174`、`denominator=179,271`、`coverage=9 bp`、`gap=7,991 bp` | 原 Gemini 版本可偽造 aggregate coverage、未綁 registry revision 即 materialize、resume 前先 fetch、run_id 可逃逸 TEMP root；本次已 fail-closed 修正。P0/P13 source acceptance、coverage、license 與 owner/reviewer 決議仍未完成；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 僅可在 TEMP 以前景 bounded batch 累積候選；先取得 P0/P13 人工決議與可引用 evidence，再由獨立 revision 重新驗證；不得寫正式 DB、feature、fit、training 或 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-79-mops-engineering-fail-closed-hardening` 只完成可獨立驗證的工程安全增量：
1. **Aggregate contract** 強制 source identity、canonical hash、candidate hash、coverage count conservation 與固定 8,000 bp policy；不接受呼叫端自報 coverage。
2. **Materialization gate** 必須同時具備合約有效 aggregate、具名 owner/reviewer dossier 與 append-only decision registry 的 applying revision；revision、evidence、timestamp、rollback 與 use case 不一致即拒絕。
3. **TEMP containment** 驗證 run_id 與 candidate output 皆在 development root，aggregate/readiness package 以 staging + atomic rename 發布。
4. **Foreground resume** 僅在既有 candidate hash 與 manifest 預期 hash 完全相同時 skip；缺 hash 或衝突均在 fetch 前 fail，revision 另行處理。
5. **Overlay lineage** 只接受 aggregate 已列出的 candidate hash，並按 decision date 選取最新適用 available date；formal／production flag 固定關閉。

以上工程 hardening 不等於 coverage 達標、不等於授權/License 核准、更不等於 Formal credit。P0/P13 的 source acceptance 仍等待 Owner 決議；Formal lane 仍缺真實 decision-time Rule-only `manual_observed` artifact，本次 snapshot、elapsed formal day、outcome revision、matured denominator、formal credit 與 consumption increments 均為 0。
