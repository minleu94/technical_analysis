...
|---|---|---|---|---|
| Development | `DEV-79-mops-engineering-fail-closed-hardening` | `resolve_mops_numeric_pit_source_mapping`、aggregate conservation/hash validator、TEMP-root containment、atomic aggregate/package publication、registered applying-revision gate、candidate lineage selection 與 hash-addressed resume 已完成獨立 hardening；Owner 已於 `owner:p0-p13.research-acquisition:20260805-r1` 授權全 13 個 lane 在實際可取得時做 bounded research acquisition；4 份既有 immutable candidates 仍為 `candidate_count=4`、`eligible_rows=174`、`denominator=179,271`、`coverage=9 bp`、`gap=7,991 bp` | 原 Gemini 版本可偽造 aggregate coverage、未綁 registry revision 即 materialize、resume 前先 fetch、run_id 可逃逸 TEMP root；本次已 fail-closed 修正。Owner 授權不等於逐來源 applying acceptance；license、quality/PIT、coverage、具名 reviewer 與相符 registry revision 仍缺；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 依 Owner 授權僅在 TEMP 以前景 bounded batch 取得實際存在的資料；公告／availability 與 numeric source 分離，抓不到的數值 quarantine／排除且不得補值；完成逐來源 evidence 後才由獨立 revision 重新驗證；不得寫正式 DB、feature、fit、training 或 promotion |
