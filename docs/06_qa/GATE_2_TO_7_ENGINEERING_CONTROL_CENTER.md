...
|---|---|---|---|---|
| Development | `DEV-88-mops-numeric-pit-bounded-coverage-expansion-20260805` | DEV-87 的 40 個候選保持有效；本批新嘗試 1324、1325、1326、1337、1338 五個 2025-Q1 上市身份，3/5 取得並通過 immutable raw/source/candidate/PIT lineage 驗證；累計 `candidate_count=43`、`matching_decision_row_count=4,927`、`eligible_rows=1,823`、`denominator=179,271`、`coverage=101 bp`、`gap=7,899 bp`；aggregate hash=`sha256:6a11844e445a460af0d1aee6b95942215db34fb18b785e077c887f6c2874147f` | 1326 `doc.twse.com.tw` DNS resolution failure；1337 MOPS numeric request timeout；兩者均未產生 candidate bundle、未補值；1256 與 DEV-84 的其他 timeout／DNS diagnostics 仍保留且未重跑；coverage 仍遠低於固定 8,000 bp，且 license、quality/PIT、具名 reviewer 與 applying registry revision 仍缺；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留三個新 candidate hashes、兩個 failure diagnostics 與 cumulative aggregate；不把 research candidate materialize 或送入正式 DB／feature／fit／training；下一批只選新的未嘗試身份或新的可驗證官方 artifact，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-88-mops-numeric-pit-bounded-coverage-expansion-20260805` 完成一個可獨立驗證的 research-only numeric PIT acquisition 增量；DEV-79 至 DEV-87 的工程安全約束仍全部適用：
1. **Bounded numeric acquisition** 新嘗試 1324、1325、1326、1337、1338；3/5 通過 official MOPS numeric／listing raw hash 與 PIT lineage 驗證，1326 DNS failure、1337 timeout fail-closed，沒有猜測或補值。
2. **Aggregate coverage increased but remains blocked** 43 個 immutable candidates 聚合為 1,823 個 PIT-eligible rows、101 bp coverage；固定 8,000 bp 門檻仍未達成，未建立 feature materialization。
