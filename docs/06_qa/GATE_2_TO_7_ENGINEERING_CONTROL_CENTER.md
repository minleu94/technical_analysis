...
|---|---|---|---|---|
| Development | `DEV-95-twse-openapi-historical-financial-statements-capability-probe-20260805` | 以官方 TWSE OpenAPI catalog（`200`、306,043 bytes、SHA=`sha256:2c2cecccb7a220ac9e263228a7659aa49b1ada5aea397650e601ad3dfcc48043`）及 `mopsfin.twse.com.tw/opendata` 三個新 CSV endpoint 執行四個 read-only GET；一般業損益表與資產負債表各 82 rows，兩者是 `ROC 115-Q2`（出表日期 `1150805`）；金融業損益表只有 1 筆空內容列（`年度`／`季別`／`公司代號` 均空，僅有 `出表日期=1150805`），五個 2025-Q1 目標 match 均為 0；probe result SHA=`sha256:69a303c93e82fa45a2ae63f7821a6dfe377baeebd8dbba1ece786d5f2d152f05`、manifest SHA=`sha256:0679e383971d63f2e55fe4c2efa87730003e1e1e41453c705fe93b94850b68aa`；未建立 candidate，cumulative aggregate 維持 `candidate_count=47`、`matching_decision_row_count=5,363`、`eligible_rows=1,987`、`denominator=179,271`、`coverage=110 bp`、`gap=7,890 bp` | 這些官方 CSV 端點目前只呈現最新季度／最新出表批次，且金融業端點本次為空內容列；catalog 未提供可用的歷史 period 參數。`1150805` 只能是本次出表欄位，不能倒推 2025-Q1 announcement／available、revision 或 correction。沒有目標 2025-Q1 numeric row，不能以最新值代替 PIT；coverage、license／quality／rollback 與具名 reviewer acceptance 仍缺失，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP raw catalog／CSV/result 作 official capability evidence；不把最新季度或空內容列當歷史、不建立 candidate、不 materialize 或寫正式 DB／feature／fit／training；下一步只接受可追溯的歷史檔案／公告可得 sidecar 或 owner 授權 PIT export，再走 validator 與 acceptance gate，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-95-twse-openapi-historical-financial-statements-capability-probe-20260805` 完成一個可獨立驗證的 research-only official capability probe；DEV-79 至 DEV-94 的工程安全約束仍全部適用：
