...
| Formal evidence | `capture_failed_deferred` | EV1 manual CLI 僅接受明確 `manual_observed` artifact，且只可寫 TEMP／shadow sidecar；DEV-42 preflight 只檢查 owner decision、registry 與既有 snapshot 契約 | snapshot count=`0`；missing=`manual_observed decision artifact`、`HoldoutConsumptionRegistry.jsonl`；degraded=`none`；capture failure=`no decision-time snapshot supplied` | 先由具名 owner 建立並授權 holdout 綁定，再於真實決策當下保存 observed snapshot；禁止歷史回填、synthetic day、replay／fixture credit 與 pending denominator |
