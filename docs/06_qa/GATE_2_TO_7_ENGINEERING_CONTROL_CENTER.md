...
## Current Forward Clock projection（2026-08-29）

| Clock | 今日狀態 | 可引用證據 | 缺失／degraded／failure | 下一步 |
|---|---|---|---|---|
| Development | `DEV-139-weekly-sidecar-lineage-projection-refresh-20260829 / validated_pending_zero_credit` | 正式 weekly sidecar 以 SQLite URI `mode=ro` 重驗為 9 筆 append-only records，最新 `ewc_0cde0f7a8154a7a1` 自然涵蓋 `2026-08-24..2026-08-28`，last trading date=`2026-08-28`、status=`pending_human_review`；sidecar hash=`sha256:897b38a32e0f7cf2d4cf3a9cf822df0bb7f18305495b7683f6c3c0761fc6913e`，同期間 report hash=`sha256:49bbe6ecf08a313731edaceb3f75537f8945705fdc1c1fe341730b87bdbfefb0`。9-row owner-review packet hash=`sha256:65aef55d21911cf00ab2273aa1a1893bb9feabde9f498f8857d1565b21215d4e`，固定 `candidate_only=true`、`write_performed=false`、`formal_credit_authorized=false`、`production_scheduler_allowed=false`；既有具名 approved projection 維持 `3/3`、hash=`sha256:02eb67ff812c17f604c34f4eb30c1b87941596af6aa9ebaff6ec1d555b64b4d8` | pending sidecar=`9`，全部仍需具名 owner／reviewer；pending 不進 approved weekly count、Formal snapshot、matured denominator、elapsed day 或 credit。8/28 prospective one-shot 的 `0/3` formal publication 與不可重跑邊界維持 DEV-138 結論，本次沒有執行 collector、scheduler、producer、publisher 或 one-shot | 由具名 owner／reviewer 逐期審核 pending rows；只有另行產生可引用的 approved weekly projection 才能更新 weekly UI disclosure，且仍不得授予 Formal credit。新的 prospective clock 仍須未來 owner 決議與真實 boundary，不得重跑或補發布 8/28 one-shot |
