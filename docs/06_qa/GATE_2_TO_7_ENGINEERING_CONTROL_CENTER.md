...
|---|---|---|---|---|
| Development | `DEV-115-ml-universe-pit-research-backfill-20260805` | current ML raw PIT 11-symbol scope 中尚無 candidate 的 `1216,2412,2881,6505` 前景 bounded MOPS batch；2412／6505 取得 immutable candidate／run-manifest SHA=`sha256:ca2e8ae8f5058d42057137d9a345513ad474a299e5dcba357214ad08181de2b3`／`sha256:2aadcb77aadcab853ba4ea900949165a5b05e2e9f415c610b2f7a95c3b3dd7a1` 與 `sha256:2aee8cca363b914ac8c5738ca84f21329109b363b05a1c3e003fac2766718b0c`／`sha256:2b829c5cf2b6b63cf45a1663ad71352ed8c5f48f6d22434427514054bf3ae6a7`；50 candidates aggregate=`2,128 / 179,271 = 118 bp`、aggregate SHA=`sha256:f8760ca81e1151d968e6d1a5ee95c4cd350102c49739e18467e96ae32dbfae79` | 1216=`doc.twse.com.tw` DNS fail-closed；2881=document listing 無 financial-report rows；coverage gap=`7,882 bp`；無 source acceptance／applying revision、無正式 DB／feature materialization；current raw publication 未改寫、`direct_training_input=false`、無 snapshot／registry／training／promotion／unblind／blend | 下一個 bounded increment 必須使用新的未嘗試官方 artifact 或新增 source/reviewer evidence；不可重試 1216／2881 的未變輸入；Formal 仍只等待 genuine current-session `manual_observed`，禁止歷史回填、replay、fixture 或 invocation credit |
| Formal evidence | `rule_only_20260807_snapshot_pending` | owner=`archi` 的新 Rule-only lane `holdout_id=formal-rule-only-20260807-r1` 於台北 `2026-08-07T05:15:10.321523+08:00` 決議並以 registry v3 綁定第一個未使用 session=`2026-08-07`；lane content SHA=`sha256:a6937b96e1ea00ea9d254350c0e99e692684395596cd93fd40d8eba39c3d620b`；whitelist 僅 `daily_prices`、`industry_indices`、`market_indices`、`technical_indicators` | 本次 snapshot count=`0`；outcome revision count=`0`；matured denominator/formal credit/consumption increment=`0`；missing=`manual_observed_snapshot_missing`；現場只找到 scheduled recommendation，不能作為 manual observed；歷史 2026-07-29 artifact=`invalid`（`manual_observed_decision_output_lineage_missing`），不回填；degraded=`none`；capture failure=`no genuine current-session Rule-only decision output exists yet` | 今日 session 僅可由真實當下 Rule-only decision output 產生含 hash-addressed lineage 且 whitelist 完全符合的 `manual_observed`；preflight=`can_capture_shadow_snapshot=true` 後才 append TEMP/shadow snapshot；不得回填、重綁、將 scheduled output 或同日 invocation 換算 elapsed day／credit |

截至本次 **2026-08-06** projection refresh，`DEV-102-datagov-history-target-detail-probe-20260805` 的 official target-detail capability probe 與 `DEV-105-ml-raw-publication-refresh-current-sqlite-20260805` 的 current SQLite TEMP raw PIT refresh 均已完成；DEV-79 至 DEV-115 的工程安全約束仍全部適用：
...
以上工程 hardening 不等於 coverage 達標、不等於來源方授權／License 核准、更不等於 Formal credit。Owner 已將工作正式納入 development plan 並授權 bounded research acquisition，但 P0/P13 的逐來源 applying source acceptance 仍等待可引用 license、quality/PIT、coverage、rollback 與具名 reviewer evidence；最新 aggregate 為 118 bp／8,000 bp（gap 7,882 bp）。Formal lane 仍缺真實 decision-time Rule-only `manual_observed` artifact，本次 snapshot、elapsed formal day、outcome revision、matured denominator、formal credit 與 consumption increments 均為 0。
