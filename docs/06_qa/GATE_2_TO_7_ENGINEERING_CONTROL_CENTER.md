...
|---|---|---|---|---|
| Development | `DEV-94-finmind-quarterly-financials-capability-probe-20260805` | 以 FinMind `TaiwanStockFinancialStatements` API 對 1240、1259、1264、1268、1336 執行五個無 token GET；5/5 API 成功，取得 2025-03-31 共 78 筆 numeric rows（18、16、16、14、14），欄位只有 `date`、`stock_id`、`type`、`value`、`origin_name`，未含 announcement／available／revision／correction；未建立 candidate，cumulative aggregate 維持 `candidate_count=47`、`matching_decision_row_count=5,363`、`eligible_rows=1,987`、`denominator=179,271`、`coverage=110 bp`、`gap=7,890 bp`；probe result SHA=`sha256:35efc57fee1a6e329fc15285eee0d6fb1317ac38ed0f4b2489ee963ea65a2720` | FinMind 可提供缺口公司的 2025-Q1 數值，但 `date=2025-03-31` 是期間日期，不足以證明 decision-time availability；第三方來源也尚無具名 license／quality／revision／rollback 與 owner/reviewer acceptance，不能把 numeric row 冒充 PIT、不能推定公告日或補正式資料；coverage 仍低於固定 8,000 bp，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP raw JSON/result 作 numeric-only research evidence；不以期間日期推定公告／可得日、不建立 candidate、不 materialize 或寫正式 DB／feature／fit／training；下一步需同一來源的可追溯 announcement/available artifact 或 owner 授權的交叉 lineage，再走 validator 與 acceptance gate，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-94-finmind-quarterly-financials-capability-probe-20260805` 完成一個可獨立驗證的 research-only numeric-only source capability probe；DEV-79 至 DEV-93 的工程安全約束仍全部適用：
