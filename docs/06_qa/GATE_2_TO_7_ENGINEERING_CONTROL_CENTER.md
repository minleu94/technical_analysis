...
|---|---|---|---|---|
| Development | `DEV-106-ml-raw-delta-asof-assembly-blocker-audit-20260805` | 以 current SQLite file SHA=`sha256:31f4ca4b8789805239884da22a6a36cbebf482d0726fcdcbc6f52e1842113414` 對 TEMP publication `pit-413ff34c1d2c6cc9fdaa2849` → `pit-1b53e73127a5246d3255ee95` 做 canonical identity／payload／value／source hash delta audit；formal datasets 各 `52,873→67,902` rows（新增 15,029、changed 5,346、value +92,603），shadow `36,789→50,906` rows（新增 14,117、皆為 fundamentals、無 changed payload）。TEMP result=`sha256:c2eaa45ba4839fc2c1f85531c79fc23aef73b624d01d9c74222a0123d1744946`、manifest=`sha256:0f3c44f538a90d57eabb088ab946cbd1f869e8c21f0263159cc929a16a2dfa1e`、self-check=`sha256:f87d7417b8ee8c4561cc3146477399e70ee0feed90ebb8904d4c861cac39ed68`；`checks=78`、validation PASS | T-1 potential pairs=`6,798`、eligible decision dates=`618`、available_at failures=`0`、future-feature failures=`0`；feature masks missing=`13,757`、stale=`276,914`、quality-blocked=`0`；Unicode-safe `daily_prices.收盤價` label audit：h5 `6,752/6,754` matured/row、h10 `6,697/6,699`、h20 `6,587/6,589`、h60 `6,147/6,149`（maturity `9,932/9,851/9,689/9,042 bp`）；purged folds=`5`（min train 252／test 63／purge 60／embargo 5）。sidecar custody：sector membership、corporate-action manifest、causal portfolio ledger 均未 bound；assembler 未 invoked、`direct_training_input=false`、`formal_consumer_compatible=false`。前一版 label 0 為 PowerShell 中文 literal encoding mismatch，已在 audit metadata correction 中保留且不代表 source/publication 變更 | 全部仍為 TEMP research-only；numeric backfill rows=`0`、candidate delta=`0`、aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp`；未寫正式 DB、未 materialize、未 fit／training／promotion、未重跑 scheduler，GPU 不在 scope。下一個合格 increment 只能處理 owner/reviewer-bound sector／corporate-action／causal-ledger sidecar 或其明確 blocker；若輸入 hash 不變則 no_op |
