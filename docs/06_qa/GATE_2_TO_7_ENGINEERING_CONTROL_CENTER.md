...
|---|---|---|---|---|
| Development | `development_observed_degraded` | Dataset `terra-development-v0:terra-v0-canonical-2025-dev-20260714`；Research run `sha256:36e57e0c370ce0f08eb625de0871ffa49699bfdb469272f4c5d7bacaf8d910c5`；projection SHA-256 `06BED1ABF3222EA1CA35DF8A25EDB8950025FA2314B4493A1AAEFEB18CA3614E`；DEV-19 Phase3C working-copy type-safety | fit rows=`179271`；evaluation rows=`0`；OOF samples=`53559`；Phase3C candidate path 不再嘗試設定不存在的 config 屬性，且只有已建立的顯式 working-copy manager 可寫入；13 項中仍僅 3 項有 candidate adapter；不是 source acceptance、formal OOS／promotion／投資有效性證據 | 僅在明確 apply 的既有 development working-copy DB 寫入；本 automation 未執行 apply；2026 matured outcomes 僅 evaluation，不得 fit |
