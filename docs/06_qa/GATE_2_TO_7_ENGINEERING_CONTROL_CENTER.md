...
|---|---|---|---|---|
| Development | `twse_microstructure_timestamp_semantics_hardened` | DEV-69 已在既有 P0 evidence audit 中拆分停復牌、處置、分盤、全額交割與漲跌停鎖死的公告／有效／session／first-observed／capture 語義，並新增五個逐 source sanitized machine recommendation | 五源目前仍全為 `deferred`、`ready_for_owner_review=false`；limit-lock 正確標成 `market_session_observation`／`decision_time_availability_not_proven`，不再錯列公告時間缺失；HTTP/capture/first-observed 不得升格；正式白名單仍只含 `daily_prices`、`industry_indices`、`market_indices`、`technical_indicators` | 等待新的 source-specific machine evidence 與 legal/owner review；Formal lane 另等待 2026-07-29 真實 decision-time snapshot |
...
截至本次 **2026-07-28** projection refresh，DEV-69 已補強既有 P0 evidence audit 的五個 TWSE microstructure timestamp semantics 與逐 source owner packet，但沒有執行 live probe，也沒有替 owner/reviewer 作決議。停復牌、處置與分盤仍缺可引用 row-level 公告時間；全額交割與漲跌停鎖死只證明交易日狀態／行情觀測，且 decision-time availability 尚未證明。owner=`archi` 之前已核准新的 Rule-only／candidate-source-exclusion observation lane；決議 artifact serialized SHA-256 為 `BFB319F1129373B339F1A6CCD473A0F34DC833E2AF1CEF5A7559D477539C1072`，`2026-07-29` 已綁定首個未使用完整交易時段。唯讀 preflight 現只回報 `manual_observed_snapshot_missing`，這是正確等待狀態。富邦依既有授權繼續供 shadow 計算，但不進 Formal credit。V2.2 approved weekly history `3/3 ready` 不變；Formal Week 3 尚未自然結束，故不新增週報 revision；2026 年 7 月尚未自然結束，故未新增月度 Gate Review。
