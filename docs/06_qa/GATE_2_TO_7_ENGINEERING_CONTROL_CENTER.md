...
|---|---|---|---|---|
| Development | `DEV-89-mops-numeric-pit-bounded-coverage-expansion-20260805` | DEV-88 的 43 個候選保持有效；本批新嘗試 1339、1340、1341、1342、1402 五個 2025-Q1 上市身份，4/5 取得並通過 immutable raw/source/candidate/PIT lineage 驗證；累計 `candidate_count=47`、`matching_decision_row_count=5,363`、`eligible_rows=1,987`、`denominator=179,271`、`coverage=110 bp`、`gap=7,890 bp`；aggregate hash=`sha256:bf70217f64f80428acb0940d3b06607df88b44b4539035c2149db7b1db58bf69` | 1342 `doc.twse.com.tw` DNS resolution failure，未產生 candidate bundle、未補值；1326、1337、1256 與 DEV-84 的其他 timeout／DNS diagnostics 仍保留且未重跑；coverage 仍遠低於固定 8,000 bp，且 license、quality/PIT、具名 reviewer 與 applying registry revision 仍缺；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留四個新 candidate hashes、一個 failure diagnostic 與 cumulative aggregate；不把 research candidate materialize 或送入正式 DB／feature／fit／training；下一批只選新的未嘗試身份或新的可驗證官方 artifact，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-89-mops-numeric-pit-bounded-coverage-expansion-20260805` 完成一個可獨立驗證的 research-only numeric PIT acquisition 增量；DEV-79 至 DEV-88 的工程安全約束仍全部適用：
1. **Bounded numeric acquisition** 新嘗試 1339、1340、1341、1342、1402；4/5 通過 official MOPS numeric／listing raw hash 與 PIT lineage 驗證，1342 DNS failure fail-closed，沒有猜測或補值。
2. **Aggregate coverage increased but remains blocked** 47 個 immutable candidates 聚合為 1,987 個 PIT-eligible rows、110 bp coverage；固定 8,000 bp 門檻仍未達成，未建立 feature materialization。
