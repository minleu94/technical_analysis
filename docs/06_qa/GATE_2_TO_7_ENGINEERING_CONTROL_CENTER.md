...
|---|---|---|---|---|
| Development | `DEV-84-mops-numeric-pit-bounded-coverage-expansion-20260805` | DEV-83 的 25 個候選保持有效；本批新嘗試 1215、1217、1218、1219、1220 五個 2025-Q1 上市身份，1215 取得並通過 immutable raw/source/candidate/PIT lineage 驗證；累計 `candidate_count=26`、`matching_decision_row_count=3,095`、`eligible_rows=1,186`、`denominator=179,271`、`coverage=66 bp`、`gap=7,934 bp`；aggregate hash=`sha256:336f2cd61adce6cbe347a62a26fd7a6c0e5273a7479be0195bd6679f87b18a5d` | 1217 MOPS numeric request timeout；1218 `doc.twse.com.tw` DNS failure；1219 t57 listing timeout；1220 MOPS request timeout；四者未產生 candidate bundle，未補值；coverage 仍遠低於固定 8,000 bp，且 license、quality/PIT、具名 reviewer 與 applying registry revision 仍缺；`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留四個新 failure diagnostics 與 1215 candidate hashes；不重跑相同輸入、不把 timeout／DNS 當成缺值、不 materialize；下一批只選新的未嘗試身份或新的可驗證官方 artifact，仍不得寫正式 DB、feature、fit、training 或 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-84-mops-numeric-pit-bounded-coverage-expansion-20260805` 完成一個可獨立驗證的 research-only numeric PIT acquisition 增量；DEV-79 至 DEV-83 的工程安全約束仍全部適用：
1. **Bounded numeric acquisition** 新嘗試 1215、1217、1218、1219、1220；只有 1215 通過 official MOPS numeric／listing raw hash 與 PIT lineage 驗證，四個 timeout／DNS failure fail-closed，沒有猜測或補值。
2. **Aggregate coverage increased but remains blocked** 26 個 immutable candidates 聚合為 1,186 個 PIT-eligible rows、66 bp coverage；固定 8,000 bp 門檻仍未達成，未建立 feature materialization。
