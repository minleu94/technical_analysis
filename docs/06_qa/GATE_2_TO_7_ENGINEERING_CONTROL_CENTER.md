...
截至本次 **2026-08-05** projection refresh，`DEV-102-datagov-history-target-detail-probe-20260805` 的 official target-detail capability probe 與 `DEV-105-ml-raw-publication-refresh-current-sqlite-20260805` 的 current SQLite TEMP raw PIT refresh 均已完成；DEV-79 至 DEV-115 的工程安全約束仍全部適用：
