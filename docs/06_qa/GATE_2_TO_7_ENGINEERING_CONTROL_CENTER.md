...
|---|---|---|---|---|
| Development | `DEV-93-twse-eshop-financial-history-capability-probe-20260805` | 以官方 TWSE Data E-Shop 公開 MOPS financial-statements package 頁面做三請求 capability probe；中文 package page `200` 明列 `S02-2` 合併財報、`S03`、`S18` 等項目，公開 MOPS index `200` 明列 Financial Statements、Historical、Subscription 與價格條款；英文 package page 本次 read timeout；未取得任何財報數值、公告日或 PIT artifact，cumulative aggregate 維持 `candidate_count=47`、`matching_decision_row_count=5,363`、`eligible_rows=1,987`、`denominator=179,271`、`coverage=110 bp`、`gap=7,890 bp`；probe result SHA=`sha256:06c7fb1c47881ac0f4d1d39cdd2dee665c802e715c39bc50aef30f25082376e2` | 官方頁面證明存在可訂閱的歷史財報產品，但資料交付受訂閱／授權限制；本次沒有 credentialed download、沒有可引用的 2025-Q1 數值、公告／可得時間、revision 或 correction status，不能補值或宣稱 source acceptance；coverage 仍低於固定 8,000 bp，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP public-page raw/result；不購買、不登入、不重試英文 timeout、不建立 candidate、不 materialize 或寫正式 DB／feature／fit／training；下一步只能由 owner 提供授權歷史匯出／合約 artifact，再走 validator 與 acceptance gate，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-93-twse-eshop-financial-history-capability-probe-20260805` 完成一個可獨立驗證的 research-only paid-source capability blocker probe；DEV-79 至 DEV-92 的工程安全約束仍全部適用：
