...
|---|---|---|---|---|
| Development | `DEV-102-datagov-history-target-detail-probe-20260805` | 依官方 module 明確宣告的 `GET /api/front/dataset/detail?nid=94829` 執行一次 wire-preserving target detail request，HTTP `200`、wire 18,847 bytes、SHA=`sha256:994cc911b181d91d891c3e432d6c97f030c5f983fcf7cec93d62262e3a8a3288`；payload identity=`nid 94829`，current dataset metadata、64 data fields、1 current CSV resource（`vid=15679012`、last update `2026-06-30`），`modify_history=[]`、`history_note=""`、numeric rows=`0`。request SHA=`sha256:24b1f496a8621207a287ed97cdb6d10b21e2c582589b5bcd70f411e9bfef3a5e`；result SHA=`sha256:bcbfba0fff715a0ef0b00f90c6b78e6cd1b8fcda5cf190f4fda794eaeb2eb3c3`、manifest SHA=`sha256:01eb9e7f839465727be2eadcc7a2132755f72dcea0ddf858bfe2579a11e00578`；candidate／aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp` | target detail 是 current metadata/resource schema，不是歷史版本；`pubdate`、`changed`、`metadata_changed` 與 resource `last_update_time` 不是 announcement／available_at，沒有 revision、correction、歷史 bytes 或 numeric payload。wire bytes 已保留，session cookie 僅在 metadata redacted；coverage、license／quality／rollback、具名 reviewer acceptance 與 dataset-specific PIT lineage 仍缺失，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP request/response/result/manifest；不下載 current CSV、不把 metadata timestamp 當 PIT、不補值、不建立 candidate、不寫正式 DB／feature／fit／training；下一步只等待新可歸屬的歷史/version route 或授權 PIT artifact，仍不得 promotion |
