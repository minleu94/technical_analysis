...
|---|---|---|---|---|
| Development | `DEV-99-datagov-history-api-module-probe-20260805` | 官方 `C_Q17jct.js` module HTTP `200`、3,227 bytes、SHA=`sha256:74588e7aca5b7f2e0f8924e66b4f210ce07c9515b232ab8294b5dbe47bec8f45`；程式明確把 history page 對應至 `POST /api/front/dataset/simple-list`。依頁面 exact body 執行一次 bounded read-only request，HTTP `200`、`search_count=36,144`、本頁 10 rows；target `nid=94829` 不在該請求的第一頁，row fields 只有 `nid`、title、agency、status、`unpubdate`，沒有 numeric、announcement、available、revision 或 correction lineage。result SHA=`sha256:384a75eb42e816f23cab29df1213a292363d0e191c9b648fb04ed6c1d43dcad1`、manifest SHA=`sha256:2f2c179dd0d3d091eb52e347fd9a75e63cddccb862d3eac4b615fa00da274044`；candidate／aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp` | response `Content-Length=3,726` 與 client re-encoded capture `4,671` 不一致，故只作結構 capability evidence，不宣稱 wire-exact raw 或 PIT；第一頁缺少 target 不代表未查詢頁面為空。coverage、license／quality／rollback、具名 reviewer acceptance 與 dataset-specific lineage 仍缺失，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP module/response/result/manifest；不把 `unpubdate` 當公告／可得日、不補值、不建立 candidate、不寫正式 DB／feature／fit／training；下一步 fresh preflight 後以 module 支援的 dataset-specific filter 或 detail route 做一次 wire-preserving probe，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-99-datagov-history-api-module-probe-20260805` 完成一個可獨立驗證的 research-only official API-module capability probe；DEV-79 至 DEV-98 的工程安全約束仍全部適用：
