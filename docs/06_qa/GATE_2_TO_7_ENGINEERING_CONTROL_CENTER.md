...
|---|---|---|---|---|
| Development | `mops_daily_research_freshness_diagnostics_implemented` | 已完成 MOPS EZSearch F26–F29 季報發布時間每日人工 capture、freshness/outage 診斷、prior revision 比對與 Research Console sanitized projection (DEV-68)；接續既有 Rule-only observation lane contract 與 2026-07-29 session binding | MOPS 本任務狀態仍為 limited research use，不得 promotion；正式白名單維持 `daily_prices`、`industry_indices`、`market_indices`、`technical_indicators`；富邦 shadow 可用 `fubon_shadow_usable=true`、`fubon_formal_credit_allowed=false`；真實 multi-day MOPS evidence 尚未成熟，同日重跑不計日數 | 等待 2026-07-29 自然交易時段人工保存 Rule-only `manual_observed` snapshot |
...
截至本次 **2026-07-28** projection refresh，DEV-68 已完成 MOPS EZSearch 季報發布時間每日人工 capture、freshness/outage 診斷、prior revision 比對與 Research Console sanitized projection。owner=`archi` 之前已核准新的 Rule-only／candidate-source-exclusion observation lane。決議 artifact serialized SHA-256 為 `BFB319F1129373B339F1A6CCD473A0F34DC833E2AF1CEF5A7559D477539C1072`；`2026-07-29` 已綁定首個未使用完整交易時段。唯讀 preflight 現只回報 `manual_observed_snapshot_missing`，這是正確的等待狀態。富邦依既有授權繼續供 shadow 計算，但不進 Formal credit。V2.2 approved weekly history `3/3 ready` 不變；Formal Week 3 尚未自然結束，故不新增週報 revision；2026 年 7 月尚未自然結束，故未新增月度 Gate Review。
