...
| Formal evidence | `binding_valid_waiting_for_decision_time` | `HoldoutConsumptionRegistry.jsonl` v2 binding（owner=`archi`；development holdout start=`2026-07-15`；formal trading session=`2026-07-21`；SHA-256 `9B606C35438B5153DBE2968E4E1A96CBF8694D39AC78865F61C6C512781DB00B`）；EV1 manual CLI 僅接受明確 `manual_observed` artifact，且只可寫 TEMP／shadow sidecar | snapshot count=`0`；missing=`manual_observed decision artifact`、`source acceptance owner review`；degraded=`none`；capture failure=`none`（尚未到合格 decision-time） | 在 `2026-07-21` 真實 Rule-only 決策時點，以當時可得資料建立一份 manual_observed shadow snapshot；再先跑 preflight，僅在所有結構檢查通過時 append TEMP evidence；禁止歷史回填、synthetic day、replay／fixture credit 與 pending denominator |

本日尚未建立 **Formal observed day**；snapshot count=`0`，missing=`manual_observed decision artifact` 與 `source acceptance owner review`，degraded=`none`，capture failure=`none`（尚未到合格 decision-time）。Development artifacts 只從外部 development root 唯讀驗證，未掃描或寫入正式 market DB，也未啟用專案 scheduler。B 方案 owner 決議 artifact 已存在於 `C:\Temp\technical_analysis_development_output\governance\DevelopmentDataUsageDecision.jsonl`（file SHA-256 `88D853DE58791F2D8482FE2C8D61013E0556518D22C6F52ED168FE863AE14B7B`；effective `2026-07-14T05:07:25.0061485Z`），且 owner `archi` 已在決策後 append `C:\Temp\technical_analysis_development_output\governance\HoldoutConsumptionRegistry.jsonl` 的 v2 binding：`development_holdout_start=2026-07-15`、`formal_trading_session=2026-07-21`、`unconsumed_before_binding=true`。此 binding 只鎖定未來真實時段，並不等同已觀測、已消費、formal readiness 或 formal credit。週報、人工 why-not 與月 Gate Review 僅在自然週／月到期時 append；第 12 週即使 eligible 也不得 unblind 或啟動 Task 6。
