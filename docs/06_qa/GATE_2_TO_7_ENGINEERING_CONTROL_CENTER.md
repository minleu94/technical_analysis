...
|---|---|---|---|---|
| Development | `mops_quarterly_publication_timestamp_source_limited_research_ready` | Dataset 與既有 sanitized Research Console projection 未變；live read-only MOPS EZSearch F26–F29 capture 產生 36 個官方秒級事件與 36 個唯一 availability keys，artifact SHA-256 `838A7507655934646D5118E3111118679D15DC935E5FA1E4372F8F060DCD602E`，mapping SHA-256 `E621D19863BE40641EBDA7F9A6F83A5B78243D2CDA2D90E459D2EFA0DDBE838F`；validator `accepted_count=36`、diagnostics=`0`；owner=`archi` 的 limited decision `decision:mops.ezsearch.statement_publication:20260727-r1` content hash `sha256:520aa2d2e56e497a658659816677e541b095c63689dcbb36f2a4a1a2b179f930` | `pit.quarterly_financials` 的 `candidate_artifact_not_supplied`、`mops_candidate_artifact_not_supplied` 與 blanket `official publication timestamp missing` blocker 已解除；歷史完整 coverage、修訂／更補正監測、正式 ingestion 及 Formal credit 未核准；M31 因混合「預計召開董事會」與「已通過財報」語意，不作財報發布證據 | 將同一 adapter 接到日常更新的 research/TEMP projection freshness 檢查；累積多日 schema／revision／outage evidence，再另行審核 formal source lane |
...
截至本次 **2026-07-27** projection refresh，MOPS 公告快易查已由新 adapter 對 `F26` 資產負債表、`F27` 綜合損益表、`F28` 現金流量表、`F29` 權益變動表逐市場唯讀查詢；本次 live capture 涵蓋上市 12、上櫃 24 個事件，時間範圍為 `2026-07-27T14:32:47+08:00..18:17:06+08:00`，event hash 與 projection key 均為 36/36 唯一。date-only mapping 採官方 `announcement_at` 的次一曆日為 `available_date`，避免同日盤中 look-ahead；具官方 timestamp 的延後公告不再被固定 120 天窗口誤殺。owner=`archi` 已明確核准此來源供內部 research PIT availability 與 development shadow projection，決議狀態為 `limited`、禁止再散布及正式 ingestion。這項決議不改寫 Formal Week 2：既有 2026-07-21 TEMP/shadow snapshot 仍引用未接受的 Fubon candidate，故 `formal_readiness=false`、本次 snapshot/outcome/denominator/credit/consumption 增量均為 0。週三 review `3/3 ready` 維持，2026 年 7 月尚未自然結束，故未新增月度 Gate Review。
