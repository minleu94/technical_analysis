...
|---|---|---|---|---|
| Development | `DEV-96-datagov-historical-resource-metadata-capability-probe-20260805` | 對官方 data.gov.tw dataset `94829` 執行三個 bounded read-only GET：`/api/v2/rest/dataset/94829`=`200`、6,577 bytes、SHA=`sha256:dd38cf20be91ab2fd99b90914c0ad79dcb86d186b5fdfdf475512c3cd8f87c65`；`/api/rest/dataset/94829`=`404`；資料集頁=`200`、547,999 bytes、SHA=`sha256:2e204032dee3775c67008f6c59a7dd257c4fbb1a2a260d7d7697c14159d8373b`。metadata 只列一個目前 `mopsfin.twse.com.tw/opendata/t187ap07_L_basi.csv`、每 3 月更新、免費授權；頁面 record `modify_history=[]`、`history_note=""`，僅發現通用 `/datasets/history` 連結（本 increment 已用完三個 GET，未再抓取）；result SHA=`sha256:dcfc219eaddc8d31c63f62ac5461a7510b7f7c38f190a73bea9b96503e2c037d`、manifest SHA=`sha256:ab4cb3a9ad48c6b2630b2f18d426ad09a8e15bdb3f52b00047312eaa36abf554`；沒有下載 numeric payload、未建立 candidate，cumulative aggregate 維持 `candidate_count=47`、`matching_decision_row_count=5,363`、`eligible_rows=1,987`、`denominator=179,271`、`coverage=110 bp`、`gap=7,890 bp` | 這是 metadata-only/current-resource evidence，不是 2025-Q1 PIT artifact：沒有版本化歷史下載、公告／available_at、revision 或 correction lineage；通用歷史專區連結尚未形成 dataset-specific evidence。coverage、license／quality／rollback 與具名 reviewer acceptance 仍缺失，`downstream_eligibility=none`、`formal_acceptance_applied=false`、`materialization_ready=false` | 保留 TEMP raw metadata/page/record/result/manifest；不把目前 CSV 當歷史、不補值、不建立 candidate、不 materialize 或寫正式 DB／feature／fit／training；下一步只接受可引用的 dataset-specific history index、歷史檔案／公告可得 sidecar 或 owner 授權 PIT export，再走 validator 與 acceptance gate，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-96-datagov-historical-resource-metadata-capability-probe-20260805` 完成一個可獨立驗證的 research-only official metadata capability probe；DEV-79 至 DEV-95 的工程安全約束仍全部適用：
