...
|---|---|---|---|---|
| Development | `DEV-103-ml-raw-pit-publication-lineage-validation-20260805` | 既有 scheduled ML raw PIT publication `pit-413ff34c1d2c6cc9fdaa2849` 以唯讀方式完成 lineage custody 驗證：decision_at=`2026-08-05T08:30:00+08:00`、9 shards、142,535 observation rows；`all_field_enriched` 與 `core_long_history` 各 52 features／52,873 rows／332,329 values／3 shards，`research_shadow_all_fields` 為 32 features／36,789 rows／365,558 values／3 shards。publication、dataset、eligibility、compressed/content shard hash、row/value/mask/schema 與 conservation checks 全部 PASS；TEMP result SHA=`sha256:42453268247f7cf21536acd75dd336fc2f227e7cde03813b0a515363ca7ad006`、manifest SHA=`sha256:f1a47f8a06f953cf8d722aca5f3d0dfc80050f8c068314af0bea63a7ea10927e`、self-check SHA=`sha256:33f99e99b9487c9c6ff1bd498e47604dbee142894489ae3ec2ea733b174c0fe7`；source DB fingerprint=`sha256:23ad4fb019ac76d1eca0ea36a6b33e4d7719e7530a9bab1187d565a332480665`、`query_only=true`、`sqlite_mode=ro`、`direct_training_input=false` | formal-labelled raw rows 只有 `available_at`／`event_at`，52-feature datasets 的 row quality 全為 `not_provided`，每份有 319,934 formal-training-eligible values、12,395 missing、328,909 stale，沒有 announced／first_seen／effective／revision lineage；shadow dataset 365,558 values 全部 research_shadow，含 268 個 fundamental values、231 quality-blocked values、29 observed row-quality rows，但全部 formal_training_eligible=false。這是 custody snapshot，不是 source acceptance、numeric backfill 或 direct training input | 保留 TEMP result／manifest；numeric backfill rows=`0`、candidate delta=`0`、aggregate 維持 `47 / 5,363 / 1,987 / 179,271 / 110 bp`。不寫正式 DB／feature／model、不 materialize、不 fit／training；待完成 as-of decision-row join、t-1 causal portfolio state、matured targets/horizon labels、4+ purged walk-forward folds 與每個 training decision 的 staleness recomputation，仍不得 promotion |
...
截至本次 **2026-08-04** projection refresh，`DEV-102-datagov-history-target-detail-probe-20260805` 完成一個可獨立驗證的 research-only official target-detail capability probe；DEV-79 至 DEV-101 的工程安全約束仍全部適用：
