# ROADMAP 6M ENGINEERING V1 完工紀錄（2026-07 封存）

> **封存日期**：2026-07-06
> **定位**：本文件保存原 `docs/00_core/ROADMAP_6M_ENGINEERING.md` 中 V1 / Month 1-6 / V1.1-V1.9 的詳細完工紀錄與更新流水帳。它只作歷史追溯，不作目前 roadmap、優先順序或 gate 判斷依據。
> **目前權威**：未來 6 個月工程路線以 `docs/00_core/ROADMAP_6M_ENGINEERING.md` 的 Gate-Based Active Roadmap 為準。
...
- 截至 2026-07-06，Post-V1 第一批 evidence 增量已完成 Evidence Event Store v1 / Forward Outcome Calculator v1 / Evidence Importers v1 / E2E smoke / Forward Performance Read Model v1 / Evidence Source Persistence v1 / Forward Performance Dashboard read-only UI v1 / Evidence Pipeline Runner dry-run v1 / working-copy DB smoke v1 / Live vs Research Gap linkage v1 / Signal Decay Monitor v1 / Decision Quality Review v1 / Evidence Review Dashboards read-only UI pack v1 / Evidence Review UI smoke checklist / multi-day dry-run record scaffold / safe scheduled dry-run wrappers / V1.5 Data Credibility & Corporate Action Gate v1 / V1.6 Cross-sectional Factor Pipeline v1 / V1.7 Screening Matrix & Negative Evidence v1 / V1.8 Portfolio Construction & Execution Trace Sandbox v1 / V1.9 Read-only Agent MCP Evidence Access v1；safe scheduled path 已改採 CMD wrapper + Windows `schtasks.exe`，每日本機時間 04:20 做非 UI 快速資料更新、05:00 做 read-only freshness check、05:15 做 evidence dry-run report，不使用 PowerShell execution policy bypass；Codex app 另有 05:30 read-only morning report automation，只彙總既有 task / status / report，不重新執行 pipeline。這是 forward evidence 的資料底座、source capture 起點、durable Daily Decision Desk snapshot source、唯讀彙總層、Research Lab evidence inspection layer、手動 dry-run orchestration、read-only freshness check、daily evidence dry-run report、gap observation linkage、decay observation、流程覆盤資料底座、資料可信度 gate、factor attribution / negative evidence / portfolio sandbox / read-only Agent evidence access 底座、人工檢查 UI 與 QA closeout scaffold，不是 production write-mode scheduler 完成，也不是投資有效性證明。
- Post-V1 部分 design / QA 檔名保留 2026-07-05 至 2026-07-12 的里程碑命名；檔名日期不作為交付日依據。
...
- V1.6：Cross-sectional Factor Pipeline & Sector Rotation v2 已完成。參考 Qlib / Zipline 的 pipeline 思路，把 market / sector / concept / liquidity / smart money / fundamental diagnostics 寫成可追溯 factor snapshot，不直接改 `ScoringEngine`。
- V1.7：Screening Matrix & Negative Evidence 已完成。把 pass / fail / degraded / skipped / missing、Why Not、Liquidity exclusion 與負面 evidence 持久化到與 recommendation 同等的 evidence layer；舊 recommendation 缺 matrix / payload 時只診斷，不回補、不重算。
- V1.8：Portfolio Construction & Execution Trace Sandbox v1 已完成。只做 research-only allocation / constraints / virtual order lifecycle，不自動下單。
...
- 不把強化學習、LLM agent、GPU portfolio optimization 或券商自動下單提前放入近期主線；這些必須等資料治理、evidence 樣本與人工核准邊界成熟後再評估。
...
- V1.2 replay credibility v1：Profile replay comparison 支援訓練 / 獨立驗證期間；推薦組合 replay 補上 rolling Sharpe / Sortino、VaR / CVaR、drawdown duration、turnover approximation、台股微結構 preflight 與 benchmark / industry / concept relative attribution。

後續交付：

- Portfolio Replay execution model residual：零股、買賣價差、完整撮合、跳空實際成交價格調整、成交率與未成交原因深化。
- 正式微結構資料源接入：處置股、分盤、全額交割、漲跌停鎖死與除權息時間軸需補 governed source、available_date、quality 與 missing policy。
- factor attribution 與 forward performance 的保存後讀取與儀表化。
...
- Signal Decay Monitor：v1 已可讀取已保存 forward evidence 與 live gap observation，依 event_type、event_family、strategy_version、profile scope 產生 append-only decay observation 與 lifecycle proposed payload；不自動套用 demote / retire，不改策略狀態。
...
- Live vs Research Gap：linkage v1 已可保存 portfolio source trace、Research Run / strategy version id 與 Evidence Event / Outcome 的 gap observation，Research Lab read-only UI 已可檢查 source trace、match confidence、gap metrics、attribution、quality 與 warnings；後續仍需更完整人工 override / real trade linkage，才可深化實帳歸因。
- Signal Decay Monitor：v1 已可追蹤 event_type、event_family、strategy_version、profile 的近期 evidence 是否相對長窗轉弱，提供 hold / watch / demote_candidate / retire_candidate 的 proposed evidence payload，Research Lab read-only UI 已可檢查 scope、短窗 / 長窗樣本、decay score、confidence、quality 與 warnings；但不自動刪除、降級或覆寫策略版本，factor_name scope 留待後續。
- Decision Quality Review：v1 已建立週 / 月 / custom 覆盤紀錄、review item、action item、source trace / journal / manual override / large gap / signal decay coverage 檢查與 process quality bp，Research Lab read-only UI 已可檢查 review item 與 process score；下一步是人工 workflow polish。
...
4. V1.6 Cross-sectional Factor Pipeline v1 已完成：daily factor snapshot DTO / repository / migration、`FactorService` / `FactorGate` pipeline、integer rank / quantile、sector / concept metadata、concept available-date gate 與 read-only attribution summary CLI 已落地；不改 `ScoringEngine`、不把 rank 當推薦、不啟用 production scheduler。V1.7 Screening Matrix & Negative Evidence v1 已完成：新保存的 Recommendation result 會保存 pass / fail / degraded / skipped / missing matrix、Why Not / Liquidity payload，capture 可產生 screening matrix 與 exclusion events；舊 result 缺 matrix / payload 時只診斷、不回補、不重算。三大法人、信用交易與 TDCC 只能在 source / available_date / quality / missing policy 完整後接入。
5. V1.8 / V1.9 保持研究邊界：V1.8 Portfolio construction / execution trace 已完成 research-only sandbox；V1.9 read-only AI / MCP 已完成 evidence access v1，不自動交易、不自動 lifecycle action；`cuFOLIO` 與 RL 暫列 deferred。
...
- Signal Decay Monitor：只能保存 decay observation 與 proposed payload，不得改 Strategy Lifecycle state、不得自動 demote / retire、不得把小樣本近期轉弱解讀為策略失敗；缺 benchmark 或 live gap 時必須降低 confidence。
- Evidence Review Dashboards：Research Lab UI 只能讀 dashboard service；不得直接讀 SQLite、repository、scoring、portfolio mutation 或 scheduler；Decision Quality score 只能作流程 evidence，不是投資能力或責備判斷；Live vs Research Gap 在缺真實交易與人工 override 時只能稱為 research / simulated gap。
- Evidence Operations History：weekly review history 只保存人工覆盤快照；`--save-history` 必須指定 explicit DB，Research Lab `覆盤歷史` 只能讀 dashboard service，不得建立 scheduler、不得寫 action item、不得自動套用 lifecycle action。
- Evidence scheduler approval：production scheduler 現在未啟用；未來也必須先完成 manual run、multi-day dry-run record、working-copy confirm smoke、dashboard human review、manual approval checklist 與 production scheduler design，且需 explicit human approval 後才可 implementation。Scheduler 不得自動 lifecycle action、不得自動交易、不得自動寫 production DB。
...
- Screening Matrix / Negative Evidence：推薦候選的 pass / fail / degraded / skipped / missing 必須來自推薦當下流程保存的 matrix；Why Not / Liquidity exclusion 只可消費已保存 payload，不可用 importer 事後重算，也不可回補舊 result。
...
- 2026-07-05：完成 V1.6 Cross-sectional Factor Pipeline v1，新增 cross-sectional factor snapshot DTO / repository / migration、FactorGate-backed pipeline、concept basket available-date gate、integer rank / quantile 與 attribution summary CLI；保持研究治理邊界，不改 `ScoringEngine`、不啟用 scheduler。
- 2026-07-05：完成 V1.7 Screening Matrix & Negative Evidence v1，新增 recommendation screening matrix 保存、screening matrix evidence events、Why Not / Liquidity payload capture 與 source coverage warning；舊 result 不回補、不重算，不改 `ScoringEngine`、不啟用 scheduler。
- 2026-07-05：完成 V1.8 Portfolio Construction & Execution Trace Sandbox v1，新增 research-only allocation / constraints / lot sizing、virtual order lifecycle trace 與 sample CLI；不串 broker、不寫正式資料、不啟用 scheduler。
- 2026-07-06：完成 V1.9 Read-only Agent / MCP Evidence Access v1，新增 read-only evidence service、`twstock-evidence-access` MCP server、permission model 與 AI report template；不寫 DB、不改策略、不下單、不套用 lifecycle action。
...
- 2026-07-01：完成 Forward Performance Dashboard read-only UI v1，新增 Research Lab `Forward Evidence` 分頁、dashboard service / DTO / Qt table model 與禁用交易語氣測試；Dashboard 只檢查已保存 evidence outcomes，不寫 evidence、不做 scheduler、不宣稱 alpha。
- 2026-07-01：完成 Evidence Pipeline Runner dry-run v1，新增 manual runner service / DTO / CLI、diagnostics Markdown/JSON report、confirm gate、production-like DB guard 與 readiness summary；scheduler 最高只到 `ready_for_manual_confirm`，production scheduler 仍未啟用。
- 2026-07-01：完成 working-copy DB smoke v1、scheduler readiness evaluator 與 production scheduler approval checklist；可在 working-copy DB 重複 confirm 檢查 idempotency，readiness evaluator 固定 `production_scheduler_allowed=false`，正式 scheduler 仍未啟用。
- 2026-07-01：完成 Live vs Research Gap linkage v1，新增 gap DTO / repository / service / capture / inspect CLI 與 matching policy；gap observation 是 evidence，不是完整實帳歸因或 lifecycle action。
- 2026-07-01：完成 Signal Decay Monitor v1，新增 decay DTO / repository / service / capture / inspect CLI 與 lifecycle proposed payload；decay observation 是 evidence，不是自動 lifecycle action，也不證明任一訊號有效或失效。
- 2026-07-01：完成 Decision Quality Review v1，新增 review DTO / repository / service / capture / inspect CLI、review item detection、integer bp process score 與 action item；Decision Quality 是流程 evidence，不是投資能力或責備判斷。
- 2026-07-01：完成 Evidence Review Dashboards read-only UI pack v1，將 Research Lab evidence 檢查整合為 `Evidence Review` 分頁，內含 Forward Evidence、Live vs Research Gap、Signal Decay 與 Decision Quality；Dashboard 只讀已保存 evidence / observation / review，不寫 evidence、不建立 scheduler、不自動 lifecycle action、不宣稱 alpha。
- 2026-07-02：新增 Evidence Review manual smoke checklist、multi-day dry-run record template 與 scheduler approval SOP；這些是正式 scheduler 前的人工 QA scaffold，不代表 production scheduler 已啟用，也不代表 alpha 或策略有效。
- 2026-07-02：新增 safe scheduled dry-run wrappers、register / unregister scripts、morning check guide 與 scheduler QA；每日自動範圍只包含 read-only data freshness check 與 evidence pipeline dry-run，working-copy smoke 預設 disabled / manual-only，production write-mode scheduler 仍未啟用。
- 2026-07-02：因 PowerShell `.ps1` 被 local execution policy 擋住，新增 CMD wrapper + Windows `schtasks.exe` 實際註冊路徑；每日本機時間 05:00 執行 freshness check、05:15 執行 evidence dry-run，仍不建立 production confirm schedule。
- 2026-07-02：新增 Codex app read-only morning report automation；每日本機時間約 05:30 只讀 Windows Task Scheduler 狀態、latest status、最新 report 與必要 log，產生繁體中文摘要，不重新執行 pipeline、不建立或修改 Windows task。
- 2026-07-04：新增每日 04:20 非 UI 快速資料更新排程 wrapper，與 05:00 freshness / 05:15 evidence dry-run 串成 read-only evidence morning check 節奏；此 task 只更新 market data CSV / SQLite，不寫 production evidence DB、不跑 UI、不做 portfolio / lifecycle action。
- 2026-07-04：完成 Post-V1 Evidence Source Persistence v1，新增 durable Daily Decision Desk snapshot repository / capture CLI / inspect CLI、capture evidence durable provider wiring、Recommendation exclusion payload optional fields 與 source coverage CLI；當時 Why Not / Liquidity payload 仍為 partial，已由 2026-07-05 V1.7 screening matrix / negative evidence 承接，scheduler 仍不具 production readiness。
