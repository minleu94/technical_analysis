...
- **動態參數範圍設定**：為每個策略參數設定
  - 模式選擇：固定值 / 範圍
  - 範圍模式：最小値、最大値、步長
- **執行參數掃描**：背景執行，顯示進度條與完成組數 (x/y)
- **結果顯示**：在「最佳化」Tab 顯示排名表格，支援雙擊表格行套用參數到主表單
...
- ✅ 策略預設：100%
- ✅ 選股清單：100%
- ✅ 推薦組合回測 MVP：100%（已整合 Sharpe Ratio、Sortino Ratio 與 Monte Carlo P05/P50/P95 穩健分析）
- ✅ 參數最佳化：100%（含進度回調與雙擊套用參數）
- ✅ Walk-forward 驗證：100%
...
### 高優先級
1. **批次回測進度顯示**：在進度條旁顯示詳細進度文字（如正在回測 2330 (3/10)）

### 中優先級
2. **回測結果匯出**：支援將結果匯出至 Excel/PDF 報告
3. **結果篩選與搜尋**：在比較 Tab 中支援按策略、股票或日期篩選與搜尋歷史記錄
4. **圖表互動功能**：提升分析體驗
