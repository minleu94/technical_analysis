...
10. [持倉管理 - 籌碼面監控與下鑽](#持倉管理---籌碼面監控與下鑽)
...
### 功能說明
「數據更新」Tab 已整理為維運工作台，保留原本所有資料更新、合併、狀態檢查與進階維護功能，但改用左側導覽分組，讓日常操作與高風險操作分開。

### 主要區域
- **狀態摘要**：顯示每日股價、大盤指數、產業指數、券商分點與技術指標狀態
- **左側導覽**：切換每日股價、大盤指數、產業指數、券商分點、技術指標與進階維護
- **工作區**：顯示目前資料來源的檢查、更新、合併或技術指標計算控制
- **日誌與進度**：集中顯示背景任務進度、成功訊息與錯誤訊息
...
1. 打開「數據更新」Tab
2. 設定查找範圍或日期範圍
3. 點擊「安全更新（完整 CSV + SQLite）」
4. 等待流程完成，並查看底部日誌

快速更新會直接同步近期資料到 SQLite，避免重寫大型歷史 CSV。安全更新會依序執行完整原始檔、合併檔與 SQLite 同步：
- 檢查目前資料狀態
- 更新每日股價
- 將每日股價 CSV 同步到 SQLite
- 更新大盤指數
- 將大盤指數同步到 SQLite
- 更新產業指數
- 將產業指數同步到 SQLite
...
- 將合併後每日資料同步到 SQLite
- 合併券商分點資料
- 將合併後券商分點資料同步到 SQLite
- 以智慧增量模式計算技術指標

若其中任一步驟失敗，流程會停止並指出失敗步驟；此時可切換到對應左側頁面單獨重試。

安全更新的資料防護原則：
- 狀態檢查必須是唯讀；券商分點 registry 不會因狀態檢查自動修復或寫回。
- SQLite 同步只從本次已產生或已合併的 CSV 結果寫入資料庫，不會反向覆蓋 CSV；每日股價同步採日期範圍替換，避免清空未碰到的歷史資料。
- 備份檔只保留在 `meta_data/backup/`，同一來源同一天只留最新一份，且最多保留最新 5 個日期版本，降低大型 CSV 備份對硬碟的壓力。
- 大盤指數若 TWSE API 失敗而使用 yfinance 備援，只接受目標日或目標日前最近交易日資料；不使用目標日之後的資料補寫。
- 技術指標計算一律使用 `TWStockConfig` 解析的資料路徑，不使用硬編碼正式資料根目錄。

### 狀態檢查來源
「全部資料」頁使用輕量 Overview，只讀取檔案存在狀態、尾端日期、檔案大小、修改時間與最近詳細檢查快取，不會一次全量掃描所有大型檔案。切換到單一資料來源頁或按該頁的「檢查狀態」時，才執行較完整的 Detail 檢查。

狀態摘要優先顯示系統實際消費的彙整檔，而不是每次全量掃描原始 daily 目錄：
- 每日股價：`meta_data/stock_data_whole.csv`
- 大盤指數：`meta_data/market_index.csv`
- 產業指數：`meta_data/industry_index.csv`
- 券商分點：各分點 `broker_flow/<branch>/meta/merged.csv`
- 技術指標：`meta_data/all_stocks_data.csv` 與 `technical_analysis/*_indicators.csv`

詳細檢查結果會寫入 `meta_data/data_status_manifest.json`，供下次 Overview 快速顯示最近檢查結果。券商分點 registry 修復屬於實際更新/合併維護行為，不會由 Overview 或狀態檢查自動觸發。

### 技術指標智慧增量
日常安全更新不會每次強制全量重算技術指標。系統會依既有指標最新日期判斷是否需要重算；若技術指標已到最新股價日期，快速 / 安全更新與每日股價背景補齊會直接跳過。只有股價有新日期時，才往前回看 120 個交易日後重新計算並覆蓋重疊區間，避免 RSI、MACD、MA、KD 等需要歷史序列的指標因只計算新日期而和全量重算不一致。技術指標輸入、個股輸出與 `all_stocks_data.csv` 合併輸出都必須由 `TWStockConfig` 或呼叫端明確提供路徑，避免誤寫非預期資料根目錄。需要修復或完整重建時，再切到「技術指標」頁使用強制全量模式。

### SQLite 資料檢視與 CSV 匯出

- 左側「SQLite 資料檢視」提供資料表預覽、Schema 與受控篩選，僅供唯讀檢查。
- SQLite 日期篩選預設空白；打開日曆會定位到今天，單一日期可按「今日」帶入今天，區間旁「清除」會清掉所有日期條件。`broker_flows` 的券商分點可用下拉選單選取，也可手動輸入關鍵字。
- 個別資料來源頁的「匯出 CSV 備案」可匯出最近範圍或全部歷史，使用 UTF-8 with BOM。

### 高風險維護

每日股價頁的 Danger Zone 與技術指標全量重算只在資料結構修復、歷史資料重建或算法變更時使用。
...
- 新版策略配置使用 `config_schema_version >= 1` 時，啟用的指標必須提供完整且型態正確的必要參數。
- 停用的指標會完全跳過驗證與計算；拼錯參數名稱、字串代替整數、越界值或 MACD / SAR / MA 跨欄位衝突會直接拒絕分析。
- 推薦評分權重使用 `pattern`、`technical`、`volume` 三個整數基點，總和必須等於 `10000 bp`。
- 評分核心使用 Decimal，總分固定量化至 `0.01` 分。治理錯誤不會降級成空推薦結果。
...
- **一鍵送回測**：把目前推薦結果中的股票清單送到策略回測，適合檢查「這些股票」用某個策略跑起來如何。
- **送推薦組合回測**：把目前 Profile/Config 送到策略回測，在歷史時間點重新跑推薦邏輯，適合評估「這套推薦系統」本身好不好。
...
### 批次回測並行與取消
- 在選股清單模式可勾選「並行執行」，由系統依 CPU 核心數與清單規模分配工作；小型清單可能自動改用循序執行。
- 執行期間可按「取消」。按鈕顯示「正在取消...」時，系統會停止提交新工作並等待已開始的工作安全結束。
...
1. 在「推薦分析」Tab 選擇 Profile，執行推薦分析。
2. 點擊「送推薦組合回測」，系統會切到「策略回測」Tab。
3. 在左側確認「推薦組合回測」區有顯示 Profile 名稱。
4. 設定回測頁既有的開始日期、結束日期與初始資金。
5. 第一次測試建議設定：每次推薦檔數 10、每期候選上限 20、持有天數 5、重播頻率「每週重播」、資金配置「等權配置」。
6. 點擊「執行推薦組合回測」。
7. 到右側「推薦組合」結果頁查看總覽、期間明細、個股貢獻與交易紀錄。

如果 1 個月候選 20 有結果，再放大到 6 個月、候選 50。正式資料 6 個月候選 50 已驗證可產生結果，但會比一般單檔回測慢。
...
- **只有通過升級條件且交易次數 >= 10 的回測結果才能 Promote**
...
## 策略回測 - 疑難排解：回測 0 交易與高價股限制

### Q1: 為什麼買進分數達標的天數很多，但總交易次數卻是 0 次？
* **台股整股 1000 股限制**：台股撮合模擬器買入股數必須是 **1000 股的整數倍**。
* **高價股資金不足取整**：如果買進 1000 股高價股（如 2330 價格為 800 元，一張需要 80 萬元）所需要的資金超出 Sizing 金額或可用現金。模擬器在進行股數取整時，會將股數 `/ 1000 * 1000` 無情地取整為 **0 股**，進而導致買入被拒絕。
* **解決辦法**：
  1. 將 UI 中的 **「初始資金」** 調大（例如從 100 萬調大到 500 萬或 1000 萬）。
...
* **操作與標記**：
  ...
  3. 系統會自動將備註填入為「**來自回測 (強制平倉)**」。
  4. 儲存時，會自動在 `source_summary` 字典欄位中標記 `exit_reason="強制平倉"`，確保存存系統中追溯來源清晰明確。
...
**注意事項：**
- 日常維護可在「數據更新」的「技術指標」頁執行智慧增量或強制全量計算，也可用 CLI 手動執行
- 計算完成後，技術指標文件會保存在 `technical_analysis/` 目錄
- 增量更新時，若既有指標已到最新股價日期會直接跳過；若有新股價，系統會回看 120 個交易日重算重疊區間並合併新舊數據，不會刪除舊日期資料
...
### 追蹤的分點
系統目前由 `{meta_data_dir}/broker_branch_registry.csv` 管理啟用分點，現行 registry 已擴充至 37 個分點。不要在 Manual 固定複製完整清單，實際追蹤範圍以 registry 的啟用列與 Smart Money 的分點下拉選單為準。
...
#### 步驟 2：合併資料
1. 更新完成後，點擊「合併券商分點」
2. 系統會合併每日資料並同步到 SQLite `broker_flows`
3. 合併後的資料會依唯一鍵去重
...
每日 CSV 與 SQLite 記錄包含分點、股票、方向、張數、金額、排名與品質欄位。常用欄位：
- `date`：日期（YYYY-MM-DD）
...
- `counterparty_broker_code`：股票/對手代碼，依目前解析契約保存
- `counterparty_broker_name`：股票/對手名稱
- `buy_qty`：買進數量
- `sell_qty`：賣出數量
- `net_qty`：淨數量（買進 - 賣出）
- `lots_quality` / `amount_quality`：`observed`、`estimated` 或 `unavailable`
- `lots_rank` / `amount_rank`：MoneyDJ 張數榜與金額榜的獨立排名

MoneyDJ `c=E` 張數榜與 `c=B` 金額榜是各自獨立的 Top 50，系統採 union 保存，不得把未出現在其中一榜的值硬補成 0。
...
### 功能介紹
「持倉管理」Tab 的左側工作區新增了 **「籌碼監控」** 分頁。此功能可自動關聯您持倉個股的券商分點成交數據，評估近期（預設 5 日，亦可切換）的主力籌碼動向。

### 籌碼評估指標
- **累計買進/賣出股數**：展示追蹤的主力分點累計交易股數（以股為單位，1000 股 = 1 張）。
- **累計淨超股數**：主力分點淨流入或流出的股數。
- **籌碼集中度**：主力買超/賣超前幾大分點的資金集中程度，判定籌碼是否過於集中。
- **連續天數**：主力連續淨吸籌（正數）或出貨（負數）的天數。
- **資料品質**：MoneyDJ 張數榜與金額榜各自只顯示買超/賣超 Top 50。系統會標示真實張數、金額估算與不可用事件比例。

### 智能風險評級
系統根據上述指標進行多空分析，在「風險評級」欄位標示：
- 🟢 **bullish** (偏多/吸籌)：主力連續 3 日以上淨吸籌，或累計大額淨買進達 500 張（500,000 股）以上。
- 🟡 **neutral** (中性)：籌碼無明顯方向或多空分歧，累計淨超量溫和。
- 🔴 **bearish** (偏空/出貨)：主力連續 3 日以上淨出貨，或累計大額淨賣出達 500 張（-500,000 股）以上。建議密切關注其出貨風險。

### 雙向下鑽定位操作指南
當持倉股出現 **bearish** 警告或其他異常時，您可以快速進行下鑽分析：
1. 切換到「持倉管理」->「籌碼監控」分頁。
2. 點選該檔出現偏空警告的個股。
3. 點擊表格下方的 **「🔍 下鑽詳細主力流向」** 按鈕。
4. 系統將自動：
   - 切換主 Tab 至「市場觀察」。
   - 切換子 Tab 至「主力流向 (Smart Money Flow)」。
   - 在主表 Scanner 中自動尋找該股，將其高亮選取並自動滾動到該列。
   - 在右側明細表中，自動加載該股的所有主力分點進出明細，便於分析是哪些特定分點在拋售。
...
2026-06-11 前的既有資料可能只有 MoneyDJ `c=B` 仟元金額。這些資料不會被當成 0 張：

- 有當日有效收盤價：折算為估計張數，UI 顯示「金額估算」並依覆蓋率調降信心度。
- 無有效收盤價：標記為不可用，只排除該事件。
- 新更新資料會同時抓取 `c=E` 與 `c=B`，並保存兩榜各自排名。
