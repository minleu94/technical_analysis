...
詳細檢查結果會寫入 `meta_data/data_status_manifest.json`，供下次 Overview 快速顯示最近檢查結果。券商分點 registry 修復屬於 Detail 或維護行為，不會由 Overview 自動觸發。

### 技術指標智慧增量
日常安全更新不會每次強制全量重算技術指標。系統會依每檔股票既有指標檔的最新日期，往前回看一段交易日後重新計算並覆蓋重疊區間，避免 RSI、MACD、MA、KD 等需要歷史序列的指標因只計算新日期而和全量重算不一致。需要修復或完整重建時，再切到「技術指標」頁使用強制全量模式。
