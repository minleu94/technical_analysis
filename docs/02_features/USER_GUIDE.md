...
> 需要從安裝開始，依 7 個頂層工作區逐步操作時，請先閱讀 [完整操作手冊](../07_guides/APPLICATION_MANUAL.md)。
...
### 技術指標智慧增量
日常安全更新不會每次強制全量重算技術指標。系統會依每檔股票既有指標檔的最新日期，往前回看 250 個交易日後重新計算並覆蓋重疊區間，避免 RSI、MACD、MA、KD 等需要歷史序列的指標因只計算新日期而和全量重算不一致。這個保守作法會比單純追加最新日期慢，但可以降低指標漂移與暖身期不足造成的錯誤。技術指標輸入、個股輸出與 `all_stocks_data.csv` 合併輸出都必須由 `TWStockConfig` 或呼叫端明確提供路徑，避免誤寫非預期資料根目錄。需要修復或完整重建時，再切到「技術指標」頁使用強制全量模式。
...
- 增量更新時，系統會回看 250 個交易日重算重疊區間並合併新舊數據，不會刪除舊日期資料
