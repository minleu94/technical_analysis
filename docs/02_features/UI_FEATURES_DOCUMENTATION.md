...
- 結果保存已接入 Research Run Registry；legacy 歷史比較仍可用，完整 Cross-run Comparison 與 Registry-based Promote 尚待 Month 2 M2-C。
...
- Research Run Registry 的基礎保存入口已完成；跨 run 比較與 registry-based Promote 仍需要 UI 入口與結果視圖。
