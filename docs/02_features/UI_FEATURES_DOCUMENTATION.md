...
- SQLite 狀態檢查、資料表檢視、唯讀 SQL preview，以及具有穩定排序、分頁控制與 stale-result 防護的檢視器分頁。
...
- 結果保存已接入 Research Run Registry；Research Lab 內新增 Registry 比較子頁，可篩選、分頁、多選 2 至 5 個 run，顯示 comparability、參數差異、normalized equity、metrics、Regime 與保存 benchmark。Registry-based Promote 尚待 Month 2 M2-C 後續任務。
...
- Research Run Registry 的基礎保存入口與第一版跨 run 比較 UI 已完成；registry-based Promote 仍需要 Gate 與 reconciliation UI/流程。
